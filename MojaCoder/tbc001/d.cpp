...
int main() {
	...
	def(ll,n);
	vl a(n); in(a);
	vector<vvm> dp(1<<n, vvm(1<<n, vm(n*2+1,0)));
	dp[0][0][n]=1;
	rep(s,1<<n)rep(t,1<<n) {
		ll bitcnts = __builtin_popcountll(s);
		ll bitcntt = __builtin_popcountll(t);
		if(bitcnts != bitcntt) continue;
		rep(i,n)if(!(s>>i&1)) rep(j,n)if(!(t>>j&1)) {
			rep(u,n*2+1){
				ll score = u + (a[i] > a[j] ? 1 : (a[i]==a[j] ? 0 : -1));
				dp[s|1<<i][t|1<<j][score] += dp[s][t][u];
			}
		}
	}

	mymint ans = 0;
	rep(u,n+1,2*n+1) ans += dp[(1<<n)-1][(1<<n)-1][u];
	out(ans);
