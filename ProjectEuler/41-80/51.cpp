...
int target(string s, string c){
	if(s.find(c[0]) == -1)return 0;
	int ret=0;
	rep(t,10){
		auto rep=regex_replace(s, regex(c), to_string(t));
		if(rep[0]=='0')continue;
		if(Prime.isPrime(stoll(rep)))ret++;
	}
	return ret;
}
...
int main() {
	// 下一桁は対象外、1,3,7,9
	// 置き換えの対象になる最小の数字は0,1,2　いずれかを含む数字だけ対象
	int c=0;
	rep(i,1,1000000)if(Prime.isPrime(i)){
		string d=to_string(i);
		bool contains = false;
		contains |= (d.find('0') != -1);
		contains |= (d.find('2') != -1);
		if(d.find('1') != -1){
		 	if(count(all(d), '1')!=1 or d.back()!='1') contains = 1;
		}
		if(!contains)continue;
		
		if(int cnt=target(d, "0")){
			if(cnt>=8) out(d,"0", cnt);
		}
		if(int cnt=target(d, "1")){
			if(cnt>=8) out(d,"1", cnt);
		}
		if(int cnt=target(d, "2")){
			if(cnt>=8) out(d,"2", cnt);
		}
	}
	out(c);
}
