...
int main() {
	ch.resize(100, vi(100,0));
	rep(n,1,1000){
		if(int v=n*(n+1)/2 ; v<10000)ch[v/100][v%100]|=1;
		if(int v=n*n ; v<10000)ch[v/100][v%100]|=1<<1;
		if(int v=n*(3*n-1)/2 ; v<10000)ch[v/100][v%100]|=1<<2;
		if(int v=n*(2*n-1) ; v<10000)ch[v/100][v%100]|=1<<3;
		if(int v=n*(5*n-3)/2 ; v<10000)ch[v/100][v%100]|=1<<4;
		if(int v=n*(3*n-2) ; v<10000)ch[v/100][v%100]|=1<<5;
	}

	// i個数字を利用し、集合Sを埋めてあり、末尾の数字2ケタがjの場合の答えの最小値
	const int INIT=1000000;
	auto calc = [&](int init){
		vector<vvi> dp(7, vvi(1<<6, vi(100, INIT)));
		dp[0][0][init] = 0;
		rep(i,6)rep(s,1<<6)rep(j,10,100)if(dp[i][s][j]!=INIT){
			rep(k,10,100)if(ch[j][k]){
				if(chmin(dp[i+1][s|ch[j][k]][k], dp[i][s][j] + (int)(j*100+k))){
					if(i+1==6 and (s|ch[j][k])==63 and k==init){
						out(dp[6][63][init]);
					}
				}
			}
