...
private:
	...
	const ull pollard_seed[7]={2, 325, 9375, 28178, 450775, 9780504, 1795265022};
public:
...
constexpr ll digit(ll a){
	...
	if(a<100000000)return 100000000;
	return 1000000000;
}

bool isPrime2(ll a, ll b){
	const int c=digit(a), d=digit(b);
	return (Prime.isPrime(b*c+a) and Prime.isPrime(a*d+b));
}

ll v1=3, v2=7;

bool calc_3_7(ll a){
	return isPrime2(a,v1) and isPrime2(a,v2);
}
...
ll calc(ll a, ll b){
	if(!isPrime2(a,b))return LINF;
	v1=a, v2=b;
	vl targets;
	
	for(auto&p:primes)if(p>v2){
		if(p>=bestAns)break;
		if(calc_3_7(p)){
	...
	ll ans=bestAns;
	rep(i,sz(targets)) rep(j,i+1,sz(targets))
	if(isPrime2(targets[i], targets[j])) rep(k,j+1,sz(targets)) {
		ll val=v1+v2+targets[i]+targets[j]+targets[k];
		if(val>=ans)continue;
		if(!isPrime2(targets[i], targets[k])) continue;
		if(!isPrime2(targets[j], targets[k])) continue;

		if(chmin(ans, val)){
			out(val, ":", v1,v2,targets[i], targets[j], targets[k]);
		}
	}
	return ans;
}
...
int main() {
	/*
		3, 7, 109, 673
		任意の２数を任意の順で結合しても素数になる素数の集合
		条件を満たす5要素の集合のうち、和が最小のものを求める
	*/
	/*
		26033 : 13 5197 5701 6733 8389
	*/
	rep(i,2,1000000)if(Prime.isPrime(i))primes.emplace_back(i);

	rep(i,sz(primes)) rep(j, i+1,sz(primes)){
		if(Mgr::timer.get() > 9000)break;
		chmin(bestAns, calc(primes[i],primes[j]));
	}
