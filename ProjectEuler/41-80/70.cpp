...
vl euler_phi_vec(ll n) {
  ...
  rep(i,2,n)if(ret[i]==i)for(ll j=i;j<n;j+=i)ret[j]-=ret[j]/i;
  return ret;
}

int main() {
	auto phi = euler_phi_vec(10000000);
	
	ld val=1e18;
	int ans=0;

	rep(i,2,10000000){
		string s=to_string(i), t=to_string(phi[i]);
		sort(all(s)); sort(all(t));
		if(s!=t)continue;
		if(chmin(val, (ld)i/phi[i])) ans=i;
	}
	out(ans);
}
// 8319823
