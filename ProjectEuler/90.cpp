...
bool judge(vi a, vi b, int v){
	if(binary_search(all(a), 6))a.emplace_back(9);
	else if(binary_search(all(a), 9))a.emplace_back(6);
	if(binary_search(all(b), 6))b.emplace_back(9);
	else if(binary_search(all(b), 9))b.emplace_back(6);
	int c1=v/10, c2=v%10;
	bool f1=false, f2=false;
	for(auto ai:a)f1|=ai==c1;
	for(auto bi:b)f2|=bi==c2;
	return f1 and f2;
}

bool isAccept(vi&a, vi&b){
	const static vi tgt{1,4,9,16,25,36,49,64,81};
	bool ok=true;
	for(auto&v:tgt) ok &= judge(a,b,v) or judge(b,a,v);
	return ok;
}
...
int main() {
	...
	do {
		kouho.emplace_back();
		rep(i,6)kouho.back().emplace_back(p[i]);
	} while(next_combination(all(p),6));

	ll ans=0;
	rep(i,kouho.size())rep(j,i,kouho.size()){
		if(isAccept(kouho[i],kouho[j]))ans++;
	}
	out(ans);
}