/* 問題概要
 * 1e5頂点の木、辺は双方向で、辺上には英小文字が1つ書かれている
 * パス（一意な最短路）上の文字を並び変えて回文にできるような頂点対の個数は？
 *
 */
...
struct node{
	bool visited = false;
	valarray<int> childrenCharEven;
	valarray<int> childrenCharOdd;
	int partialCount = 0, parent = -1;
	node(){
		childrenCharEven.resize(26, 0);
		childrenCharOdd.resize(26, 0);
	}
};
vector<node> nodes;
...
void dfs(int from=0){
	...
	for(auto[to,chr]:edge[from]){
		if(nodes[to].visited){
			nodes[from].parent = to;
			continue;
		}
		dfs(to);
		auto even = nodes[to].childrenCharEven;
		auto odd = nodes[to].childrenCharOdd;
		swap(even[chr], odd[chr]);
		nodes[from].partialCount += nodes[to].partialCount;
		nodes[from].childrenCharEven += even;
		nodes[from].childrenCharOdd += odd;
	}
	nodes[from].partialCount++;
	nodes[from].childrenCharEven+=1;
	
	// fromをLCAとして
	if(from==0)for(auto[to,chr]:edge[from]){
		if(to == nodes[from].parent)continue;
		auto even = nodes[to].childrenCharEven;
		auto odd = nodes[to].childrenCharOdd;
		swap(even[chr], odd[chr]);
		
		puta(to,":",nodes[to].partialCount);
		rep(i,26)cout<<even[i]<<" ";
		cout<<endl;
		rep(i,26)cout<<odd[i]<<" ";
		cout<<endl;
		puta("===");
...
int main(){
	...
	nodes.resize(n);
	rep(n-1){
		geta(int,a,b); geta(char,c);
		edge[a-1].emplace_back(b-1,c-'a');
		edge[b-1].emplace_back(a-1,c-'a');
	}
	dfs(0);
}
