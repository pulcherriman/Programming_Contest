...
int main() {
	...
	vb u1(26,false),u2(26,false);
	for(char c:s1)u1[c-'a']=true;
	for(char c:s2)u2[c-'a']=true;

	vvb t1(n),t2(n);
	rep(i,n){
		t1[i].resize(26, false);
		t2[i].resize(26, false);
		string s;
		cin>>s;
		for(char c:s)t1[i][c-'a'] = true;
		cin>>s;
		for(char c:s)t2[i][c-'a'] = true;
	}

	rep(x,26)rep(y,26)if(u1[x] && u2[y]){
		bool ok=true;
		rep(i,n){
			if(t1[i][x] and t2[i][y]){
				ok=false;
				break;
			}
		}
