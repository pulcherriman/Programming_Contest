...
vl solve(){
	def(int, n, m, q);
	vector<HashMap<int,ll>> edge(n);
	vector<HashMap<int,ll>> memo(n);
	...
	rep(q){
		def(int,a,b); a--; b--;
		if(edge[a].size() > edge[b].size())	swap(a, b);

		if(memo[a].find(b)!=memo[a].end()){
			ans.push_back(memo[a][b]);
			continue;
		}

		ll val=0;
		for(auto&[to,cost] : edge[a]){
			if(edge[b].find(to)!=edge[b].end()){
				val+=min(cost, edge[b][to]);
			}else if(to==b){
				val+=cost*2;
			}
		}
