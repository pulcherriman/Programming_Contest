...
void solve() {
	...
	vl mn(n+1,0), mx(n+1,0);
	for(int i=0;i<n;i++) {
		mn[i+1]=*min_element(all(a[i]));
		mx[i+1]=*max_element(all(a[i]));
	}

	rep(i,n){
		chmin(dp[i+1][0], dp[i][0] + abs(mn[i]-mx[i+1]) + abs(mn[i+1]-mx[i+1]));
		chmin(dp[i+1][0], dp[i][1] + abs(mx[i]-mx[i+1]) + abs(mn[i+1]-mx[i+1]));
		chmin(dp[i+1][1], dp[i][0] + abs(mn[i]-mn[i+1]) + abs(mn[i+1]-mx[i+1]));
		chmin(dp[i+1][1], dp[i][1] + abs(mx[i]-mn[i+1]) + abs(mn[i+1]-mx[i+1]));
	}
	out(min(dp[n]));
}
