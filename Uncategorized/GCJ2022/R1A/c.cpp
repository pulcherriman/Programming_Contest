...
void solve_min() {
	...
	vi mn(w,INF);
	rep(i,e)rep(j,w)chmin(mn[j], ex[i][j]);
	ll ans=sum(mn) * 2;
	rep(i,e)rep(j,w)ex[i][j]-=mn[j];
	map<vi, int> dp;
	dp[vi()]=ans;
	rep(i,e){
		map<vi, int> ndp;
		vi nxt;
		rep(j,w)rep(ex[i][j])nxt.push_back(j);
		do {
			for(auto&[p,a] : dp){
			// pからnxtにするための最小コスト
			int cost = a;
			...
			cost += sz(p) - sameId - 1;
			cost += sz(nxt) - sameId - 1;
			if(ndp.find(nxt) == ndp.end()) ndp[nxt]=cost;
				else chmin(ndp[nxt], cost);
			}
	...
	ans = INF;
	for(auto&[p,a] : dp) chmin(ans, a + sz(p));
	out(ans);
}
