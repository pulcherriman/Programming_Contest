...
void set_pixel_to_black(int x, int y, int c=2){
	ans[y][x]|=c;
}

void draw_circle_perimeter(int R, int R2){
	rep(x, -R, R+1){
		int y = round(sqrt(R * R - x * x));
		set_pixel_to_black(x+R2, y+R2);
		set_pixel_to_black(x+R2, -y+R2);
		set_pixel_to_black(y+R2, x+R2);
		set_pixel_to_black(-y+R2, x+R2);
	}
}
...
void solve(){
	...
	ans= vvl(n*2+1, vl(n*2+1, 0));
	draw_circle_filled(n);
	draw_circle_filled_wrong(n);
	ll v=0;
	for(auto&a:ans)v+=count(all(a), 1);
	out(v);
