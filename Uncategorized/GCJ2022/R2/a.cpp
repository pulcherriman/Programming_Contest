...
void solve(int n, int k){
	...
	// dist[i][j]=頂点iにコストjで辿り着くための遷移元
	unordered_map<int, unordered_map<int, int>> dist;
	dist[0][0]=0;
	for(auto&[next, edge] : mp){
		int maximum = n*n-1-next;
		for(auto&[current, cost] : edge){
			if(current+cost+maximum < k)continue;
			if(dist.count(current)){
				for(auto&[cDst, _] : dist[current]){
					if(cDst+cost>k)continue;
					if(cDst+cost+maximum < k)continue;
					dist[next][cDst+cost]=current;
				}
			}
		}
	}
	...
	while(c){
		...
		int pc=dist[c][k];
		k-=mp[c][pc];
		if(c-pc!=1 and mp[c][pc]==1)ans.emplace_back(pc+1, c+1);
		c=pc;
	}
	reverse(all(ans));
	out(ans.size());
...
// Dante君は今マス1の上にいて、隣接マスへの移動で中心を目指す。
// Dante君は、数字が増える方向にしか移動できない。
// また、あるマスXからX+2以上の隣接マスへの移動を、ショートカットと呼ぶことにする。

// ちょうどK回の移動で中心にたどり着くような経路があるか判定せよ。
// また、移動可能な場合は具体的な経路を1つ構築し、ショートカットのfrom,toを順に全て出力せよ。

// 制約
// 共通：テストケース100個、1≤K≤N^2-1、Nは奇数
// 部分点1：3≤N≤9
// 部分点2：3≤N≤39
// 部分点3(満点)：3≤N≤9999
