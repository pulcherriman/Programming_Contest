...
int main() {
	...
	rep(ti,t){
		...
		// 有効な直近5文字の組を全探索（数値にする）
		vi isPalindrome5(1<<6, 0);
		isPalindrome5[0b00000] = 1;
		isPalindrome5[0b00100] = 1;
		isPalindrome5[0b01010] = 1;
		isPalindrome5[0b01110] = 1;
		isPalindrome5[0b10001] = 1;
		isPalindrome5[0b10101] = 1;
		isPalindrome5[0b11011] = 1;
		isPalindrome5[0b11111] = 1;

		vi isPalindrome6(1<<6, 0);
		isPalindrome6[0b000000] = 1;
		isPalindrome6[0b001100] = 1;
		isPalindrome6[0b010010] = 1;
		isPalindrome6[0b011110] = 1;
		isPalindrome6[0b100001] = 1;
		isPalindrome6[0b101101] = 1;
		isPalindrome6[0b110011] = 1;
		isPalindrome6[0b111111] = 1;

		unordered_set<int> st,st2;
		st.insert(0);
		rep(i,5){
			for(auto&v:st){
				if(s[i]!='1') st2.insert(v*2);
				if(s[i]!='0') st2.insert(v*2+1);
			}
			st.clear();
			swap(st, st2);
		}
		{
			for(auto&v:st){
				if(!isPalindrome5[v]) st2.insert(v);
			}
			st.clear();
			swap(st, st2);
		}
		...
		rep(i,5,n){
			for(auto&v:st){
				if(s[i]!='1' and !isPalindrome6[v*2]) st2.insert(v*2);
				if(s[i]!='0' and !isPalindrome6[v*2+1]) st2.insert(v*2+1);
			}
			st.clear();
			swap(st, st2);
			// 6桁の判定は終わったので、5桁の判定する
			for(auto v:st){
				if(v>=32)v-=32;
				if(!isPalindrome5[v]) st2.insert(v);
			}
