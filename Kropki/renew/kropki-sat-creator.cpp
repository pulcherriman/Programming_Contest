...
public:
	...
	void AddRulesByRandom(){
		...
		rep(i,n){
			shuffle(all(values), Random);
			if(values[0]!=n-1) AddRulesByMark(i, values[0], i, values[0]+1, 1);
			rep(j,1,2/n){
				if(values[0]!=n-1) AddRulesByMark(i, values[j], i, values[j]+1, 0);
			}
		}
		rep(i,n-1){
			shuffle(all(values), Random);
			AddRulesByMark(i, values[0], i+1, values[0], 1);
			rep(j,1,2/n){
				AddRulesByMark(i, values[j], i+1, values[j], 0);
			}
		}
	}
...
int main(){
	geta(int, n);
	KropkiCreator creator(n);
	Timer timer(1000 * 60 * 60 * 4);
	while(timer){
		if(auto [isCreated, answer] = creator.Create(10000) ; isCreated){
			puta("Create succeeded");
			puta(creator.GetURL());
			rep(i,n)puta(answer[i]);
			cout<<endl;
			cout<<creator.GetMarkInfo()<<endl;
			cout<<endl;
		}else{
			puta("Create failed");
		}
