...
struct S{
    ll sum, product, gcd, lcm, min, max;
    static S create(ll v){ return S{v,v,v,v,v,v}; }
    static S op(S a, S b){
        return S{
            a.sum+b.sum,
            a.product*b.product,
    ...
    static S e(){ return S{0,1,1,1,LINF,0}; }
};
...
int main(){
    ...
    rep(i,n) st.set(i, S::create(i+1));
    
    puta(st.all_prod().sum);
    puta(st.all_prod().product);
    puta(st.all_prod().gcd);
    puta(st.all_prod().lcm);
    puta(st.all_prod().min);
    puta(st.all_prod().max);
