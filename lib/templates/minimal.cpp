...
//output
template<class S,class T>ostream&operator<<(ostream&os,pair<S,T>p){return os<<'['<<p.first<<", "<<p.second<<']';};
...
template<class T>void puta(T&t,false_type, ostream&os){os<<t;}
template<class T>void puta(T&t,true_type, ostream&os){
constexpr bool h=hasItr<typename T::value_type>::v;bool F=true,I;
for(auto&i:t){if(!F)os<<' ';puta(i,bool_constant<h>(),os);F=I=h;}if(!I)os<<endl;}
template<class T>void puta(const T&t, ostream&os=cout){
puta(t,bool_constant<hasItr<T>::v>(),os);if(!hasItr<T>::v)os<<endl;}
...
if constexpr(i==sizeof...(T)-1)puta(get<i>(t),os);else{os<<get<i>(t)<<' ';puta<i+1>(t,os);}}
