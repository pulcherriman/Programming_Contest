...
int main(){
	...
	string s;
	cin>>s;
	vb b={s.size()>=6,false,false,true};
	rep(i,s.size()){
		if(isdigit(s[i]))b[1]=true;
		if(isalpha(s[i]))b[2]=true;
		if(i>1 and s[i-2]==s[i-1] and s[i-1]==s[i])b[3]=false;
	}
	tf(b[0]&b[1]&b[2]&b[3],"Valid","Invalid");
	return 0;
}