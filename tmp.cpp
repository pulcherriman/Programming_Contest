...
int main(){
	...
	vvl dp(1<<n,vl(n+1,0));
	//{i}が販売を停止していて、j番目までの財宝をうったときのお金で買える商品の価値の和
	range(i,1,1<<n)rep(j,n){
		rep(k,n)if(i&(1<<k)){
			dp[i][j+1]=max(dp[i][j+1],dp[i&~(1<<k)][j]+x[j]);
		}
	}

	vvl dp2(1<<n,0);//{i}が販売を停止しているときのこたえ
	rep(i,1<<n){
		dp2[i]=dp[i][bitcnt(i)];//{i}が停止している->bitcnt(i)個財宝を売却している
	}
	range(i,1,1<<n)rep(j,n){
