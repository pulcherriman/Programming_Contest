...
struct ParameterPack {
	...
	static constexpr double startTemp   =  2000;
...
struct Solver {
	...
	int remain_kinds = S*C, errorCount = 0, remain_items = 0;
	vi hanger_on_hook;
	...
	int solve(int ashikiri = INF) {
		while (!que.empty()) {
			walk();
			if(a*X + b*Y + remain_kinds * 5 * b > ashikiri) return INF;
		}
...
void solve(ParameterPack&params, bool isBeamSearch = false, int threshold = 10) {
	...
	if(isBeamSearch)while(!solver.que.empty()){
		if( solver.que.size() >= threshold or !Marathon::baseTimer){
			solver.walk();
			continue;
		}
...
struct PermutationHash{
	...
	PermutationHash(int _len):len(_len){
		f.resize(len,1);
		rep(i,1,len)f[i]=min(i*f[i-1], INF);
		total=f.back()*len;
	}

	vi toPermutation(int index){
		vb used(len,0);
...
int main() {
	...
	// do {
	// 	int value=0;
	// 	rep(i,C)value+=B[colors[i]][colors[(i+1)%C]];
	// 	calcs.emplace_back(value, hash++);
	// 	colorScore.emplace_back(value);
	// }while(next_permutation(all(colors)));
	// sort(all(calcs));
	// PermutationHash ph(C);
	// colors = ph.toPermutation(calcs.front().second);

	do{
		{
			debug("start small :", currentParams.penalty);
			Timer timer(epoch);
			do {
				bool isAfter = Marathon::baseTimer.get() > Marathon::TLE/2;
				params.nearby_small();
				// while(timer){
				// 	params.nearby_small();
				// 	int index = ph.toIndex(params.colors);
				// 	if(colorScore[index] <= calcs[calcs.size()/2].first) break;
		...
		{
			debug("start large :", currentParams.penalty);
			Timer timer(epoch);
			do {
				bool isAfter = Marathon::baseTimer.get() > Marathon::TLE/2;
				params.nearby_large();
		...
		params = currentParams;
		{
			...
			solve(params, true, S*C/2);
			if(chmin(currentParams, params)) bestParams = currentParams;
		}
	}while(SATimer);
	...
	// {
	// 	for(auto c:bestParams.colors)cout << c << " ";
	// 	print("");
	// 	int index = ph.toIndex(bestParams.colors);
	// 	print(colorScore[index]);
	// 	print(calcs[0].first, calcs[calcs.size()/2].first ,calcs.back().first);
	// }
	debug_f(solveCount, bestParams.penalty);
