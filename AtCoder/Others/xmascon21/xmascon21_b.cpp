...
pll solve(ll h, ll w){
	ll  cell = h*w, node=(h+1)*(w+1);
	ll maxAns=0, maxSet=0;
	ll minAns=INF, minSet=0;
	rep(s,1<<cell){
		ll val=node;
		UnionFind uf(node);
		rep(i,h)rep(j,w){
			ll ind=i*w+j;
			if((s>>ind)&1){
				if(uf.unite(ind+i, ind+i+w+2))val--;
				// puta(ind, ind+i, ind+i+w+2);
			}else{
				if(uf.unite(ind+i+1, ind+i+w+1))val--;
				// puta(ind, ind+i+1, ind+i+w+1);
			}
		}
		if(chmax(maxAns, val))maxSet=s;
		if(chmin(minAns, val))minSet=s;
		// puta(val);
	}
	return {minAns, maxAns};
}

pll solve2(ll h, ll w){
	if(h%2==0 and w%2==1){
		return {(h+w+1), (w/2*3+4)+(h/2-1)*(w+1)}; 
	}
	if(h%2==1 and w%2==0){
		return solve2(w,h);
	}
	if(h%2==0 and w%2==0){
		return {(h+w+1), 1+(h/2+1)*(w/2+1)+(h/2)*(w/2)}; 
	}
	if(h%2==1 and w%2==1){
		return {(h+w+1), 1+(w+1)*(h+1)/2}; 
	}
}
