...
int main(){
	...
	geta(ll, n, K);
	vl x(n),y(n),z(n),tx(n),ty(n),tz(n),cx,cy,cz;
	rep(i,n){
		geta(ll,a,b,c,d,e,f);
		x[i]=a,y[i]=b,z[i]=c,tx[i]=d,ty[i]=e,tz[i]=f;
		cx.push_back(a);
		cx.push_back(d);
		cy.push_back(b);
		cy.push_back(e);
	...
	Compress zipx(cx), zipy(cy),zipz(cz);
	vector<vvl> s(zipx.size(), vvl(zipy.size(), vl(zipz.size(), 0)));
	rep(i,n){
		auto [a,b,c,d,e,f] = make_tuple(zipx.zip(x[i]), zipy.zip(y[i]), zipz.zip(z[i]), zipx.zip(tx[i]), zipy.zip(ty[i]), zipz.zip(tz[i]));

		range(p,a,d)range(q,b,e)range(r,c,f){
			s[p][q][r]++;
		}
	}

	ll ans=0;
	rep(i,zipx.size()-1)rep(j,zipy.size()-1)rep(k,zipz.size()-1){
		if(s[i][j][k]>=K){
			// puta(i,j,k, zipx.unzip(i+1)-zipx.unzip(i), zipy.unzip(j+1)-zipy.unzip(j), zipz.unzip(k+1)-zipz.unzip(k));
			ans += (zipx.unzip(i+1)-zipx.unzip(i)) * (zipy.unzip(j+1)-zipy.unzip(j)) * (zipz.unzip(k+1)-zipz.unzip(k));
		}
	}
	puta(ans);
