...
void Calculate() {
	...
	rep(i,HarvesterCount) {
		...
		while (!q.empty()) {
			...
			if(y>0 and hrvField[y-1][x]!=NONE and field[y-1][x]!=NONE and !visited[y-1][x]) q.push(field[y-1][x]);
			if(x>0 and hrvField[y][x-1]!=NONE and field[y][x-1]!=NONE and !visited[y][x-1]) q.push(field[y][x-1]);
			if(y<n-1 and hrvField[y+1][x]!=NONE and field[y+1][x]!=NONE and !visited[y+1][x]) q.push(field[y+1][x]);
			if(x<n-1 and hrvField[y][x+1]!=NONE and field[y][x+1]!=NONE and !visited[y][x+1]) q.push(field[y][x+1]);
		}
		money += maxValue * cnt;
	}
}
	...
	rep(i,t){
		...
		if(HarvesterCount == 0) {
			...
			if(harvesterMoveTo[i] == NONE_COOR) {
				...
				MoveHarvester(0, harvesterMoveTo[i].first, harvesterMoveTo[i].second);
			}
		}
		Calculate();
		UpdateField_Night(i);
