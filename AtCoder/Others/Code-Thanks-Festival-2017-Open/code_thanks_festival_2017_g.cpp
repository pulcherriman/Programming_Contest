...
int main(){
    ...
    rep(i,m){
        geta(ll,a,b);
        a--,b--;
        ignore[a][b]=ignore[b][a]=1;
    }

    // 前半組の全集合に対してbitdpで解を出す
    vl dp(1<<(n/2),0);
    rep(s,1<<(n/2)){
        rep(i,n/2)if(!((s>>i)&1)){
            bool ok=true;
    ...
    rep(s,1<<(n-n/2)){
        vl used(n,0);
        rep(i,n-n/2)if((s>>i)&1)used[i+n/2]=1;
        bool enable=true;
        rep(i,n-n/2)if(used[i+n/2])
            range(j,i+1,n-n/2)if(used[j+n/2])
                enable&=!ignore[i+n/2][j+n/2];
        if(enable){
            ll s2=0;
            rep(i,n/2){
                bool ok=true;
                range(j,n/2,n)if(used[j])ok&=!ignore[i][j];
                if(ok)s2|=(1<<i);
            }
            chmax(ans, dp[s2] + sum(used));
        }
    }
    puta(ans);
