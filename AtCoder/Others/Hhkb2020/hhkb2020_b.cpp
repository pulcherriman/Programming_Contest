...
template<class Cell=int>struct GridGraph : public Graph<int>{
	...
	void create(){
		...
		rep(i,1,h-1)rep(j,1,w-1)if(get(i,j)=='.'){
			if(get(i+1,j)=='.')add(i*w+j, i*w+j+w, 1);
			if(get(i,j+1)=='.')add(i*w+j, i*w+j+1, 1);
		}
	}
...
void Main(){
	...
	vs f(h); cin>>f;
	ll ans=0;
	rep(i,h)rep(j,w-1)if(f[i][j]=='.')if(f[i][j]==f[i][j+1])ans++;
	rep(i,h-1)rep(j,w)if(f[i][j]=='.')if(f[i][j]==f[i+1][j])ans++;
