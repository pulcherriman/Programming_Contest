...
int main(){
    ...
    ll n,m;
    cin>>n>>m;
    vl x(n+m),y(n+m),c(n+m);
    vvd cost(n+m, vd(n+m,LINF));
    rep(i,n+m)cin>>x[i]>>y[i]>>c[i];
    rep(i,n+m)range(j,i+1,n+m){
        cost[i][j]=cost[j][i]=hypot(abs(x[i]-x[j]),abs(y[i]-y[j]))*(c[i]==c[j]?1:10);
    }
    double ans=LINF;
    rep(s,1<<m){
        vb use(n+m,true);
        rep(i,m)if(s&(1<<i))use[n+i]=false;
        UnionFind uf(n+m);
        vector<e> q;
        rep(i,n+m)if(use[i])range(j,i+1,n+m)if(use[j]){
