...
#pragma endregion
const int n=40;
const int seemax=n*n;
vvi e(n,vi(n));
vvi dist(seemax, vi(seemax,0));

pair<int,vvi> calc(vector<tlll>&ep, int see){
	auto power=vec(0,n,n);
	int score=0;
	rep(i,see){
		auto[ei,yi,xi]=ep[i];
		int dmin=n;
		rep(j,see)if(i!=j)chmin(dmin,dist[i][j]);
		int pow=dmin-1;
		power[yi][xi]=pow;
		score+=ei*pow;
	}
	rep(i,see,seemax){
		...
		rep(j,i){
			auto[ej,yj,xj]=ep[j];
			if(power[yj][xj]==0)continue;
			if(power[yj][xj]>=dist[i][j]){
				dmin=1;
				break;
			}
			chmin(dmin, dist[i][j]);
...
void Main(){
	...
	cin>>e;
	vector<tlll> ep;
	rep(i,n)rep(j,n)ep.emplace_back(e[i][j],i,j);
	sort(rall(ep));

	rep(i,seemax)rep(j,seemax){
		auto[ei,yi,xi]=ep[i];
		auto[ej,yj,xj]=ep[j];
		dist[i][j]=abs(yi-yj)+abs(xi-xj);
	}
	
	int maxScore=0, maxScoreSee=0;
	vvi maxPower;
	rep(see,1,100){
		auto[score, power]=calc(ep,see);
		if(chmax(maxScore,score)){
			maxScoreSee=see;
			maxPower=power;
		}
	}
	dump(maxScore, maxScoreSee);
	puta(maxPower);
