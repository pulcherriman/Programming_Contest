...
int main(){
	...
	k=min(k,accumulate(all(a),n));
	vector<vector<vm>> dp(n+1,vector<vm>(k+1,vm(2,0)));
	dp[0][0][0]=1;
	rep(i,n)rep(j,k+1)rep(l,min(k-j,a[i])+1){
		dp[i+1][j+l][l>=a[i]]+=dp[i][j][0];
		dp[i+1][j+l][1]+=dp[i][j][1];
	}
	mint ans=dp[n][k][0];
	rep(i,k+1)ans+=dp[n][i][1];
	cout<<ans<<endl;
	return 0;
}