...
int main() {
	...
	ll ans=0;
	rep(i,h)rep(j,w){
		if(i+1<h and f[i+1][j]!='#') g.add_edge(i*w+j,(i+1)*w+j,LINF,0);
		if(j+1<w and f[i][j+1]!='#') g.add_edge(i*w+j,i*w+j+1,LINF,0);
		if(f[i][j]=='o'){
			g.add_edge(h*w, i*w+j, 1, 0);
			ans += h-1-i + w-1-j;
		}
		g.add_edge(i*w+j,h*w+1,1,h-1-i + w-1-j);
	}

	out(ans - g.flow(h*w, h*w+1).second);
}