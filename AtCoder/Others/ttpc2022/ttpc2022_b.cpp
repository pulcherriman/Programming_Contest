...
int main() {
	...
	vvl dp(n+1,vl(10000, -1));// i番目まで見て、j円残している時の答え
	dp[0][k] = 0;
	rep(i,n){
		rep(j,10000)if(dp[i][j] != -1) {
			for(auto&q:p[j]){
				chmax(dp[i+1][q], dp[i][j]);
				if(q-a[i] >= 0) chmax(dp[i+1][q-a[i]], dp[i][j]+1);
			}
		}
	}
	rep(i,n+1)rep(j,10000)if(dp[i][j] != -1) {
		debug(i,j,dp[i][j]);
	}
	ll ans = 0;
	rep(i,10000) chmax(ans, dp[n][i]);
	out(ans);
