...
void Main(){
	...
	sort(all(p),[](pll a, pll b){return a.first*b.second>a.second*b.first || (a.first*b.second==a.second*b.first && a.first>b.first);});

	auto dp=vec(-1ll,n+1,h+100101);
	dp[0][h+100100]=0;
	rep(i,n){
		dp[i+1]=dp[i];
		rep(j,h+1)if(dp[i][j+100100]!=-1){
			// chmax(dp[i+1][j+100100], dp[i][j+100100]);
			chmax(dp[i+1][j+100100-p[i].second], dp[i][j+100100]+p[i].first*j);
		}
	}
	puta(max(dp[n]));
}
