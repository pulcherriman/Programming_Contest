...
void Main(){
	geta(ll, h,w);
	getv(f,0ll,h,w);
	auto dp=vec(0ll,h,w,h+w);
	dp[h-1][w-1][1]=f[h-1][w-1];
	rrep(i,h)rrep(j,w)rep(k,h+w){
		if(j+1<w)chmax(dp[i][j][k], dp[i][j+1][k]);
		if(i+1<h)chmax(dp[i][j][k], dp[i+1][j][k]);
		if(j+1<w and k>0)chmax(dp[i][j][k], dp[i][j+1][k-1]+f[i][j]);
		if(i+1<h and k>0)chmax(dp[i][j][k], dp[i+1][j][k-1]+f[i][j]);
	}
	range(i,1,h+w)puta(dp[0][0][i]);
}
