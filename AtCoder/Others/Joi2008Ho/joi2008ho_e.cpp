...
int main(){
	...
	Compress y(yv),x(xv);
	vvl s(y.size(), vl(x.size(),0));
	rep(i,n){
		s[y.zip(t[i])][x.zip(l[i])]++;
		s[y.zip(t[i])][x.zip(r[i]+1)]--;
		s[y.zip(b[i]+1)][x.zip(l[i])]--;
		s[y.zip(b[i]+1)][x.zip(r[i]+1)]++;
	}
	rep(i,y.size())rep(j,x.size()-1){
		s[i][j+1]+=s[i][j];
	}
	rep(j,x.size())rep(i,y.size()-1){
		s[i+1][j]+=s[i][j];
	}

	rep(i,y.size())s[i].pop_back();
	s.pop_back();

	vvb v(s.size(), vb(s[0].size(),false));
	ll ans=0;
		...
		if(!v[i][j] && s[i][j]==0){
			...
			while(!q.empty()){
				...
				rep(k,4){
					...
					if(ny<0 or nx<0 or ny>=s.size() or nx>=s[0].size()){
						continue;
					}
					if(!v[ny][nx] and s[ny][nx]==0)q.emplace(ny,nx);
				}
			}
			ans++;
		}
