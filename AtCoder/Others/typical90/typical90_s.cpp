...
void Main(){
	geta(ll, n);
	vl v(n*2); cin>>v;
	auto dp=vec(LINF,n+1, n*2+1);
	rep(i,2*n-1) dp[0][i]=abs(v[i]-v[i+1]);

	range(i,1,n){
		rep(j,2*(n-i)-1){
			// puta(i,j);
			// puta("init",i-1,j+1,dp[i-1][j+1],"+",abs(v[j]-v[j+1+2*i]));
			dp[i][j]=dp[i-1][j+1]+abs(v[j]-v[j+1+2*i]);
			rep(k,i){
				// puta(k,j,"+",i-1-k,j+(k+1)*2);
				chmin(dp[i][j], dp[k][j]+dp[i-1-k][j+(k+1)*2]);
			}
			// puta("!!",i,j,dp[i][j]);
		}
	}
