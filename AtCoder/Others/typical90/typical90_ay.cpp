...
int main() {
	...
	def(ll,n,k,p);
	vl a(n); cin>>a;
	vector<HashMap<ll,ll>> half(n/2+1), half2((n-n/2)+1);
	half[0][0]=half2[0][0]=1;
	rep(i,n/2){
		rrep(j,n/2)for(auto&[val, cnt]:half[j]){
			if(val+a[i]<=p){
				half[j+1][val+a[i]] += cnt;
			}
		}
	}
	rep(i,n/2,n){
		rrep(j,n-n/2)for(auto&[val, cnt]:half2[j]){
			if(val+a[i]<=p){
				half2[j+1][val+a[i]] += cnt;
			}
		}
	}
	vector<vp> h1(n/2+1), h2((n-n/2)+1);
	rep(i,n/2+1){
		h1[i] = vp(all(half[i]));
		sort(all(h1[i]));
	}
	rep(i,n-n/2+1){
		h2[i] = vp(all(half2[i]));
		h2[i].emplace_back(-1,0);
		sort(all(h2[i]));
		rep(j,sz(h2[i])-1){
			h2[i][j+1].second += h2[i][j].second;
		}
	}
	...
	rep(i,min(k+1, n/2+1)){
		int tgt = k-i;
		if(tgt>n-n/2 or tgt<0)continue;
		for(auto&[val, cnt]:h1[i]){
			auto itr=upper_bound(all(h2[tgt]), pll{p-val+1, 0});
			itr--;
			ans += cnt*itr->second;
		}
	}
