...
void Main(){
	...
	rep(i,1,h-1)rep(j,1,w-1){
		rep(k,4){
			int source=(i*w+j)*4+k;
			rep(l,k+1,4){
				int dest=(i*w+j)*4+l;
				g.add(source,dest,1);
			}
		}
		for(auto[y,x,d]:f.neighborIndex(i,j,0)){
			int source=(i*w+j)*4+d;
			int dest=(y*w+x)*4+d;
			g.add(source,dest,0,1);
		}
	}
	rep(i,4){
		g.add(h*w*4, (sy*w+sx)*4+i, 0, 1);
		g.add((ty*w+tx)*4+i, h*w*4+1, 0, 1);
	}
	
	puta(g.bfs(h*w*4)[h*w*4+1]);
}
