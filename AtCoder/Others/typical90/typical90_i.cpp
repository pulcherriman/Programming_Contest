...
void Main(){
	...
	rep(i,n){
		vector<ld> deg;
		rep(j,n)if(i!=j)deg.emplace_back( arg(p[j]-p[i])*180/PI );
		rep(j,n-1)if(deg[j]<0)deg[j]+=360;
		sort(all(deg));
		rep(j,n-1){
			int p=lower_bound(deg.begin()+j+1,deg.end(),deg[j]+180)-deg.begin();
			if(j<p and p<n-1)chmax(ans, abs(deg[j]-deg[p])<180?abs(deg[j]-deg[p]):360-abs(deg[j]-deg[p]));
			if(j<p-1 and p-1<n-1)chmax(ans, abs(deg[j]-deg[p-1])<180?abs(deg[j]-deg[p-1]):360-abs(deg[j]-deg[p-1]));
		}
	}
	fcout(10);
	puta(ans);
