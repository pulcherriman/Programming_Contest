...
template<class T=ll>struct Graph{
	...
	friend ostream &operator<<(ostream &os, const Graph<T> &g) {
		rep(i,g.size)for(auto[from,val]:g.edge[i])puta(i,"=>",from,", cost :",val,os);
		return os;
	}
	...
	struct DFSResult{
		vb connected;
		vector<T> distance;
		vi preorder, postorder, eulertour;
		DFSResult(Graph<T>&g):
...
void Main(){
	...
	loop(q){
		...
		if(t==1){
			g[y][x]=1;
			for(auto[dy,dx]:g.neighbor4(y,x,1)){
				uf.unite(y*w+x,dy*w+dx);
			}
		}else{
			geta(ll,dy,dx);
			Yn(g[y][x]==1 and uf.same(y*w+x, dy*w+dx));
		}
	}
}
