...
template<class T>
vi lis(vector<T>&a, bool strict){
	vi ret(sz(a));
	SegmentTree<SegTreeUtil::Update_Max<int>> st(max(a)+1, 0);
	rep(i,sz(a)){
		T threshold = strict ? a[i] : a[i]+1;
		int maxVal = st.query(0,threshold);
		ret[i]=maxVal+1;
		st.update(a[i], maxVal+1);
	}
	return ret;
}
...
int main() {
	...
	vi lis_left(n), lis_right(n);
	lis_left = lis(a, true);
	reverse(all(a));
	lis_right = lis(a, true);
	reverse(all(lis_right));

	rep(i,n)lis_left[i]+=lis_right[i]-1;
	out(max(lis_left));
}