...
int main() {
	/*$1*/
	auto [n,g] = Graph<>::input();
	debug(g);
	vl dist(n,-1), sameDist(n,0);
	bool bipartite=true;
	ll ans=0,cnt=0;
	rep(i,n)if(dist[i]==-1){
		ll zero=0,one=0,edgeCount=0;;
		queue<int> q;
		q.push(i);
		dist[i]=0;
		vl used;
		...
		while(!q.empty()){
			...
			for(auto&[to,_]:g.edge[p]){
				if(dist[to]==-1){
					...
					if(dist[to]%2==dist[p]%2)bipartite=false;
				}
			}
		}
		for(auto&j:used){
			if(dist[j]%2==0)sameDist[j]=zero;
			else sameDist[j]=one;
			edgeCount+=g.edge[j].size();
		}
		edgeCount/=2;
		debug(edgeCount);
		ans += zero*one - edgeCount;
		ans += (zero+one) * cnt;
		cnt += zero+one;
	}
	if(!bipartite){
