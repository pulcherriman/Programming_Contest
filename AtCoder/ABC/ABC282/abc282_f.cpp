...
int main() {
	...
	rep(q){
		def(ll,l,r);
		ll r1 = *(upper_bound(all(st[l]), r)-1);
		ll l1 = *(lower_bound(all(ed[r]), l));

		auto p1 = lower_bound(all(p), pll{l,r1}) - p.begin();
		auto p2 = lower_bound(all(p), pll{l1,r}) - p.begin();
		debug(l,r1, l1,r);
		out(p1+1,p2+1)<<flush;
	}
