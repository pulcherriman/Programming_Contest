...
template<class T=ll>
struct ZobristMultiHash {
	ZobristMultiHash() {}
	ll get_or_create(T key) {
		if(mp.find(key)==mp.end())mp[key]=Random(1ll,numeric_limits<ll>::max()) % MOD;
		return mp[key];
	}
...
int main() { /*$1*/
	...
	auto hasher = ZobristMultiHash<ll>();
	auto ha = hasher.calc_hash_seq(a);
	auto hb = hasher.calc_hash_seq(b);


	rep(q){
		def(ll,l1,r1,l2,r2);
		--l1,--l2;
		ll h1 = (ha[r1] - ha[l1] + MOD) % MOD;
		ll h2 = (hb[r2] - hb[l2] + MOD) % MOD;
		Yn(h1==h2);
	}
}