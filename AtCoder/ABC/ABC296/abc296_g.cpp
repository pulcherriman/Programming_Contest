...
D distLC(P a1, P a2, P c, D r) {
  return max(distLP(a1, a2, c) - r, 0.0L);
}
...
D distSC(P a1, P a2, P c, D r) {
  ...
  return max(distSP(a1, a2, c) - r, 0.0L);
}
...
VP crosspointLC(P a1, P a2, P c, D r) {
  ...
  P dir = sqrt(max(r*r - norm(ft-c), 0.0L)) / abs(a2-a1) * (a2-a1);
  ps.push_back(ft + dir);
...
D distCC(P a, D ar, P b, D br) {
  D d = abs(a-b);
  return GE(d, abs(ar-br)) ? max(d-ar-br, 0.0L) : abs(ar-br) - d;
}
...
VP circlesPointsTangent(P a, P b, P l1, P l2) {
  ...
  if (qd < -EPS) return cs;
  if (EQ(qa, 0)) {
    if (!EQ(qb, 0)) cs.push_back((a+b)*0.5L - m * (qc/qb/2));
    return cs;
  }
  D t = -qb/qa;
  cs.push_back(              (a+b)*0.5L + m * (t + sqrt(max(qd, 0.0L))/qa));
  if (qd > EPS) cs.push_back((a+b)*0.5L + m * (t - sqrt(max(qd, 0.0L))/qa));
  return cs;
}
...
Graph visibilityGraph(const VP& ps, const vector<VP>& objs) {
  ...
  rep (i,n) rep (j,i) {
    ...
    if (!EQ(norm(a-b), 0)) rep (k, objs.size()) {
      ...
      if ((inStA ^ inStB) % 2 || inStA * inStB != 1 && inConvex((a+b)*0.5L, obj) == 1) goto skip;
      rep (l, obj.size()) {
        P cur = obj[l];
...
int main() {
	...
	vector<string> ans={"OUT","IN","ON"};
	rep (i,q) {
		int x, y; cin >> x >> y;
		P p(x,y);
		cout << ans[inCcwConvex(ps, p)] << endl;
	}
