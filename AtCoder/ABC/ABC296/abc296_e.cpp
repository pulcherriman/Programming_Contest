...
int main() {
	...
	queue<int> q;
	rep(i,n)if(isCycle[i])q.push(i);
	while(!q.empty()){
		int i = q.front(); q.pop();
		if(isCycle[a[i]-1])continue;
		isCycle[a[i]-1] = true;
		q.push(a[i]-1);
	}

	ll ans=0;
	rep(i,n)if(isCycle[i])ans++;
	out(ans);

	// vvl parent(n);
	// rep(i,n){
	// 	parent[a[i]-1].push_back(i);
	// }

	// vl ans(n,-1);
	// function<ll(int)> rec = [&](int p) {
	// 	if(parent[p].empty()) return ans[p]=0ll;
	// 	if(ans[p]!=-1) return ans[p];
	// 	for(auto&i:parent[p]){
	// 		if(!isCycle[i])chmax(ans[p], (ll)(rec(i)+1));
	// 	}
	// 	return ans[p];
	// };
	// rep(i,n)if(isCycle[i])rec(i);
