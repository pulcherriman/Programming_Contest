...
int main() {
	...
	vl w(n); cin>>w;
	vl cum(n*2+1,0);
	rep(i,n)cum[i+1]=cum[i+n+1]=w[i];
	partial_sum(all(cum),begin(cum));
	debug(cum);
	ll total=sum(w);
	vl mp(n), mpc(n);
	rep(i,n){
		//i番目から始めて重さがx以上になるindexを求める
		ll loopCount=x/total;
		ll val=x-loopCount*total;
		if(val==0){
			mp[i]=(i+n-1)%n;
			mpc[i]=n*loopCount;
			continue;
		}
		debug(i,val, cum[i]+val);
		
		ll id=lower_bound(all(cum),cum[i]+val)-begin(cum);
		debug((id+n-1)%n);
		mp[i]=(id+n-1)%n;
		debug("!!", loopCount, i,mp[i],(n+mp[i]-i)%n+1);
		mpc[i]+=n*loopCount + (n+mp[i]-i)%n+1;
	}
	debug(mp);
	debug(mpc);
	int curIndex=0;
	vl ans; vl memo(n,-1);
	ll loopStart=0,loopEnd=0;
	while(true){
		if(memo[curIndex]!=-1){
			loopStart=memo[curIndex];
			loopEnd=ans.size();
			break;
		}
		debug("!",ans.size(),curIndex,mpc[curIndex],(mp[curIndex]+1)%n );
		memo[curIndex]=ans.size();
		ans.push_back(mpc[curIndex]);
		curIndex = (mp[curIndex]+1)%n;
	}
	debug(ans);
	debug(memo);
	debug(loopStart,loopEnd);
	//i番目から始めると、mp[i]番目まで入れ終わる
	rep(q){
		def(ll,k);
		k--;
		if(k<loopEnd)out(ans[k]);
		else {
			ll id=(k-loopStart)%(loopEnd-loopStart)+loopStart;
			out(ans[id]);
		}
	}
}