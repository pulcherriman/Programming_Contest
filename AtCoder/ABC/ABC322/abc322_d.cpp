...
int main() { /*$1*/
	...
	auto convert = [](vs p){
		int bc=0;
		rep(i,4)rep(j,4) bc+=p[i][j]=='#';
		vector<bitset<16>> items;
		rep(4){
			vs tmp=p;
			rep(i,4)rep(j,4) tmp[i][j]=p[3-j][i];
			p=tmp;
			rep(dy,-3,4)rep(dx,-3,4) {
				tmp=vs(4, string(4, '.'));
				int cnt=0;
				rep(i,4)rep(j,4){
					if(i+dy<0||i+dy>=4||j+dx<0||j+dx>=4) continue;
					tmp[i][j]=p[i+dy][j+dx];
					if(tmp[i][j]=='#') cnt++;
				}
				if(cnt==bc) {
					bitset<16> item;
					rep(i,4)rep(j,4) if(tmp[i][j]=='#') item[i*4+j]=1;
					items.pb(item);
				}
			}
		}
	...
	auto as=convert(a), bs=convert(b), cs=convert(c);
	for(auto&i:as)for(auto&j:bs)for(auto&k:cs) {
		if((i&j).count() or (j&k).count() or (k&i).count()) continue;
		if((i|j|k).count()==16) {
			out("Yes");
			return 0;
		}
