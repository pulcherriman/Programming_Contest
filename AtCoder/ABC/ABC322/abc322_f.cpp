...
namespace SegTreeUtil {
	...
	struct ABC322F {
		struct Val {
			...
			int val0, left0, inner0, right0;
			Val(int v=0) {
				val = v;
				len = 1;
				left = inner = right = v;
				left0 = inner0 = right0 = !v;
			}
			friend Val operator+(const Val&a, const Val&b) {
				Val res;
				res.len = a.len + b.len;
				res.left = a.left;
				if(a.inner == a.len) res.left +=b.left;
				res.right = b.right;
				if(b.inner == b.len) res.right +=a.right;
				res.inner = max({a.inner, b.inner, a.right+b.left});
				res.val = max({a.val, b.val, res.inner, res.left, res.right});

				res.left0 = a.left0;
				if(a.inner0 == a.len) res.left0 +=b.left0;
				res.right0 = b.right0;
				if(b.inner0 == b.len) res.right0 +=a.right0;
				res.inner0 = max({a.inner0, b.inner0, a.right0+b.left0});
				res.val0 = max({a.val0, b.val0, res.inner0, res.left0, res.right0});
				return res;
			}
		};
		using Op = int;
		...
		static Val operate(const Val& l, const Op& r) {
			if(r%2==0) return l;
			Val res=l;
			swap(res.left, res.left0);
			swap(res.right, res.right0);
			swap(res.inner, res.inner0);
			swap(res.val, res.val0);
			return res;
		}
		static Op concat_op(const Op& l, const Op& r) { return l + r; }
	};
...
int main() { /*$1*/
	def(ll,n,q);
	def(string,s);
	SegmentTree<SegTreeUtil::ABC322F> st(n);
	rep(i,n) if(s[i]=='1') st.update(i, 1);
	rep(q){
		def(int,t,l,r);
		if(t==1) st.update(l-1, r,1);
		else out(st.query(l-1, r).val);
		
		// vi a(n);
		// rep(j,n) a[j] = st.query(j, j+1).val;
		// debug(a);
	}
