...
void Main(){
    ...
    while(!q.empty()){
        ...
        // puta(a,b,memo[a][b],ans);
        if(memo[a][b]>ans)break;
        if(a==b){
            chmin(ans,memo[a][b]);
        }
        if(f[a][b]!='-'){
            chmin(ans,memo[a][b]+1);
        }
        for(auto&[to,cost]:g.edge[a]){
            for(auto&[to2,cost2]:g.edge[b])if(cost==cost2){
                if(memo[to][to2]!=-1)continue;
                memo[to][to2]=memo[to2][to]=memo[a][b]+2;
                q.emplace(to,to2);
            }
        }
    }
