...
int main(){
    ...
    range(use,2,n+1){
        //use個使うと決め打つ
        vvl dp(use+1,vl(use,-1));   //dp[i][j]=i個使ってj mod use になるような最大の商
        dp[0][0]=0;
        vvl ndp;
        rep(i,n){
            ndp=dp;
            rep(j,min(i+1,use))rep(k,use)if(dp[j][k]!=-1){
                ll index=(k+a[i])%use;
                chmax(ndp[j+1][index],(dp[j][k]*use+k+a[i])/use);
            }
            dp=ndp;
        }
        if(dp[use][x%use]==-1)continue;
        chmin(ans, (x-(dp[use][x%use]*use+(x%use)))/use);
    }
    puta(ans);
    return 0;
}