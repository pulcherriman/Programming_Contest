...
template<class T, int CHILD_NODE = 20>
struct PersistentArray{
	...
	static Np rewrite(int idx, T val, Np& t){
		if (!t) {
			t = new Node();
		}
		...
		if(idx==0){
			...
			t->ch[idx%CHILD_NODE] = rewrite(idx/CHILD_NODE, val, t->ch[idx%CHILD_NODE]);
		}
		return t;
	}
	static Np set(int idx, T val, const Np& t){
		Np res = new Node();
		if(t){
		...
		if(idx==0){
			...
			res->ch[idx%CHILD_NODE] = set(idx/CHILD_NODE, val, res->ch[idx%CHILD_NODE]);
		}
		return res;
	}
	static T get(int idx, Np t){
		if(!t) return 0;
		if(idx==0){
			return t->data;
		}else{
			return get(idx/CHILD_NODE, t->ch[idx%CHILD_NODE]);
		}
	}
};

// 永続配列のバージョン管理を直感的に行う
template<class T, int CHILD_NODE = 10>
class VersionArray {
	using Pa = PersistentArray<T, CHILD_NODE>;
	using Np = typename Pa::Np;
	struct Node {
		Np np;
		int version = 0;
		Node* parent;
		Node(Np np) : np(np), parent(nullptr) {}

		Node* set(int idx, T val) {
			auto ch = new Node(Pa::set(idx, val, np));
			return ch;
		}

		void rewrite(int idx, T val) {
			Pa::rewrite(idx, val, np);
		}
	};
	...
	VersionArray(): versions(1, new Node(Pa().getRoot())){
		current = versions.front();
	}

	VersionArray(int size, T val = T()): VersionArray(){
		apply(0, val);
		rep(i,1,size) {
			current->rewrite(i, val);
		}
	}
	...
	int rewrite(int idx, T val) {
		current->rewrite(idx, val);
		return current->version;
	}

	int apply(int idx, T val) {
		auto child = current->set(idx, val);
		child->version = versions.size();
		child->parent = current;
		versions.push_back(child);
		current = child;
		return current->version;
	}

	T get(int idx) {
		return Pa::get(idx, current->np);
	}

	int undo() {
		if(current->parent == nullptr) return -1;
...
struct RollbackUnionFind{
	int n;
	VersionArray<int> par, edges;
	VersionArray<int> ans; VersionArray<int, 1> ansSum;
	RollbackUnionFind(int x): n(x), par(x, -1), edges(x, 0), ans(x, 0), ansSum(1, 0) {}
	int find(int x){return par[x]<0?x:find(par[x]);}
	bool same(int x,int y){return find(x)==find(y);}
	int size(int x){return -par[find(x)];}
	int edgeCount(int x){return edges[find(x)];}
	bool unite(int x, int y, T k=T()){
		x=find(x); y=find(y);
		if (x==y) {
			par.apply(x, par[x]);
			par.rewrite(y, par[y]);
			edges.apply(x, edges[x]+1);
			ansSum.apply(0, ansSum[0] + min(size(x), edgeCount(x)) - ans[x]);
			ans.apply(x, min(size(x), edgeCount(x)));
			return false;
		}
		if(size(x)<size(y)){swap(x,y);}
		par.apply(x, par[x]+par[y]);
		par.rewrite(y, x);
		edges.apply(x, edges[x]+edges[y]+1);
		ansSum.apply(0, ansSum[0] + min(size(x), edgeCount(x)) - ans[x] - ans[y]);
		ans.apply(x, min(size(x), edgeCount(x)));
		return true;
	}
	...
	void rollback() {
		par.undo(); edges.undo(); ans.undo(); ansSum.undo();
	}
};
