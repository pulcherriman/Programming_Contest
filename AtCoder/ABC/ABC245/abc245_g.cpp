...
int main() {
	...
	vl b(l); cin>>b;
	Graph<ll> g(n+1);
	g.inputEdge(m,true,false);


	vl ans(n, LINF);
	vb isTgtCountry(k, true);
	rep(i,k/2)isTgtCountry[i] = false;
	while(Mgr::timer.get()<=1800){
		shuffle(all(isTgtCountry), Random);

		g.edge[n].clear();
		for(auto&d:b)if(isTgtCountry[a[d-1]-1])g.add(n,d-1,0,true);

		Dijkstra dijk(g);
		vl dist=dijk.dist(n);
		rep(i,n){
			if(!isTgtCountry[a[i]-1])chmin(ans[i], dist[i]);
		}
	}
