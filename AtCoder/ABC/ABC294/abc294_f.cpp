...
int main() {
	...
	ld ok=0, ng=1.1;//濃度がmid以上になるものがk個以上あるか
	while(ng-ok>numeric_limits<ld>::epsilon()*10){
		ld mid=(ok+ng)/2;

		vd pa(n), pb(m);
		rep(i,n) pa[i]=-(a[i].first - mid*(a[i].first+a[i].second));
		rep(i,m) pb[i]=b[i].first - mid*(b[i].first+b[i].second);
		sort(all(pb));

		ll cnt=0;
		rep(i,n){
			cnt += pb.end() - lower_bound(all(pb), pa[i]);
		}
