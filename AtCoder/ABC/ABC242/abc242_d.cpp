...
// level分進めたindex文字目
char calc(string&s, ll level, ll index){
	...
	ll beforeIndex = index/2;
	ll afterIndex = index%2;
	auto c=calc(s, level-1, index/2);
	if(c=='A'){
		return "BC"[afterIndex];
	}else if(c=='B'){
		return "CA"[afterIndex];
	}else{
		return "AB"[afterIndex];
	}
