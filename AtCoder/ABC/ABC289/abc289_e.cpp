...
ll solve(){
	def(ll,n,m);
	vl color(n); cin>>color;
	Graph<> g(n);
	g.inputEdge(m,false,false);
	
	vvl visited(n, vl(n, -1));
	visited[0][n-1] = 0;
	queue<pii> q;
	q.push({0, n-1});
	while(!q.empty()){
		...
		ll cost = visited[l][r];
		debug(l,r, cost);
		for(auto [to1, _] : g.edge[l]){
			for(auto [to2, _] : g.edge[r]){
				if(color[to1] == color[to2]) continue;
				if(visited[to1][to2]!=-1) continue;
				visited[to1][to2] = cost+1;
				q.push({to1, to2});
				debug("  ->", to1, to2, cost+1);
				if(to1 == n-1 && to2 == 0) return visited[n-1][0];
			}
		}
	}
	return -1;
