...
int main() {
	...
	def(ll, n);
	vl a(n); cin>>a;
	SegmentTree<SegTreeUtil::Add_Sum<ll>> st_cnt(200001);
	SegmentTree<SegTreeUtil::Add_Sum<ll>> st_val(200001);
	mymint prev=0;

	rep(i,n){
		mymint ans = prev * i*i ;
		//自分より小さい要素の個数
		mymint cnt = st_cnt.query(0,a[i]+1).val;
		//自分より大きい要素の和
		mymint val = st_val.query(a[i]+1,200001).val;
