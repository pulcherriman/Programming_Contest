...
int main() {
	...
	vm patterns(n,0);
	vm smalls(n,0), bigs(n,0);
	rep(i,n){
		if(s[i]>='A' and s[i]<='Z'){
			ll pat=capCnt[s[i]-'A']+anyCnt;
			patterns[i]=pat;
			capCnt[s[i]-'A']++;
			bigs[i]+=1;
		}else if(s[i]=='?'){
			ll pat=sum(capCnt) + anyCnt*26;
			patterns[i]=pat;
			anyCnt++;
			smalls[i]+=26;
			bigs[i]+=26;
	...
	// rrep(i,n-1)smalls[i]+=smalls[i+1];
	// rrep(i,n-1)bigs[i]+=bigs[i+1];

	debug(patterns);
	debug(smalls);
	debug(bigs);
	vm smalls2(n,0); mymint bigSum = 0;
	rrep(i,n){
		smalls2[i] = smalls[i] * bigSum;
		debug("?",i,smalls[i], bigSum);
		bigSum += bigs[i];
	}
	debug(smalls2);
