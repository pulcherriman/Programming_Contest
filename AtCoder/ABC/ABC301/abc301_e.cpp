...
int main() {
	...
	rep(i,h)rep(j,w){
		if(f[i][j]=='#')continue;
		if(i!=h-1 and f[i+1][j]!='#'){
			g.add(i*w+j, (i+1)*w+j, 1);
		}
		if(j!=w-1 and f[i][j+1]!='#'){
			g.add(i*w+j, i*w+j+1,1);
		}
		if(f[i][j]=='S')s=i*w+j;
		if(f[i][j]=='G')t=i*w+j;
		if(f[i][j]=='o'){
	...
	auto[_,war]=WarshallFloyd(g2).dist();
	if(war[0][1]>n){
		out(-1);
		return 0;
	}
	...
	vvl dp(oc, vl(1<<oc, LINF));
	dp[0][1]=0;
	rep(ss, 1<<oc){
		rep(p,oc)if((1<<p)&ss)if(dp[p][ss]!=LINF){
			rep(p2,oc)if(!((1<<p2)&ss)){
				chmin(dp[p2][ss|(1<<p2)], dp[p][ss]+war[p][p2]);
			}
		}
	}
	ll ans=0;
	rep(ss,1<<oc)if(dp[1][ss]<=n)chmax(ans, __builtin_popcount(ss)-2LL);
	out(ans);
