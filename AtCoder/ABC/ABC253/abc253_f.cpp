...
int main() {
	...
	SegmentTree<SegTreeUtil::Add_Sum<ll>> st(m);
	vl replaced(n,-1);
	reverse(all(query));
	rep(i,n)query.emplace_back(2,i,0,0);
	vector<map<ll,ll>> question_diff(n);

	vl answer(query.size(), -1);
	rep(ind, query.size()){
		auto&[t,v1,v2,v3] = query[ind];
		if(t==1){
			ll l=v1,r=v2,x=v3;
			st.update(l,r+1,x);
		}else if(t==2){
			ll h=v1,x=v2;
			replaced[h]=x;
			for(auto&[i,diff]:question_diff[h]){
				ll w=get<2>(query[i]);
				ll value = st.query(w,w+1).val;
				ll ans = x + value - diff;
				debug("Q",i,"=", x,"+",value,"-",diff,"=",ans);
				answer[i]=ans;
			}
			question_diff[h].clear();

		}else{
			ll h=v1,w=v2;
			question_diff[h][ind]=st.query(w,w+1).val;
			debug("set diff of Q",ind,":",st.query(w,w+1).val);
		}
	}
	reverse(all(answer));
	debug(answer);
	for(auto&p:answer)if(p!=-1)out(p);
