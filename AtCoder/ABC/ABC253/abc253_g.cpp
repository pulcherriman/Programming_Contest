...
int main() {
	...
	if(st.first==ed.first){
		rep(i,st.second, ed.second+1) swap(a[st.first],a[i]);
		out(a);
		return 0;
	}
	rep(i,st.second, n) swap(a[st.first],a[i]);
	ll start_index = st.first+1;
	ll len = ed.first-st.first-1;
	vl b(n,0);
	rep(i,start_index)b[i]=a[i];
	rep(i,len){
		ll j=start_index+i;
		b[j] = a[n-1-i];
	}
	rep(i,n-start_index-len){
		ll j=start_index+len+i;
		b[j] = a[start_index+i];
	}
	a=b;
	debug(start_index, len);
	rep(i,ed.first+1, ed.second+1) swap(a[ed.first],a[i]);
