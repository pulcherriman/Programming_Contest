...
void Main(){
	...
	h-=2;w-=2;
	vvl ans=vec(-1ll,h+2,w+2);
	rep(i,w+2)ans[0][i]=ans[h+1][i]=-9;
	rep(i,h+2)ans[i][0]=ans[i][w+1]=-9;

	while(!q.empty()){
		auto[y,x,c]=q.front(); q.pop_front();
		if(ans[y][x]!=-1)continue;
		ans[y][x]=c;
		for(auto[ny,nx,_]:g.neighborIndex(y,x,0)){
			if(ans[ny][nx]!=-1)continue;
			q.emplace_front(ny,nx,c);
		}
		for(auto[ny,nx,_]:g.neighborIndex(y,x,-1)){
			if(ans[ny][nx]!=-1)continue;
			q.emplace_back(ny-1,nx-1,c+1);
			q.emplace_back(ny,nx-1,c+1);
			q.emplace_back(ny+1,nx-1,c+1);
			q.emplace_back(ny-1,nx,c+1);
