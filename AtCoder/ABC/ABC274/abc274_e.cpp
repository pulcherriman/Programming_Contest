...
int main() {
	...
	vvd dp(n+m, vd(1<<(n+m), LINF));
	rep(i,n+m) dp[i][1<<i] = hypot(a[i].first, a[i].second);

	ld ans=LINF;
	rep(s,1<<(n+m)){
		int boost = __builtin_popcount(s&(((1<<m)-1)<<n));
		rep(i,n+m)if(s>>i&1){
			rep(j,n+m)if(!(s>>j&1)){
				// i to j
				ld cost = hypot(a[i].first-a[j].first, a[i].second-a[j].second);
				cost /= (1<<boost);
				chmin(dp[j][s|1<<j], dp[i][s] + cost);
			}

			if((s&((1<<n)-1)) == (1<<n)-1){
				ld cost = hypot(a[i].first, a[i].second);
				cost /= (1<<boost);
				chmin(ans, dp[i][s] + cost);
			}
		}
