...
int main() {
	...
	rep(i,n){
		debug(i);
		// 魚iを基準にする
		vector<tuple<ld, ld, int>> tm;
		vector<ld> st;
		rep(j,n)if(i!=j){
			ld s,t;
			if(v[i]!=v[j]){
				// sはx[i]+v[i]*t=x[j]+v[j]*tの解
				s = (x[j]-x[i])/(v[i]-v[j]);
				// tはx[i]+v[i]*t+a=x[j]+v[j]*tの解
				t = (x[j]-x[i]-a)/(v[i]-v[j]);
				if(x[j]<x[i]){
					if(s<0)continue;
				}else if(x[j]<=x[i]+a){
		...
		imos[0]=w[i];
		for(auto&[vs,vt,j]:tm){
			ll s=cp.zip(vs), t=cp.zip(vt);
			debug(i,j,"=>",vs,vt,s,t);
			imos[s]+=w[j];
			imos[t+1]-=w[j];
		}
		rep(j,imos.size()-1)imos[j+1]+=imos[j];
		debug(imos);
