...
void Main(){
	...
	vvl d(20,vl(m,0));
	rep(i,m){
		auto it=edge[re[i][1]].lower_bound(re[i][3]);
		if(it==edge[re[i][1]].end()){
			d[0][i]=i;
		}else d[0][i]=it->second[2];
	}
	...
	rep(q){
		geta(ll,x,y,z);
		auto it=edge[y-1].lower_bound(x);
		if(it==edge[y-1].end()){
			// 乗れる電車がない
			puta(y);
			continue;
		...
		int renum=it->second[2];
		if(z<=re[renum][3]){
			puta(re[renum][0]+1, re[renum][1]+1);
			continue;
		}
		while(true){
			bool rode=false;
			rrep(i,20){
				int nxtnum=d[i][renum];
				if(re[nxtnum][3]<z){
					if(renum!=nxtnum){
						renum=nxtnum;
						rode=true;
