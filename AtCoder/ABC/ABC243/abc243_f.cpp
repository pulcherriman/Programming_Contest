...
int main(){
	...
	vm prod(n);
	cin>>prod;
	mymint prod_sum=sum(prod);
	rep(i,n){
		prod[i] = prod[i] / prod_sum;
	}

	// i番目まででちょうどj種類、合計K個当てる確率
	vector<vvm> dp(n+1, vvm(m+1, vm(K+1, 0_m)));
	dp[0][0][0]=1;
	rep(i,1,K+1)dp[0][0][0]*=i;
	rep(i,n){
		rep(j,m+1){
			rep(k,K+1){
				//i番目を使わない
				...
				mymint factorial = 1;
				rep(l,1,K-k+1){	//i番目をl個当てることにする
					factorial *= l;
					if(j+1<=m and k+l<=K) dp[i+1][j+1][k+l] += dp[i][j][k] * prod[i].pow(l) / factorial;
				}
			}
		}
	}
