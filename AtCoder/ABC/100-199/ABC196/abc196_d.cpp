...
map<tuple<vvi,ll,ll>,ll> memo;
ll solve(vvi f, ll a, ll b){
    if(a+b==0) return 1;
    auto tp=make_tuple(f,a,b);
    if(memo.find(tp)!=memo.end())return memo[tp];
    dump(a,b);
    // puta("call:");
    // puta(f);
    // puta(a,b);
    bool ok=false;
    ll ans=0;
    rep(i,f.size())rep(j,f[0].size())if(f[i][j]==0){
        f[i][j]=1;
        if(b>0){
            ans+=solve(f,a,b-1);
            ok=true;
        ...
        if(a>0){
            if(j+1<f[0].size() && f[i][j+1]==0){
                f[i][j+1]=1;
                ans+=solve(f,a-1,b);
                f[i][j+1]=0;
                ok=true;
            }
            if(i+1<f.size() && f[i+1][j]==0){
                f[i+1][j]=1;
                ans+=solve(f,a-1,b);
                f[i+1][j]=0;
                ok=true;
