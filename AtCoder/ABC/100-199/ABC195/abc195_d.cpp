...
int main(){
    ...
    sort(all(item),[](pll a, pll b){return a.second>b.second or (a.second==b.second and a.first>b.first);});
    vl x(m);
    cin>>x;
    rep(_,q){
        auto copyItem=item;
        ...
        geta(ll,l,r);
        l--,r--;
        rep(i,m)if(i<l or i>r)cap.push_back(x[i]);
        sort(all(cap));
        // puta(cap);
        ll ans=0;
        for(auto&c:cap){
            for(auto&[w,v]:copyItem){
                if(c>=w){
                    ans+=v;
                    w=LINF;
                    break;
