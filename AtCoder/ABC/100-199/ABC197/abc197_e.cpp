...
void Main(){
    ...
    rep(i,n){
        cin>>p[i]>>c[i];
        if(mp.find(c[i])==mp.end())mp[c[i]]=vl();
        mp[c[i]].emplace_back(p[i]);
    }
    ...
    dp[0]=0;
    for(auto&[col,pos]:mp){
        ll l=min(pos), r=max(pos);
        map<int,ll> dp2;
        dp2[l]=LINF;
        dp2[r]=LINF;
        for(auto&[cur,cost]:dp){
            chmin(dp2[l], cost+abs(cur-r)+r-l);
            chmin(dp2[r], cost+abs(cur-l)+r-l);
        }
        dp=dp2;
    }
    ll ans=LINF;
    for(auto&[cur,cost]:dp){
        chmin(ans, cost+abs(cur));
    }
