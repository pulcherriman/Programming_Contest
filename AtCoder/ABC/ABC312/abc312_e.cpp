...
int main() {
	/*$1*/
	def(ll,n);
	vector<vvl> a(103, vvl(103, vl(103, 0)));
	rep(i,n){
		def(ll,x1,y1,z1,x2,y2,z2);
		x1++; y1++; z1++; x2++; y2++; z2++;
		rep(x,x1,x2) rep(y,y1,y2) rep(z,z1,z2) a[x][y][z] = i+1;
	}

	vector<set<ll>> ans(n);
	rep(i,103) rep(j,103) rep(k,103) if(a[i][j][k]) {
		//隣接6方向
		rep(l,6){
			ll nx = i+dx[l], ny = j+dy[l], nz = k+dz[l];
			if(a[nx][ny][nz] && a[nx][ny][nz] != a[i][j][k]){
				ans[a[i][j][k]-1].insert(a[nx][ny][nz]);
			}
		}
	}
	rep(i,n){
