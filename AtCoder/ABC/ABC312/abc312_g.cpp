...
// dfs
// child[i] = 頂点iの、自身を含める子の数
// pairs[i] = 頂点iを根とする部分木で、自身を含めて親子関係にある頂点の組の数
vl child, pairs, parent;
ll ans=0;
void dfs(int p, int from, Tree<>& g){
	for(auto&[to,_]:g.edge[p])if(to!=from){
		dfs(to,p,g);
		ans += child[to] * child[p];
		child[p]+=child[to];
		pairs[p]+=pairs[to]+child[to];
	}
	for(auto&[to,_]:g.edge[p])if(to!=from){
		ans += pairs[to] * (child[p]-child[to]+1);
	}
