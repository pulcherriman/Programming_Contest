...
struct Trie {
	...
	int query(const string &word) {
        ...
        for (int i = 0; i < (int)word.size(); i++) {
            int c = (int)(word[i] - base);
			if(nodes[node_id].common > 1) chmax(answer,i);
            int &next_id = nodes[node_id].next[c];
            if (next_id == -1) {  // 次の頂点が存在しなければ終了
                break;//
            }
		...
		debug(nodes[node_id].c, nodes[node_id].common);
		if(nodes[node_id].common > 1) return word.size();
		// if(node_id!=-1) return word.size();
		return answer;
    }
};
