...
int main() {
	...
	rep(q){
		...
		if((x2-x1)%2==0){
			...
			// 奇数が多い列
			{
				mymint cnt=(y2-y1+2)/2;
				mymint width = (x2-x1+2)/2;
				mymint mn = y1*w+x1+1;
				mymint mx = ((y2-y1)%2==1 ? y2-1 : y2)*w+x1+1;

				debug(width, cnt, mn, mx);
				ans+=width * (mn + mx) * cnt / 2;
				// 横の増分
				mymint d=width*(width-1);
				ans+=d*cnt;
			}
			// 奇数が少ない列
			if(y2-y1>0){
				mymint cnt=(y2-y1+1)/2;
				mymint width = (x2-x1+1)/2;
				mymint mn = (y1+1)*w+x1+2;
				mymint mx = ((y2-y1)%2==1 ? y2 : y2-1)*w+x1+2;
		...
		if((y1+x1)%2==0){
			...
			{
				mymint cnt=(y2-y1+1);
				mymint width = (x2-x1+1);
				mymint mn = y1*w+x1+1;
				mymint mx = y2*w+x1+1;
				total+=width * (mn + mx) * cnt / 2;
				// 横の増分
				mymint d=width*(width-1)/2;
				total+=d*cnt;
			}
			out(total-ans);
		}
