...
int main() {
	...
	do {
		bool ok=true;
		vvi b(3, vi(3,0));
		rep(i,9) b[p[i]/3][p[i]%3]=-i-1;
		rep(i,9){
			b[p[i]/3][p[i]%3]=a[p[i]/3][p[i]%3];
			if(b[0][0]<0 and b[0][1]==b[0][2]) ok=false;
			if(b[0][1]<0 and b[0][2]==b[0][0]) ok=false;
			if(b[0][2]<0 and b[0][0]==b[0][1]) ok=false;
			if(b[1][0]<0 and b[1][1]==b[1][2]) ok=false;
			if(b[1][1]<0 and b[1][2]==b[1][0]) ok=false;
			if(b[1][2]<0 and b[1][0]==b[1][1]) ok=false;
			if(b[2][0]<0 and b[2][1]==b[2][2]) ok=false;
			if(b[2][1]<0 and b[2][2]==b[2][0]) ok=false;
			if(b[2][2]<0 and b[2][0]==b[2][1]) ok=false;
			if(b[0][0]<0 and b[1][0]==b[2][0]) ok=false;
			if(b[1][0]<0 and b[2][0]==b[0][0]) ok=false;
			if(b[2][0]<0 and b[0][0]==b[1][0]) ok=false;
			if(b[0][1]<0 and b[1][1]==b[2][1]) ok=false;
			if(b[1][1]<0 and b[2][1]==b[0][1]) ok=false;
			if(b[2][1]<0 and b[0][1]==b[1][1]) ok=false;
			if(b[0][2]<0 and b[1][2]==b[2][2]) ok=false;
			if(b[1][2]<0 and b[2][2]==b[0][2]) ok=false;
			if(b[2][2]<0 and b[0][2]==b[1][2]) ok=false;
			if(b[0][0]<0 and b[1][1]==b[2][2]) ok=false;
			if(b[1][1]<0 and b[2][2]==b[0][0]) ok=false;
			if(b[2][2]<0 and b[0][0]==b[1][1]) ok=false;
			if(b[0][2]<0 and b[1][1]==b[2][0]) ok=false;
			if(b[1][1]<0 and b[2][0]==b[0][2]) ok=false;
			if(b[2][0]<0 and b[0][2]==b[1][1]) ok=false;
		}
		if(ok) ans++;
