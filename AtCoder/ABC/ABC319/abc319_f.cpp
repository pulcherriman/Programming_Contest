...
int main() {
	...
	rep(i,1,n){
		...
		if(t==1){
			...
			vs[i]=v;
			meds.pb(i);
			medMap[i]=meds.size()-1;
		}
	}
	// bitDPしながらやる N=500なので解放された部分木ごとにシミュレーションはさんでも行ける
	vector<tuple<int,minpq<pll>,vl>> dp(1<<med,{0, minpq<pll>(), vl()});
	{
		minpq<pll> pq;
		vl adjMed;
		queue<int> q;
		...
		while(!q.empty()){
			...
			for(auto&[to,cost]:g.edge[s]){
				if(medMap.count(to)==1){
					adjMed.pb(medMap[to]);
					continue;
				}
				q.emplace(to);
	...
	rep(i,1<<med){
		auto&[cnt, pq, adjMed]=dp[i];

		for(auto j:meds)if(!(i>>j&1)){
			ll cpw=cnt;
			auto cpq=pq;
			auto cadjMed=adjMed;
			cadjMed.erase(find(all(cadjMed),j));
			cpw*=vs[meds[j]];
			for(auto k:cadjMed){
				cpw+=vs[meds[k]];
				for(auto&[v,to]:g.edge[meds[k]]){
					if(medMap.count(to)==1){
						cadjMed.pb(medMap[to]);
						continue;
					}
					cpq.emplace(ss[to],to);
				}
			}
			while(!cpq.empty()){
				auto [v,i]=cpq.top();
			...
			dp[i|(1<<j)]={cpw, cpq, cadjMed};
		}
	}
	out(get<0>(dp[(1<<med)-1]));
