...
int main() {
	/**/
	def(ll, n,sy,sx,gy,gx);
	sy--; sx--; gy--; gx--;
	vs s(n); cin>>s;
	vector<vvb> visit(n, vvb(n, vb(4, false)));

	deque<tuple<int,int,int,int>> q;
	q.emplace_back(sy,sx,0,1);
	q.emplace_back(sy,sx,1,1);
	q.emplace_back(sy,sx,2,1);
	q.emplace_back(sy,sx,3,1);
	vi dy{1,1,-1,-1};
	vi dx{1,-1,-1,1};
	while(!q.empty()) {
		auto [y,x,d,c] = q.front(); q.pop_front();
		if(visit[y][x][d]) continue;
		if(y==gy&&x==gx) {
			out(c);
			return 0;
		}
		visit[y][x][d] = true;
		rep(nd,4){
			auto ny=y+dy[nd], nx=x+dx[nd];
			if(ny<0||nx<0||ny>=n||nx>=n) continue;
			if(s[ny][nx]=='#') continue;
			if(visit[ny][nx][nd]) continue;
			if(d==nd)q.emplace_front(ny,nx,nd,c);
			else q.emplace_back(ny,nx,nd,c+1);
		}
	}
	out(-1);
}