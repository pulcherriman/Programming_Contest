...
void Main(){
	...
	ll ans=0;
	int d=as.size();
	rep(i,d)sort(all(m[as[i]]));
	rep(i,d)if(m[as[i]].size()>1)rep(j,i+1,d)if(m[as[j]].size()>1){//1000^2
		vl c;
		set_intersection(all(m[as[i]]),all(m[as[j]]),back_inserter(c));
		ans += c.size() * (c.size()-1) / 2;
		// cout<<as[i]<<" "<<as[j]<<" "<<c.size()<<endl;
	}
	puta(ans);
}
