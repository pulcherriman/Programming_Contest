...
int main() {
	...
	vl index(n,0);
	iota(all(index),0);
	sort(all(index),[&](int i,int j){return a[i]>a[j] or (a[i]==a[j] and i<j);});
	debug(index);
	rep(i,n){
		if(suc[index[i]])continue;
		if(x==0)break;
		x--;
		suc[index[i]]=true;
	}
	
	sort(all(index),[&](int i,int j){return b[i]>b[j] or (b[i]==b[j] and i<j);});
	debug(index);
	rep(i,n){
		if(suc[index[i]])continue;
		if(y==0)break;
	...
	sort(all(index),[&](int i,int j){return a[i]+b[i]>a[j]+b[j] or (a[i]+b[i]==a[j]+b[j] and i<j);});
	debug(index);
	rep(i,n){
		if(suc[index[i]])continue;
		if(z==0)break;
