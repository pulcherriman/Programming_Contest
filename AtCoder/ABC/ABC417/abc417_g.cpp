...
public:
	...
	SegmentTree(const vector<Val>& data_)
		: h(ceil(log2(data_.size()))), n(1<<h), data(n*2, Monoids::id_val()), lazy(n*2, Monoids::id_op()) {
			rep(i, data_.size()) data[i+n] = data_[i];
			rrep(i, 1, n) data[i] = Monoids::concat_val(data[i*2], data[i*2+1]);
	}
...
const ll MAX_LEN = 1e18;

struct Str {
	int value = 0;
	ll size=1;
	Str *left, *right;
	vector<Str*> leftDoubling, rightDoubling;

	Str(int v) : value(v), left(nullptr), right(nullptr) {}
	Str(Str *l, Str *r) : left(l), right(r) {
		if(l->size >= MAX_LEN) {
			size = MAX_LEN;
		} else {
			size = l->size + min(r->size, MAX_LEN);
		}

		leftDoubling.push_back(l);
		rep(i,1000) {
			auto last = leftDoubling.back();
			if(last->leftDoubling.size() > i) {
				leftDoubling.push_back(last->leftDoubling[i]);
			} else break;
		}
		rightDoubling.push_back(r);
		rep(i,1000) {
			auto last = rightDoubling.back();
			if(last->rightDoubling.size() > i) {
				rightDoubling.push_back(last->rightDoubling[i]);
			} else break;
		}
	}
	Str* getIndex(ll i) {
		...
		Str *lTo=nullptr, *rTo=nullptr;
		rrep(ld, leftDoubling.size()) {
			if(i < leftDoubling[ld]->size) {
				lTo=leftDoubling[ld];
				break;
			}
		}
		rrep(rd, rightDoubling.size()) {
			if(i >= size - rightDoubling[rd]->size) {
				rTo=rightDoubling[rd];
				break;
			}
		}

		if(lTo && rTo) {
			if(lTo->size <= rTo->size) {
				return lTo->getIndex(i);
			} else {
				return rTo->getIndex(rTo->size - (size - i));
			}
		}else if(lTo) {
			return lTo->getIndex(i);
		} else if(rTo) {
			...
			if(i < left->size) {
				return left->getIndex(i);
			} else {
				return right->getIndex(i - left->size);
			}
		}
	}
};
...
int main() { /**/
	...
	vector<Str*> strs;
	strs.push_back(new Str(0));
	strs.push_back(new Str(1));

	rep(n){
		def(ll, l,r,x);
		if(strs[l]->size == MAX_LEN) {
			strs.push_back(strs[l]);
		}else{
			Str *s = new Str(strs[l], strs[r]);
			strs.push_back(s);
		}
		out(strs.back()->getIndex(x-1)->value);
	}
}