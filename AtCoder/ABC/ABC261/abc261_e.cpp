...
int main() {
	...
	def(ll,n,x);
	vp a(n); cin>>a;
	vl status(30,2);//0 0にする, 1 1にする, 2 キープ, 3 反転
	vl current(30,0);
	rep(i,30)current[i]=x>>i&1;

	rep(i,n){
		if(a[i].first == 1){
			rep(j,30)if(((a[i].second>>j)&1)==0){
				status[j] = 0;
			}
		}else if(a[i].first == 2){
			rep(j,30)if(((a[i].second>>j)&1)==1){
				status[j] = 1;
			}
		}else {
			rep(j,30)if(((a[i].second>>j)&1)==1){
				if(status[j]==0)status[j]=1;
				else if(status[j]==1)status[j]=0;
				else if(status[j]==2)status[j]=3;
				else status[j]=2;
			}
		}
		ll ans=0;
		rep(j,30){
			if(status[j]==0)current[j]=0;
			else if(status[j]==1)current[j]=1;
			else if(status[j]==2)current[j]=current[j];
