...
int main() {
	...
	rep(4){
		while(ah>0 and count(all(a.back()), '#')==0) {
			a.pop_back();
			ah--;
		}
		//rotate clockwise
		vs a2(aw, string(ah, '.'));
		rep(i,ah)rep(j,aw) a2[j][ah-i-1] = a[i][j];
		a = a2;
		swap(ah, aw);
	}

	rep(4){
		while(bh>0 and count(all(b.back()), '#')==0) {
			b.pop_back();
			bh--;
		}
		//rotate clockwise
		vs b2(bw, string(bh, '.'));
		rep(i,bh)rep(j,bw) b2[j][bh-i-1] = b[i][j];
		b = b2;
		swap(bh, bw);
	}

	rep(4){
		while(ch>0 and count(all(c.back()), '#')==0) {
			c.pop_back();
			ch--;
		}
		//rotate clockwise
		vs c2(cw, string(ch, '.'));
		rep(i,ch)rep(j,cw) c2[j][ch-i-1] = c[i][j];
		c = c2;
		swap(ch, cw);
	}

	bool ok = false;
	rep(i,ch-ah+1)rep(j,cw-aw+1){
		rep(k,ch-bh+1)rep(l,cw-bw+1){
			vs s(ch, string(cw, '.'));
			rep(y,ah)rep(x,aw) if(a[y][x]=='#') s[y+i][x+j] = '#';
			rep(y,bh)rep(x,bw) if(b[y][x]=='#') s[y+k][x+l] = '#';
			ok |= s==c;
		}
	}
	Yn(ok);
