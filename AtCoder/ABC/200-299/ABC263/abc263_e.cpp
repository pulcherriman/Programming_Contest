...
public:
	...
	SegmentTree(const vector<Val>& data_)
		: h(ceil(log2(data_.size()))), n(1 << h), input_n(data_.size()), data(n * 2, Monoids::id_val()), lazy(n * 2, Monoids::id_op()) {
...
int main() {
	...
	def(ll,n);
	vm a(n-1); cin>>a;
	SegmentTree<SegTreeUtil::Update_Sum<mymint>> dp(n);

	rrep(i,n-1){
		auto c=(a[i]+1)/a[i];
		auto q = dp.query(i+1, i+a[i].x+1).val;
		dp.update(i, i+1, c + q/a[i]);
		out(dp);
	}
	out(dp[0].val);
}