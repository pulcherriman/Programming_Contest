...
int main() {
	/**/
	def(ll,n,m);
	// i番目まで決めて、末尾要素が最小のLISを構成する数字がそれぞれj1,j2,j3である
	auto dp=vec(0_m, n+1, m+2, m+2, m+2);
	vector<vector<vvm>> dp(n+1, vector<vvm>(m+2, vvm(m+2, vm(m+2, 0))));
	dp[0][m+1][m+1][m+1] = 1; // m+1 未確定

	rep(i,n)rep(j1,1,m+2)rep(j2,j1,m+2)rep(j3,j2,m+2){
		rep(k,1,m+1){
			if(k <= j1) dp[i+1][k][j2][j3] += dp[i][j1][j2][j3];
			if(j1 < k && k <= j2) dp[i+1][j1][k][j3] += dp[i][j1][j2][j3];
			if(j2 < k && k <= j3) dp[i+1][j1][j2][k] += dp[i][j1][j2][j3];
		}
	}

	mymint ans = 0;
	rep(j1,1,m+1)rep(j2,j1,m+1)rep(j3,j2,m+1)if(dp[n][j1][j2][j3]){
		debug(j1,j2,j3,"=",dp[n][j1][j2][j3]);
		ans += dp[n][j1][j2][j3];
	}
	out(ans);
}