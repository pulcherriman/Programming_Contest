...
int main() {
	/*$1*/
	auto[n,g]=Tree<>::input();
	auto[_, path] = g.diameter();

	int p1=path[0], p2=path.back();
	...
	vi nearDist(n,0);
	{
		...
		for(auto&p:path){
			...
			while(!q.empty()){
				int i=q.front();q.pop();
				for(auto&[j,__]:g.edge[i]){
					if(nearby[j]==-1){
						nearby[j]=p;
						nearDist[j]=nearDist[i]+1;
						q.push(j);
					}
				}
			}
	...
	// LCA lca(g);
	// auto euler = g.dfs(0).eulertour;
	// // vvi 
	// vector<pii> eulerMap;
	// for(auto&i:euler){
	// 	eulerMap.emplace_back(lca.dist[i], i);
	// }
	// vi l(n,-1),r(n,-1);
	// rep(i,euler.size()){
	// 	if(l[euler[i]]==-1) l[euler[i]]=i;
	// 	else r[euler[i]]=i;
	// }
	// SegmentTree<SegTreeUtil::Update_Max<pii>> st(eulerMap);
	LCA lca1(g, p1);
	LCA lca2(g, p2);
	...
	rep(q){
		...
		if(d1[u] >= v){
			if(nearDist[u] > v){
				int tgtDist = lca1.dist[u] - v;
				debug(tgtDist);
				debug(lca1.parent);
				rrep(d,lca1.parent.size()){
					if(lca1.parent[d][u]!=-1 && lca1.dist[lca1.parent[d][u]]>=tgtDist){
						debug_f("->",u);
						u=lca1.parent[d][u];
					}
				}
			...
			int remain=v-nearDist[u];
			int req=d1[nearby[u]]-remain;
			//uからpath上でreq離れたところ
			out(path[req]+1);
		}else{
			if(nearDist[u] > v){
				int tgtDist = lca2.dist[u] - v;
				rrep(d,lca2.parent.size()){
					if(lca2.parent[d][u]!=-1 && lca2.dist[lca2.parent[d][u]]>=tgtDist){
						u=lca2.parent[d][u];
					}
				}
				out(u+1);
			...
			int remain=v-nearDist[u];
			int req=d2[nearby[u]]-remain;
			//uからpath上でreq離れたところ
			out(path[path.size()-1-req]+1);
		}
	}
}