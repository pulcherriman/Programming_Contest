...
int main() {
	/*$1*/
	def(ll,n,p);
	// 1->2, 2->2, 3~9->2, 10~99->3, 100~999->4, 1000~3000->5
	// 圧縮後長さiの文字列で、元々j文字だったもの
	vvl dp(n+1, vl(n+1, 0));
	vvl dp2(n+1, vl(n+1, 0));
	dp[0][0]=1;
	dp2[0]=vl(n+1, 1);
	...
	rep(i,1,n+1){
		rep(j,n+1){
			if(i-2>=0 and j-1>=0){
				ll v=dp2[i-2][j-1];
				if(j-10>=0)v=v+p-dp2[i-2][j-10];
				dp[i][j]+=v*(i-2==0 ? 26 : 25)%p;
			}
			if(i-3>=0 and j-10>=0){
				ll v=dp2[i-3][j-10];
				if(j-100>=0)v=v+p-dp2[i-3][j-100];
				dp[i][j]+=v*(i-3==0 ? 26 : 25)%p;
			}
			if(i-4>=0 and j-100>=0){
				ll v=dp2[i-4][j-100];
				if(j-1000>=0)v=v+p-dp2[i-4][j-1000];
				dp[i][j]+=v*(i-4==0 ? 26 : 25)%p;
			}
			if(i-5>=0 and j-1000>=0){
				ll v=dp2[i-5][j-1000];
				if(j-3001>=0)v=v+p-dp2[i-5][j-3001];
				dp[i][j]+=v*(i-5==0 ? 26 : 25)%p;
			}
		}
		rep(j,n+1){
			dp2[i][j]=dp[i][j];
			if(j)dp2[i][j]=(dp2[i][j]+dp2[i][j-1])%p;
		}
	}
	ll ans=0;
	rep(i,1,n){
