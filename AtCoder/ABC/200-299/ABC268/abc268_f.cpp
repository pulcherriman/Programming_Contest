...
int main() {
	...
	rep(i,n){
		...
		rep(j,sz(s[i])){
			if(s[i][j]!='X'){
				base +=  (s[i][j]-'0') * xCnt;
				// single_score[i] += (s[i][j]-'0') * xCnt;
				score_of_prev_x_count[i] += s[i][j]-'0';
			}else{
				xCnt++;
			}
		}
	...
	debug(base);
	debug(x_cnt);
	debug(score_of_prev_x_count);

	// vi recommendP;

	{
		vector<pair<ld, int>> p_score(n);
		rep(i,n){
			if(score_of_prev_x_count[i] == 0) p_score[i]={0, i};
			else if(x_cnt[i] == 0) p_score[i]={1e9, i};
			else p_score[i]={(ld)score_of_prev_x_count[i]/x_cnt[i], i};
		}
		debug(p_score);
		...
		ll ans=base;
		ll asum=0;
		for(auto&[_,i]:p_score){
			ans+=asum*score_of_prev_x_count[i];
			asum+=x_cnt[i];
		}
		out(ans);
	}
	...
	// 		ll asum=0;
	// 		rep(i,n){
	// 			ans+=asum*score_of_prev_x_count[p[i]];
	// 			asum+=x_cnt[p[i]];
	// 		}
	// 		if(chmax(greedyMax, ans)){
	// 			greedyP=p;
	// 		}
	// 		// if(ans==71){
	// 		// 	out(ans,":",p);
	...
	// 		// 	cerr<<endl;
	// 		// 	rep(i,n){
	// 		// 		cerr<<score_of_prev_x_count[p[i]]<<" ";
	// 		// 	}
	// 		// 	cerr<<endl;
	// 		// }
