...
namespace IO {
	...
	#else
	template<class...Ts> constexpr ostream& debug(Ts&...args) { return cerr; }
	#endif
...
int main() {
	...
	// i回移動して、今jにいて、Xを踏んだ回数がk=0?偶数:奇数
	vector<array<mymint,2>> dp(n, {0, 0});
	dp[s][0] = 1;
	
	rep(i,k){
		vector<array<mymint,2>> ndp(n, {0, 0});
		rep(j,n){
			for(auto&[to,_]:g.edge[j]){
				ndp[to][0] += (to==x ? dp[j][1] : dp[j][0]);
				ndp[to][1] += (to==x ? dp[j][0] : dp[j][1]);
			}
		}
		dp=ndp;
	}
