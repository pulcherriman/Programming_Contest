...
int main() {
	...
	def(ll,n,mx,mn);
	vl a(n); cin>>a;
	vl mnIndex(n,INF), mxIndex(n,INF), lowerIndex(n,INF), upperIndex(n,INF);
	rep(i,n){
		if(a[i]==mn)mnIndex[i]=i;
		if(a[i]==mx)mxIndex[i]=i;
		if(a[i] < mn)lowerIndex[i]=i;
		if(a[i] > mx)upperIndex[i]=i;
	}
	rrep(i,n-1){
		chmin(mnIndex[i], mnIndex[i+1]);
		chmin(mxIndex[i], mxIndex[i+1]);
		chmin(lowerIndex[i], lowerIndex[i+1]);
		chmin(upperIndex[i], upperIndex[i+1]);
	}
	...
	rep(l,n){// 左端を固定
		...
		if(r==INF)continue;
		ll x=n-1;
		chmin(x, lowerIndex[l]-1);
		chmin(x, upperIndex[l]-1);
		if(r > x)continue;
		debug(l,r,x);
		ans += x-r+1;
	}
