...
int main() {
	/*$1*/
	def(ll,n);
	HashMap<ll,HashMap<ll,ll>> p;
	HashMap<ll,ll> h, w;
	HashMap<ll,HashSet<ll>> xi;
	rep(i,n) {
		def(ll,x,y,v);
		p[x][y] = v;
		h[x]+=v;
	...
	for(auto&[k,v]:h) {
		ll hsum = v;
		for(auto&existY: xi[k]){
			if(w.find(existY) == w.end()) continue;
			ll wsum = w[existY];
			ll sum = hsum + wsum - p[k][existY];
			chmax(ans, sum);
			// debug(k,v, "dup", existY, wsum, p[k][existY], "->", sum);
		}

		for(auto&[wsum, x]:ymax) {
			if(xi[k].find(x) != xi[k].end()) continue;
			ll sum = hsum + wsum;
			chmax(ans, sum);
			// debug(k,v, "new", x, wsum, "->", sum);
			break;
		}
	}
	out(ans);
