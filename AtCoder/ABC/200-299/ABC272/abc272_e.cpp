...
int main() {
	...
	rep(i,n){
		...
		if(b[i]==0)targets.emplace(i);
		add_timing[b[i]].emplace_back(i);
	}
	// debug(b);
	...
	rep(i,1,m+1){
		ans.reset();
		for(auto&v:add_timing[i]){
			targets.emplace(v);
		}
		// debug(i,":");
		ll s=targets.size();
		rep(s){
			ll index=targets.front(); targets.pop();
			ll value=a[index] + (index+1) * i;
			// debug("  ", index, value);
			if(0<=value and value<=n) ans[value]=1;
			if(value<=n)targets.emplace(index);
		}
		rep(j,n+1){
			if(!ans[j]){
				out(j);
