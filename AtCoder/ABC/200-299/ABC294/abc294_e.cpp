...
int main() {
	...
	rep(i,1,n+1){
		a2.emplace_back(a[i].first, a[i-1].second);
		chg.insert(a2.back().second);
	}
	...
	rep(i,1,m+1){
		b2.emplace_back(b[i].first, b[i-1].second);
		chg.insert(b2.back().second);
	}
	debug(chg);
	...
	ll ans=0, aind=0, bind=0;;
	rep(i,sz(chg2)-1){
		auto st = chg2[i];

		while(aind+1 < sz(a2) and a2[aind+1].second <= st){
			aind++;
		}
		while(bind+1 < sz(b2) and b2[bind+1].second <= st){
			bind++;
		}
		debug(st, aind, bind, a2[aind], b2[bind]);
		if(a2[aind].first == b2[bind].first){
			ans += (chg2[i+1] - st);
		}
	}
	out(ans);
