...
int main(){
	geta(ll, n ,x,y);
	vl a(n),b(n);cin>>a>>b;
	auto dp=vec(LINF, n+1,1<<n);
	dp[0][0]=1;
	rep(i,n){
		rep(j,1<<n)if(dp[i][j]>0){
			rep(k,n)if((j&(1<<k))==0){// i番目に（もともと）k番目の数を利用
				int uppermask=((1<<n)-1)<<(k+1);
				int upperCnt=__builtin_popcount(j&uppermask);

				chmin(dp[i+1][j|(1<<k)], dp[i][j] + y*(k-i+upperCnt) + abs(b[i]-a[k])*x);
			}
		}
	}
	puta(dp[n][(1<<n)-1]-1);
