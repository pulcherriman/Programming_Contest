...
int main() {
	...
	// auto [_, d] = g.diameter();
	// out(d);
	auto [s,_]=getMaxAndIndex(g.bfs(n));
	auto [t,__]=getMaxAndIndex(g.bfs(s));

	auto dists_s = g.bfs(s);
	auto dists_t = g.bfs(t);
