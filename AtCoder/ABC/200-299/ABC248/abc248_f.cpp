...
int main(){
	...
	auto dp=vec(0ll, 2, n+2, 3);// 直前の二つが連結？　今までの全体が連結？
	dp[0][1][0]=1;
	dp[0][0][2]=1;
	...
	rep(i,n-1){
		...
		rep(j,n){
			dp[ni][j+1][0] += dp[ci][j][0] * 1;
			dp[ni][j+0][2] += dp[ci][j][0] * 1;

			dp[ni][j+2][0] += dp[ci][j][1] * 2;
			dp[ni][j+1][1] += dp[ci][j][1] * 1;
			dp[ni][j+1][2] += dp[ci][j][1] * 2;
			dp[ni][j+0][2] += dp[ci][j][1] * 1;
			
			dp[ni][j+2][0] += dp[ci][j][2] * 2;
			dp[ni][j+1][1] += dp[ci][j][2] * 1;
			dp[ni][j+1][2] += dp[ci][j][2] * 2;
			dp[ni][j+0][2] += dp[ci][j][2] * 1;
		}
		rep(j,n+2){
			dp[ni][j][0] %= md;
			dp[ni][j][1] %= md;
	...
	vl ans;
	rep(i,1,n)ans.push_back(
		(dp[(n-1)%2][i][1]+dp[(n-1)%2][i][2])%md);
	puta(ans);
}
