...
struct nCr{ nCr(int _) {}
	mymint operator()(int n, int r)const{ return comb.C(n,r); }
} nCr(0);
struct nPr{ nPr(int _) {}
	mymint operator()(int n, int r)const{ return comb.P(n,r); }
} nPr(0);
struct nHr{ nHr(int _) {}
	mymint operator()(int n, int r)const{ return comb.H(n,r); }
} nHr(0);

map<tl3, mymint> memo;
mymint calc(ll h, ll w, ll cnt){
	if(memo.count({h,w,cnt})) return memo[{h,w,cnt}];
	mymint ans = 0;
	rep(i,h+1)rep(j,w+1){
		ans += (-1_m).pow(i+j) * nCr(h,i) * nCr(w,j) * nCr((h-i)*(w-j),cnt);
	}
	return memo[{h,w,cnt}]=ans;
}

int main(){
	// 最終的に白を置く行の数、黒を置く行の数、（置かない行の数=y-p-q）
	// 最終的に白を置く列の数、黒を置く列の数、（置かない列の数=x-p-q）
	// 組合せのあと、行と列の組合せを求める
	geta(ll, y, x, b, w);
	mymint ans=0;
	rep(rb,0,y+1)rep(rw,0,y+1-rb)rep(cb,0,x+1)rep(cw,0,x+1-cb){
		if(rb*cb < b)continue;
		if(rw*cw < w)continue;
		mymint vb = calc(rb, cb, b);
		mymint vw = calc(rw, cw, w);
		mymint rows = nCr(y, rb) * nCr(y-rb, rw);
		mymint cols = nCr(x, cb) * nCr(x-cb, cw);
		mymint val= vb*vw*rows*cols;
		ans+=val;
	}
	puta(ans);
}
