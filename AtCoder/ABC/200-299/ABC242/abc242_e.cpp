...
int main(){
	...
	rep(q){
		...
		mymint strict=1, free=0, needMin=0;
		rep(i,(n+1)/2){
			if(s[i] < s[n-1-i]){
				char minC=min(s[i], s[n-i-1]);
				int cnt=minC-'A'+1;
				mymint nstrict=strict + needMin;
				mymint nfree = free*26 + strict*(cnt-1) + needMin * (cnt-1);
				mymint nneedMin = 0;
				strict=nstrict;
				free=nfree;
				needMin=nneedMin;
				...
				char minC=min(s[i], s[n-i-1]);
				int cnt=minC-'A'+1;
				mymint nstrict=strict;
				mymint nfree = free*26 + strict*(cnt-1) + needMin * (cnt-1);
				mymint nneedMin = needMin;
				strict=nstrict;
				free=nfree;
				needMin=nneedMin;
			}else{
				...
				int cnt=minC-'A'+1;
				mymint nstrict=0;
				mymint nfree = free*26 + strict*(s[i]-'A') + needMin * (s[i]-'A');
				mymint nneedMin = max(strict, needMin);
				strict=nstrict;
				free=nfree;
				needMin=nneedMin;
			}
			// puta(strict, free, needMin);
		}
		puta(strict + free);
	}
}
