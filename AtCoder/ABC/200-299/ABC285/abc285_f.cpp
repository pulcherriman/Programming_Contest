...
public:
	...
	friend ostream &operator<<(ostream &os, SegmentTree &st) {
		Val v;
		rep(i,st.n-1) os<<(v=st[i])<<' ';
		return os<<(v=st[st.n-1]);
	}
};
...
int main() {
	...
	vector<SegmentTree<SegTreeUtil::Add_Sum<ll>>> alphabet(26, n);
	rep(i,n){
		alphabet[s[i]-'a'].update(i,1);
	}
	SegmentTree<SegTreeUtil::Update_Min<ll>> diff(n-1);
	rep(i,n-1){
		diff.update(i, s[i] <= s[i+1]);
	}
	...
	rep(q){
		...
		if(tp==1){
			def(ll,x); x--;
			def(char,ch); ll c=ch-'a';
			alphabet[s[x]-'a'].update(x,-1);
			mp[s[x]-'a']--;
			s[x]=ch;
			alphabet[s[x]-'a'].update(x,1);
			mp[s[x]-'a']++;
			if(x>0){
				diff.update(x-1, s[x-1] <= s[x]);
			}
			if(x<n-1){
				diff.update(x, s[x] <= s[x+1]);
			}
		}else{
			def(ll,l,r); l--; r--;
			ll isShojun = diff.query(l,r);
			if(isShojun == 0){
				Yn(0);
				continue;
			}
			vl cnt(26,0);
			ll exist_l = n+1, exist_r = -1;
			rep(i,26){
				cnt[i]=alphabet[i].query(l,r+1).val;
				if(cnt[i]>0){
					exist_l = min(exist_l, i);
					exist_r = max(exist_r, i);
				}
			}
			bool ok=true;
			ok &= cnt[exist_l] <= mp[exist_l];
			ok &= cnt[exist_r] <= mp[exist_r];
			rep(i,exist_l+1,exist_r){
				ok &= cnt[i] == mp[i];
			}
			// out(s,mp);
			// out(s.substr(l,r-l+1),cnt);
			// out(exist_l, exist_r);
			Yn(ok);
		}
	}
