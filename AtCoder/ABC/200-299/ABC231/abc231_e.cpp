...
int main(){
	...
	rep(i,n){
		if(i+1!=n){
			b[i]=p2%a[i+1]/a[i];// ちょうど払うのに必要な枚数
			p2-=a[i]*b[i];
			c[i]=a[i+1]/a[i];//上の桁まで何倍か
		}else{
			b[i]=p2/a[i];
			c[i]=1;
		}
	}
	auto dp=vec(LINF, n+1, 2);
	dp[0][0]=0;
	dp[0][1]=1;
	dp[1][0]=b[n-1];
	dp[1][1]=b[n-1]+1;
	...
	rep(i,1,n){
		ll need = b[n-i-1];
		ll mx=c[n-i-1];
		chmin(dp[i+1][0], dp[i][0]+need);
		chmin(dp[i+1][0], dp[i][1]+(mx-need));
		chmin(dp[i+1][1], dp[i][0]+need+1);
		chmin(dp[i+1][1], dp[i][1]+(mx-need-1));
	}
	// puta(dp);
	// puta(b);
	// puta(c);
