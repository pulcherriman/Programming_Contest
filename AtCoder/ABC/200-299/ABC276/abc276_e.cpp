...
int main() {
	...
	ll y,x;
	rep(i,h)rep(j,w)if(f[i][j]=='S'){y=i;x=j;}

	vvb c(h, vb(w, false));
	vvl used(h, vl(w, false));
	used[y][x]=15;
	queue<tuple<ll,ll,ll>> q;
	if(x-1>=0 and f[y][x-1]!='#')q.emplace(y,x-1,1);
	if(x+1<w and f[y][x+1]!='#')q.emplace(y,x+1,2);
	if(y-1>=0 and f[y-1][x]!='#')q.emplace(y-1,x,4);
	if(y+1<h and f[y+1][x]!='#')q.emplace(y+1,x,8);
	while(!q.empty()){
		auto [y,x,d]=q.front(); q.pop();
		if(used[y][x] & d)continue;
		used[y][x]|=d;
		debug(y,x,d);
		if(__builtin_popcount(used[y][x])==2){
			out("Yes");
			return 0;
		}

		if(x-1>=0 and f[y][x-1]!='#' and !(used[y][x-1] & d))q.emplace(y,x-1,d);
		if(x+1<w and f[y][x+1]!='#' and !(used[y][x+1] & d))q.emplace(y,x+1,d);
		if(y-1>=0 and f[y-1][x]!='#' and !(used[y-1][x] & d))q.emplace(y-1,x,d);
		if(y+1<h and f[y+1][x]!='#' and !(used[y+1][x] & d))q.emplace(y+1,x,d);
	}
