...
void Main(){
	geta(ll, h,w,n);
	vector<IndexedMap<int,vi>> Y(h),X(w);
	rep(i,n){
		geta(ll,y,x,a);
		Y[y-1][a].push_back(i);
		X[x-1][a].push_back(i);
	...
	rep(2){
		for(auto&ar:Y){
			vvi values;
			rep(j,ar.size()-1){
				auto f=(*ar.find_by_order(j)).second;
				auto t=(*ar.find_by_order(j+1)).second;
				if(f.size()==1){
					for(auto&to:t){
						g.add(f[0], to, 1, true);
					}
					...
					for(auto&from:f){
						g.add(from, t[0], 1, true);
					}
				}else{
					auto ind = g.addNode()-1;
					for(auto&from:f){
						g.add(from, ind, 0, true);
					}
					for(auto&to:t){
						g.add(ind, to, 1, true);
					}
	...
	auto[_,topological] = g.topologicalSort();
	vi dp(g.size,0);
	rrep(i,g.size){
		int c=topological[i], maxV=0;
		for(auto&[to,cost]:g.edge[c])chmax(maxV, dp[to]+cost);
		dp[c]=maxV;
	}
