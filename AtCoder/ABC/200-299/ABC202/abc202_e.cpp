...
void Main(){
	...
	vvl da(n);
	rep(i,n){
		da[r.distance[i]].push_back(r.preorder[i]);
	}
	rep(i,n){
		sort(all(da[i]));
	}
	...
	rep(q){
		geta(ll,u,d);
		ll le=r.preorder[u-1],ri=r.preorder[u-1]+r.subtreeCount[u-1];
		ll ans=lower_bound(all(da[d]), ri)-da[d].begin();
		ans-=lower_bound(all(da[d]), le)-da[d].begin();
		puta(max(ans,0ll));
	}
