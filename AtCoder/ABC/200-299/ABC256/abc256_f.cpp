...
template <int mod> struct ModInt {
	static const inline ModInt MIN = ModInt(0);
	static const inline ModInt INF = ModInt().set_inf(true);
	...
	ModInt &set_inf(bool b) { x = b ? mod : 0; return *this; }
	ModInt operator+(const ModInt &p) const { return ModInt(*this) += p; }
	ModInt operator-(const ModInt &p) const { return ModInt(*this) -= p; }
	ModInt operator*(const ModInt &p) const { return ModInt(*this) *= p; }
	ModInt operator/(const ModInt &p) const { return ModInt(*this) /= p; }
	ModInt operator-() const { return x==mod ? ModInt(*this) : ModInt(-x); }
	ModInt &operator+=(const ModInt &p) { if(x < mod and (x += p.x) >= mod) x -= mod; return *this; }
	ModInt &operator-=(const ModInt &p) { if(x < mod and (x += mod - p.x) >= mod) x -= mod; return *this; }
	ModInt &operator*=(const ModInt &p) { if (x < mod) x = (int) (1LL * x * p.x % mod); return *this; }
...
namespace std {
    template<> struct numeric_limits<mymint> {
       static mymint max() { return mymint::INF; };
       static mymint min() { return mymint::MIN; };
    };
}
...
int main() {
	...
	SegmentTree<SegTreeUtil::Update_Sum<mymint>> st2(n), st1(n), st0(n);

	rep(i,n){
		mymint ind=i+1;
		st2.update(i, ind*ind*a[i]);
		st1.update(i,     ind*a[i]);
		st0.update(i,         a[i]);
	}
	...
	rep(i,q){
		...
		if(t==1){
			def(ll,v);
			st2.update(x-1, 1_m*x*x*v);
			st1.update(x-1,   1_m*x*v);
			st0.update(x-1,     1_m*v);
		}else{
			mymint ans=	 st2.query(0,x).val
			        	-st1.query(0,x).val*(2*x+3)
						+st0.query(0,x).val*(x+1)*(x+2);
			out(ans/2);
		}
	}
}