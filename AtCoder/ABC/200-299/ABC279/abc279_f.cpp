...
int main() {
	...
	def(ll,n,q);
	UnionFind uf(n+q);
	vl balls(n); iota(all(balls), 0);
	vl ans(n+q); iota(all(ans),0);
	...
	rep(q){
		...
		if(t==1){
			...
			uf.unite(balls[x], balls[y]);
			balls[x] = uf.find(balls[x]);
			balls[y] = -1;
			ans[balls[x]] = x;
		}else if(t==2){
			def(ll,x); //xにsize(x)+1をいれる
			x--;
			if(balls[x] == -1) {
				balls[x] = total;
				ans[total] = x;
			}else{
				uf.unite(balls[x], total);
				balls[x] = uf.find(balls[x]);
				ans[balls[x]] = x;
			}
			total++;
