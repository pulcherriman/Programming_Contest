...
int main() {
	...
	vl a(n); in(a);
	vs edge(n); in(edge);
	vector<vp> warshall(n, vp(n, {LINF,LINF}));
	rep(i,n)rep(j,n)if(edge[i][j]=='Y'){
		warshall[i][j] = {1,-a[j]};
	}
	rep(k,n)rep(i,n)rep(j,n)if(i!=j){
		warshall[i][j] = min(warshall[i][j], make_pair(
			warshall[i][k].first + warshall[k][j].first,
			warshall[i][k].second + warshall[k][j].second
		));
	}
	def(ll,q);
	rep(q){
		def(ll,s,t);
		--s; --t;
		if(warshall[s][t].first==LINF){
			out("Impossible");
		}else{
			out(warshall[s][t].first, a[s]-warshall[s][t].second);
		}
	}
	debug(warshall);
