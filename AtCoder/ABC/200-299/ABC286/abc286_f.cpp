...
int main() {
	/*$1*/
	// ll m=110, prd=1000000000;
	// vl prime(m);
	// iota(all(prime), 1);

	// vvl dp(prime.size()+1,vl(m+1, 0));
	// dp[0][0]=1;
	// rep(i,0,prime.size())rep(j,0,m+1){
	// 	chmax(dp[i+1][j], dp[i][j]);
	// 	if(j+prime[i]<=m)chmax(dp[i+1][j+prime[i]], lcm(dp[i][j],prime[i]));
	// }

	// ll d=108;
	// rrep(i,prime.size(),0){
	// 	if(dp[i][d]==dp[i+1][d])continue;
	// 	rep(j,d)if(lcm(dp[i][j], d-j)==dp[i+1][d]){
	// 		out(j);
	// 		d-=j;
	// 		break;
	// 	}
	// }
	// out(Prime::factor(1338557220));
	vl p={4,9,5,11,19,7,13,17,23};
	// ll lcm_val=1;
	// for(auto&v:p)lcm_val=lcm(lcm_val,v);
	// out(sum(p));
	// out(lcm_val);
	ll m=108;
	vl a(m);
	...
	vl x,y;
	for(auto&v:p){
		x.emplace_back(b[init]-(init+1));
		y.emplace_back(v);
		init+=v;
	}

	out(ChineseRem(x,y).first)<<flush;


	// vl test(m); iota(all(test), 1);
	// ll repl=100;
	// rep(repl){
	// 	out(test);
	// 	test=f(test,a);
