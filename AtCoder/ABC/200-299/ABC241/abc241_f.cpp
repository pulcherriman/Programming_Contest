...
int main() {
	...
	queue<pii> q;
	auto add = [&](int ny, int nx, int dist){
		if(vis[{ny,nx}]!=0 and vis[{ny,nx}]<=dist)return;
		vis[{ny,nx}] = dist;
		q.emplace(ny,nx);
	};
	add(sy,sx,1);
	...
	if(vis[{gy,gx}]==0)cout<<-1<<endl;
	else cout<<vis[{gy,gx}]-1<<endl;
}