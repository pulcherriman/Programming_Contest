...
int main() {
	/**/
	auto [n,g] = Graph<>::input(false,true);
	debug(g);
	...
	vl outdeg(n,0);
	rep(i,n){
		outdeg[i] = g.edge[i].size();
		if(outdeg[i]==0)q.emplace(i);
		else for(auto&[to,_]:g.edge[i])backedge[to].emplace_back(i);
	}
	while(!q.empty()){
		int p=q.front(); q.pop();
		ans--;
		for(auto&to:backedge[p]){
			outdeg[to]--;
			if(outdeg[to]==0)q.emplace(to);
		}
	}
	out(ans);
