...
#pragma region
Timer __EXECTIMER(10000);
void Main();
int main(){cin.tie(nullptr);ios::sync_with_stdio(false);Main();dump(__EXECTIMER.get());return 0;}
#pragma endregion
#pragma endregion
...
ll calc(ll D, ll d, ll p, ll k, ll b=1){// d桁、p以下、積k以下
	ll res=0,v=1;
	if(p==1){
		rep(i,d+1,D+1)v*=i;
		return v/b;
...
ll solve(ll d, ll k, bool containFirstZero=false){
	ll res=calc(d,d,9,k);
	{
		ll v1=1,v2=1;
		rep(i,d-1){
		...
		if(containFirstZero)res+=v1*10-v2*9;
		else res+=(v1-v2)*9;
	}
	return res;
}

ll solve2(string s, ll k){
	ll res=0;
	rep(tgt,s.size()-1){
		if(s[tgt]=='0') return res+stoll(s.substr(tgt+1))+1;

		ll r=0;
		if(tgt>0){
			r=1;
			rep(s.size()-tgt-1)r*=10;
		}
		rep(v,1,s[tgt]-'0'){
			r+=solve(s.size()-tgt-1, k/v, true);
		}
		res+=r;
		k/=s[tgt]-'0';
	}
	return res+min((ll)(s.back()-'0'),k)+1;;
}

ll digitDP(string s, ll k){
	ll ans=solve2(s,k);
	rep(d,1,s.size())ans += solve(d,k);
	if(s.size()==1)ans--;
	return ans;
}
