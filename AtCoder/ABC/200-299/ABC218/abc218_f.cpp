...
void Main(){
	...
	vi path(1,n-1);
	while(path.back()!=0){
		for(auto[to,_]:ginv.edge[path.back()])if(dist[to]==dist[path.back()]-1){
			path.emplace_back(to);break;
		}
	}
	...
	for(auto&[a,b]:edges){
		...
		//400個以下
		auto backup=g.edge[a];
		e[a].erase({b,1});
		g.edge[a]=vector<tl2>(all(e[a]));
		ll ans=g.bfs(0)[n-1];
		if(ans==LINF)puta(-1);
		else puta(ans);
		g.edge[a]=backup;
