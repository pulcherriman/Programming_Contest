...
int main() {
	...
	rep(i,n){
		...
		if(s[i].size()==1){
			...
			mp2[(s[i][0]-'a')*26 + s[i][1]-'a']+=p[i];
		}else{
			mp3[(s[i][0]-'a')*26*26+(s[i][1]-'a')*26+(s[i][2]-'a')]+=p[i];
		}
	}
	...
	ll ans=max(mp1);
	rep(i,26)rep(j,26){
		ll v=mp1[i]+mp1[j]+mp2[i*26+j];
		chmax(ans, v);
	}

	rep(a,26)rep(b,26)rep(c,26){
		calc[a*26*26+b*26+c]+=mp1[a];
		calc[a*26*26+b*26+c]+=mp1[b];
		calc[a*26*26+b*26+c]+=mp1[c];
		calc[a*26*26+b*26+c]+=mp2[a*26+b];
		calc[a*26*26+b*26+c]+=mp2[b*26+c];
		calc[a*26*26+b*26+c]+=mp3[a*26*26+b*26+c];
		chmax(ans, calc[a*26*26+b*26+c]);
	}

	vvl pl(26*26, vl(26,0));

	rep(a,26)rep(b,26)rep(c,26){
		pl[a*26+b][c]+=mp1[c]+mp2[b*26+c]+mp3[a*26*26+b*26+c];
	}
	vvl edge(26*26, vl(26*26,0));
	Graph<ll> g(26*26);
	rep(a,26)rep(b,26)rep(c,26){
		g.add(a*26+b, b*26+c, pl[a*26+b][c]);
	}
	// next: グラフG上の最長のウォークを求める
