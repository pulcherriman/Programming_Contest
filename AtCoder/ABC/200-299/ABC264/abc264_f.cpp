...
int main() {
	...
	rep(2){
		...
		dp[0][0][0]=(f[0][0]=='0'?0:LINF);
		dp[0][0][1]=(f[0][0]=='1'?r[0]:LINF);
		dp[0][0][2]=(f[0][0]=='1'?c[0]:LINF);
		dp[0][0][3]=(f[0][0]=='0'?r[0]+c[0]:LINF);

		rep(i,h)rep(j,w)rep(k,4){
			if(dp[i][j][k]==LINF)continue;
			if(j+1<w){ //⇒
				if((k&1)==0){//行をひっくり返していない
					if(f[i][j+1]=='0'){//そのまま次に行ける
						chmin(dp[i][j+1][0], dp[i][j][k]);
					}else{
						chmin(dp[i][j+1][2], dp[i][j][k]+c[j+1]);
					}
				}else{
					if(f[i][j+1]=='1'){//そのまま次に行ける
						chmin(dp[i][j+1][1], dp[i][j][k]);
					}else{
						chmin(dp[i][j+1][3], dp[i][j][k]+c[j+1]);
					}
				}
			}
			if(i+1<h){// ↓
				if((k&2)==0){//列をひっくり返していない
					if(f[i+1][j]=='0'){//そのまま次に行ける
						chmin(dp[i+1][j][0], dp[i][j][k]);
					}else{
						chmin(dp[i+1][j][1], dp[i][j][k]+r[i+1]);
					}
				}else{
					if(f[i+1][j]=='1'){//そのまま次に行ける
						chmin(dp[i+1][j][2], dp[i][j][k]);
					}else{
						chmin(dp[i+1][j][3], dp[i][j][k]+r[i+1]);
					}
				}
			}
		}
		if(f[h-1][w-1]=='0'){
			chmin(ans, dp[h-1][w-1][0]);
			chmin(ans, dp[h-1][w-1][3]);
		}else{
			chmin(ans, dp[h-1][w-1][1]);
			chmin(ans, dp[h-1][w-1][2]);
		}
		rep(i,h)rep(j,w){
			if(f[i][j]=='0')f[i][j]='1';
			else f[i][j]='0';
