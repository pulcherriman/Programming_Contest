...
int main() {
	...
	// rep(m){
	// 	def(int,a,b); def(ll,c);
	// 	a--; b--;
	// 	if(edges[a].find(b)==edges[a].end() or edges[a][b]>c){
	// 		edges[a][b] = c;
	// 		edges[b][a] = c;
	// 	}
	// }
	...
	vector<tl4> edges(n+1);
	rep(m){
		def(ll,a,b,c);
		edges.emplace_back(c,a,b,0);
	}

	rep(i,q){
		def(ll,a,b,c);
		edges.emplace_back(c,a,b,i+1);
	}
	sort(all(edges));

	vb ans(q);
	UnionFind uf(n+1);
	for(auto&[c,a,b,t]:edges){
		if(t==0){
			uf.unite(a,b);
		}else{
			ans[t-1]=!uf.same(a,b);
