...
int prec=0;
void search(int tgt, int l, int r) {
	int index=inindex[tgt];
	debug(tgt,l,r,index);
	{
		prec++;
		if(prec==n)return;
		int prenext=preo[prec];
		int nextIndex=inindex[prenext];
		bool ok =false;
		if(index>nextIndex){
			// prenextは左の子
			if(l<=nextIndex && nextIndex<index){
				ans[tgt-1].first=prenext;
				debug(prenext,"is left child of",tgt);
				ok=true;
				search(prenext,l,index);
			}
		}
		if(!ok)prec--;
	}
	{
		...
		if(index<nextIndex){
			// prenextは右の子
			if(index+1<=nextIndex && nextIndex<r){
				ans[tgt-1].second=prenext;
				debug(prenext,"is right child of",tgt);
				ok=true;
				search(prenext,index+1,r);
			}
		}
		if(!ok)prec--;
	}
...
int main() {
	...
	inindex.resize(n+1,-1);
	rep(i,n) inindex[ino[i]] = i;
	search(1,0,n);
