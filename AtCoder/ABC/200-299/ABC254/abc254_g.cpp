...
int main() {
	/*$1*/
	def(int,n,m,q);
	HashMap<int,vector<pii>> elevator;
	vector<pii> elevator2;
	rep(m){
		def(int,a,b,c);
		elevator[a].emplace_back(b,c);
	}
	for(auto&[tower,elev]:elevator){
		sort(all(elev));
		elev.emplace_back(INF,INF);

		int low=0, high=0;
		for(auto&[l,h]:elev){
			if(high<l){
				if(high)elevator2.emplace_back(low,high);
				low=l, high=h;
				continue;
			}
			chmax(high, h);
	...
	sort(all(elevator2));
	elevator2.emplace_back(INF,INF);
	auto[currentLow, currentHigh]=pii{0,0};
	auto[nextLow, nextHigh]=elevator2[0];
	int dist=0, root=elevator2[0].first;
	map<int,tuple<int,int,int>> elevator3;
	for(auto&[l,h]:elevator2){
		if(currentLow==l){
			chmax(currentHigh, h);
			continue;
		}
		if(currentHigh>=l){
			chmax(nextHigh, h);
			chmin(nextLow, l);
			chmax(nextLow,currentHigh+1);
			continue;
		}
		elevator3[nextHigh]=make_tuple(nextLow,dist,root);
		dist++;
		if(nextHigh<l){
			dist=0;
			root=l;
	...
	rep(q){
		def(int,st,sf,gt,gf);
		auto[_s,sd,sr]=elevator3.lower_bound(sf)->second;
		auto[_g,gd,gr]=elevator3.lower_bound(gf)->second;
		debug(_s,sd,sr,_g,gd,gr);
		if(sr!=gr){
			out(-1);
			continue;
		}
		int ans=abs(sf-gf)+abs(sd-gd);
		out(ans);
	}
