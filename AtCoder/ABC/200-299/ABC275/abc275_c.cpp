...
int main() {
	...
	vp p;
	rep(i,h)rep(j,w)if(f[i][j]=='#')p.emplace_back(i,j);
	
	for(auto&[y1,x1]:p) for(auto&[y2,x2]:p) for(auto&[y3,x3]:p) for(auto&[y4,x4]:p) {
		...
		ll e=(x1-x3)*(x1-x3)+(y1-y3)*(y1-y3);
		ll d=(x1-x4)*(x1-x4)+(y1-y4)*(y1-y4);
		ll b=(x2-x3)*(x2-x3)+(y2-y3)*(y2-y3);
		ll f=(x2-x4)*(x2-x4)+(y2-y4)*(y2-y4);
		ll c=(x3-x4)*(x3-x4)+(y3-y4)*(y3-y4);
		if(a==b and a==c and a==d and e==f and a*2==e) {
			ans++;
		}
	}
	out(ans/8);
