...
void Main(){
	...
	cin>>ff;
	vvl a(h,vl(w,0));
	rep(i,h)rep(j,w)if(ff[i][j]=='+')a[i][j]=1;else a[i][j]=-1;
	rep(i,h)rep(j,w)if((i+j)%2==0)a[i][j]*=-1;
	auto dp=vec(0ll,h,w);
	a[0][0]=0;


	dp[h-1][w-1]=a[h-1][w-1];
	rrep(i,h)rrep(j,w)if(i<h-1 or j<w-1){
		if((i+j)%2==1){//高橋君：上と左で大きいほうを選んで+a[i][j];
			ll v=LINF;
			if(i<h-1)chmin(v,dp[i+1][j]);
			if(j<w-1)chmin(v,dp[i][j+1]);
			if(v!=LINF)dp[i][j]=v+a[i][j];
		}else{
			ll v=-LINF;
			if(i<h-1)chmax(v,dp[i+1][j]);
			if(j<w-1)chmax(v,dp[i][j+1]);
			if(v!=-LINF)dp[i][j]=v+a[i][j];
		}
	}
	// puta(dp);
	if(dp[0][0]==0)puta("Draw");
	else if(dp[0][0]>0)puta("Takahashi");
	else puta("Aoki");
}
