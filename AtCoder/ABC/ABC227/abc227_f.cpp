...
void Main(){
	...
	// 	h=w=30;
	// 	a=vvl(h,vl(w));
	// 	rep(i,h)rep(j,w)a[i][j]=Random(100,1000);
	// }
	puta(a);
	vector<vector<vvl>> lg=vec(0ll,h,w,60,0);
	vector<vvl> ls=vec(LINF,h,w,60);
	lg[0][0][1].push_back(a[0][0]);
	ls[0][0][1]=0;
	ls[0][0][1]=a[0][0];
	rep(i,h)rep(j,w)if(i+j){
		rep(l,2,i+j+2){//i,jへのパスで最大のl個が最小のものの記録
			if(j>0){
				auto tmp=lg[i][j-1][l];
				tmp.push_back(a[i][j]);
				sort(rall(tmp)); // 挿入なのでlog外せるけど

				ll total=0;
				vl inv;
				rep(m,tmp.size()){
					if(chmin(ls[i][j][l], total)){
						lg[i][j][l]=inv;
					}
					total+=tmp[m];
					inv.push_back(tmp[m]);
