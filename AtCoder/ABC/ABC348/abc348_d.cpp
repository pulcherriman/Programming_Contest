...
template<class Cell=int>struct GridGraph : public Graph<int> , private GridGraphBase{
	...
	constexpr void input(){
		...
		rep(i,1,h-1)rep(j,1,w-1){cin>>c; at(i,j)=(c==WALL_CHAR?WALL:ROAD);
			if(c=='S') sy=i,sx=j, s=i*w+j; else if(c=='T') ty=i,tx=j, t=i*w+j;
		}
...
int main() { /**/
	...
	vvi able(q,vi(q, false));
	rep(i,q){
		auto [y,x,c]=item[i];
		int st = y*w+x;
		auto dist = g.bfs(st);
		rep(j,q){
			auto [y2,x2,c2]=item[j];
			int ed = y2*w+x2;
			if(dist[ed] <= c) {
				able[i][j] = true;
			}
	...
	rep(k,q)rep(i,q)rep(j,q)if(able[i][k] && able[k][j])able[i][j]=true;
	Graph<int> g2(q+2, INF);

	rep(i,q)rep(j,q){
		if(able[i][j])g2.add(i+1, j+1, 1, 1);
	}
	...
	}
	{
		...
		rep(i,q){
			auto [y,x,c]=item[i];
			int st = y*w+x;
			if(gd[st]<=c)g2.add(i+1, q+1, 1, 1);
		}
	}
	out(g2.bfs(0)[q+1] != INF ? "Yes" : "No");
