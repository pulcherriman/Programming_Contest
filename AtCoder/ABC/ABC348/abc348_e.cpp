...
template<class T=ll>struct Graph{
	...
	constexpr DFSResult<T> dfs(int s){
		...
		const function<void(int)> dfsrec=[this, &ret ,&pre, &post, &dfsrec](int p){
			...
			ret.stWeightSum[p] = wgt[p];
			for(auto[to,cost]:edge[p])if(!ret.connected[to]){
				ret.distance[to]=ret.distance[p]+cost;
				dfsrec(to);
				ret.stWeightSum[p] += ret.stWeightSum[to];
				ret.stAns[p] += ret.stAns[to] + ret.stWeightSum[to];
...
int main() { /**/
	auto [n,g] = Tree<ll>::input(false);
	auto d = g.dfs(0);
	vl ans(n,-1);
	ans[0] = d.stAns[0];
	rep(i,sz(d.eulertour)-1){
		int from = d.eulertour[i], to = d.eulertour[i+1];
		if(ans[to]!=-1) continue;
		ans[to] = ans[from] - d.stWeightSum[to] + (d.stWeightSum[0] - d.stWeightSum[to]);
	}
