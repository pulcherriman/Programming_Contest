...
int main() { /*$1*/
	...
	vvl wing(100);
	auto replaceFunc = [&](vl indices, vl r) {
		vl res(n);
		rep(i,n) res[i] = indices[r[i]-1];
		return res;
	};

	vl base(n); iota(all(base), 1);
	wing[0] = base;
	wing[1] = replaceFunc(base, x);
	rep(i,2,100) wing[i] = replaceFunc(wing[i-1], wing[i-1]);

	vl ans=a;
	rep(j,62) if(k>>j&1) {
		vl newAns(n);
		rep(i,n) newAns[i] = ans[wing[j+1][i]-1];
		ans = newAns;
	}
