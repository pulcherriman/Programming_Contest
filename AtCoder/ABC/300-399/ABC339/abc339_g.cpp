...
int main() { /**/
	...
	map<ll,ll> bb;
	for(auto&x:a) bb[x]++;
	vector<pair<ll,ll>> b(all(bb));

	

	int sq = ceil(sz(b)*sqrt(log(sz(b)) / q));
	vvl sts;
	vl v(1,-1);
	vector<map<ll,vl>> sets;
	rep(i,sq){
		sts.emplace_back(1,10);
		sets.eb();
		ll mnVal = v.back();
		ll mxVal = b[(sz(b)-1)*(i+1)/(sq)].first;
		v.pb(mxVal);
		rep(j,n){
			if(mnVal<a[j] && a[j]<=mxVal) {
				sets.back()[a[j]].pb(j);
			}
			sts.back().push_back(a[j]<=mxVal ? a[j] : 0);
		}
		partial_sum(all(sts.back()), begin(sts.back()));
	}
	...
	rep(zzz,q){
		def(ll,l,r,x);
		l^=prevAns; r^=prevAns; x^=prevAns;
		l--;
		// l=0, r=n, x=zzz+1;
		ll ans=0;

		int lowerPos = upper_bound(all(v), x) - v.begin() - 1;
		if(lowerPos>0) {
			ans = sts[lowerPos-1][r] - sts[lowerPos-1][l];
		}
		{
			if(lowerPos<sz(sets)) {
				// debug(sz(sets[lowerPos]));
				for(auto&[val,poss] : sets[lowerPos]) {
					if(val<=x) {
						int cnt=0;
						auto it = lower_bound(all(poss), r);
						cnt += it - begin(poss);
						it = lower_bound(all(poss), l);
						cnt -= it - begin(poss);
						ans += val * cnt;
					}
				}
