...
int main() { /*$1*/
	ll xa,ya,xb,yb,xc,yc; in(xa,ya,xb,yb,xc,yc);
	/*
	高橋と荷物が
	1 2 3 3 0 5
	_____v_
	...
	*/
	// 先にXをそろえてからYをそろえる
	auto solveX = [](ll xa,ll ya,ll xb,ll yb,ll xc,ll yc){
		if(xb==xc) return make_tuple(xa, ya, xb, yb, 0ll);
		ll xt=xb-1, xf=xc-1;
		if(xb>xc) {xt=xb+1; xf=xc+1;}
		ll yt=yb;
		// (xa,ya)から(xt,yt)に移動 ただし(xb,yb)を通ってはいけない
		ll ans=abs(xa-xt)+abs(ya-yt);
		if(ya==yb and ((xa<xb and xb<xt) or (xt<xb and xb<xa))){ //abt
			ans+=2;
		}
		ans+=abs(xb-xc);
		return make_tuple(xf, yb, xc, yb, ans);
	};
	auto solveY = [](ll xa,ll ya,ll xb,ll yb,ll xc,ll yc){
		if(yb==yc) return make_tuple(xa, ya, xb, yb, 0ll);
		ll yt=yb-1, yf=yc-1;
		if(yb>yc) {yt=yb+1; yf=yc+1;}
		ll xt=xb;
		// (xa,ya)から(xt,yt)に移動 ただし(xb,yb)を通ってはいけない
		ll ans=abs(xa-xt)+abs(ya-yt);
		if(xa==xb and ((ya<yb and yb<yt) or (yt<yb and yb<ya))){ //abt
			ans+=2;
		}
		ans+=abs(yb-yc);
		return make_tuple(xb, yf, xb, yc, ans);
	};
	ll ans1=LINF, ans2=LINF;
	{
		auto [xa2, ya2, xb2, yb2, val1] = solveX(xa,ya,xb,yb,xc,yc);
		auto [xa3, ya3, xb3, yb3, val2] = solveY(xa2,ya2,xb2,yb2,xc,yc);
		chmin(ans1, val1+val2);
	}
	{
		auto [xa2, ya2, xb2, yb2, val1] = solveY(xa,ya,xb,yb,xc,yc);
		auto [xa3, ya3, xb3, yb3, val2] = solveX(xa2,ya2,xb2,yb2,xc,yc);
		chmin(ans2, val1+val2);
	}
	out(min(ans1, ans2));
