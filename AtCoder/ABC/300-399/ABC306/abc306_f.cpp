...
int main() {
	...
	vvl a(n, vl(m)); in(a);
	vl flat;;
	rep(i,n)rep(j,m) flat.push_back(a[i][j]);
	Compress<ll> c(flat);
	rep(i,n)rep(j,m) a[i][j] = c.zip(a[i][j]);
	SegmentTree<SegTreeUtil::Update_Sum<ll>> st(c.size());

	ll ans=n*(n-1)/2 * sumAtoB(1,m);
	rrep(i,n){
		rep(j,m){
			ans += st.query(0, a[i][j]).val;
		}
		rep(j,m){
			st.update(a[i][j], 1);
		}
