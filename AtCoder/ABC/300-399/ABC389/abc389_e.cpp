...
int main() { /**/
	...
	while(abs(ok-ng)>1){
		...
		ll cnt=0;
		rep(i,n){
			ll c = floor((ld)mid/n/a[i])/2;
			cnt += c * c * a[i];
			// q.emplace((c+1)*(c+1)*a[i], c+1, a[i]);
		}
		// priority_queue<tl3> q;
		// ll rem=m-cnt;
		// while(rem>0){
		// 	auto [v, c, x] = q.top(); q.pop();
		// 	if(v>rem){
		// 		break;
		// 	}
		// 	rem-=v;
		// 	q.emplace((c+1)*(c+1)*x, c+1, x);
		// }
		debug(mid, cnt);
		if(cnt <= m){
			ok = mid;
	...
	rep(i,n){
		...
		ans += c;
		rem -= c*c*a[i];
		q.emplace((c+1)*(c+1)*a[i] - c*c*a[i], c+1, a[i]);
		debug(i, c+1, (c+1)*(c+1)*a[i] - c*c*a[i]);
	}
	debug(ans, rem);
	while(rem>0){
		auto [v, c, x] = q.top(); q.pop();
		...
		rem-=v;
		q.emplace((c+1)*(c+1)*x - c*c*x, c+1, x);
		ans++;
	}
	out(ans);
