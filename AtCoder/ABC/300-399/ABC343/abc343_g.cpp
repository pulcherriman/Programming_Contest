...
int main() { /**/
	...
	rep(i,n)rep(j,n){
		...
		// s[i]とs[j]をこの順で繋げた時、重複しないs[j]の長さ
		rep(k,1,s[i].size()+1){
			if(s[j].size()<k) continue;
	...
	ll dbit=0;
	rep(i,n)if(skipped[i]) dbit|=1<<i;
	rep(i,n)if(!skipped[i]){
		dp[dbit|(1<<i)][i]=sz(s[i]);
	}
	rep(bit,1,1<<n)rep(i,n){
		if(!(bit&(1<<i))) continue;
		rep(j,n){
			if(bit&(1<<j)) continue;
