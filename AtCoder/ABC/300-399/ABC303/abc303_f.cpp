...
__int128_t sumAtoB(__int128_t a,__int128_t b){return (a+b)*(b-a+1)/2;}
...
int main() {
	...
	while(ok-ng>1) {
		...
		__int128_t remTurn = mid;
		while(remTurn and !pq.empty()){
			auto [dmg, i] = pq.back();
			if(-v[i][1] > remTurn){
				chmax(overTurn, v[i][2]);
				pq.pop_back();
				continue;
			}

			// debug("Pick",i,":",v[i]);
			__int128_t overDmg = remTurn * overTurn;
			if(dmg >= overDmg){
				// 攻撃iを可能な限り繰り返す
				__int128_t cnt = remTurn - (-v[i][1]) + 1;
				remain -= dmg * cnt;
				// debug("ATTACK", i, ":", dmg, cnt);
				if(remain <= 0) break;
				remTurn = (-v[i][1])-1;
				continue;
			}else{
				// x*overTurn >=dmgであるようなxまで、overDmg攻撃
				__int128_t x=(dmg+overTurn-1)/overTurn;
				// debug("OVERATTACK", x,"to",remTurn,":",overTurn);
				remain -= sumAtoB(x, remTurn) * overTurn;
				if(remain <= 0) break;
				remTurn = x-1;
				continue;
			}
		}
		if(remain>0 and remTurn>0){
			// debug("!!! CHECKED", mid, remain, remTurn, overTurn, pq.size());
			remain -= sumAtoB(1, remTurn) * overTurn;
		}
		if(remain<=0) ok=mid;
		else ng=mid;
	}
