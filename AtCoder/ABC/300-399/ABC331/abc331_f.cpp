...
using mymint2 = ModInt<1000000007>;
using vm = vector<mymint>;
using vm2 = vector<mymint2>;
using vvm = vector<vm>;
...
int main() { /*$1*/
	def(ll,n,q);
	def(string,s);
	SegmentTree<SegTreeUtil::Update_Sum<mymint>> st1(n), inv_st1(n);
	SegmentTree<SegTreeUtil::Update_Sum<mymint2>> st2(n), inv_st2(n);
	ll B1 = 29, B2 = 31;
	mymint v1=1; mymint2 v2=1;
	vm base1(n); vm2 base2(n);
	rrep(i,n){
		st1.update(i, v1 * (s[i]-'a'+1));
		st2.update(i, v2 * (s[i]-'a'+1));
		base1[i] = v1;
		base2[i] = v2;
		v1 = v1 * B1;
		v2 = v2 * B2;
	...
	rep(i,n){
		inv_st1.update(i, base1[n-1-i] * (s[i]-'a'+1));
		inv_st2.update(i, base2[n-1-i] * (s[i]-'a'+1));
	}
	...
	rep(q){
		...
		if(t==1){
			def(ll,i); def(char,c);
			i--;
			st1.update(i, (c-'a'+1) * base1[i]);
			st2.update(i, (c-'a'+1) * base2[i]);
			inv_st1.update(i, (c-'a'+1) * base1[n-1-i]);
			inv_st2.update(i, (c-'a'+1) * base2[n-1-i]);
		}
		else {
			def(ll,l,r);
			l--;
			mymint v1 = st1.query(l,r).val / base1[r-1];
			mymint2 v2 = st2.query(l,r).val / base2[r-1];
			mymint inv_v1 = inv_st1.query(l,r).val / base1[n-1-l];
			mymint2 inv_v2 = inv_st2.query(l,r).val / base2[n-1-l];
			Yn(v1==inv_v1 && v2==inv_v2);
