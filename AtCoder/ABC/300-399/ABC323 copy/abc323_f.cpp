...
template<class T=ll>struct Graph{
	...
	vector<vector<tuple<ll,T>>>inv_edge;
...
int main() { /*$1*/
	def(ll,n,m);
	Graph<pair<ll,ll>> g(n, pll(LINF,LINF));
	g.inputEdge(m,true,true);

	//bfs
	vector<vector<pll>> values(n);
	values[0].emplace_back(0,0);

	rep(p,n){
		auto np=set<pll>();
		if(p) sort(all(values[p]), [&](pll a, pll b){return (ld)a.first/a.second>(ld)b.first/b.second;});
		np.insert(begin(values[p]), begin(values[p])+min(100ll,sz(values[p])));
		sort(all(values[p]), [&](pll a, pll b){return a.first>b.first;});
		np.insert(begin(values[p]), begin(values[p])+min(30ll,sz(values[p])));
		sort(all(values[p]), [&](pll a, pll b){return a.second<b.second;});
		np.insert(begin(values[p]), begin(values[p])+min(30ll,sz(values[p])));
		values[p].assign(all(np));
		
		for(auto[to,cost]:g.edge[p]){
			for(auto[b,c]:values[p]){
				values[to].emplace_back(b+cost.first,c+cost.second);
			}
		}
	}
	sort(all(values[n-1]), [&](pll a, pll b){return (ld)a.first/a.second>(ld)b.first/b.second;});
	out(values[n-1][0].first / (ld)values[n-1][0].second);
