...
int main() { /**/
	def(ll,h,w,y);
	vvl a(h, vl(w)); in(a);
	vector<vector<pll>> posByHeight(100001);
	rep(i,h) rep(j,w) posByHeight[a[i][j]].emplace_back(i,j);

	UnionFind uf(h*w+1);
	rep(i,1,y+1){
		for(auto [y,x] : posByHeight[i]){
			rep(k,4){
				int ny = y+dy[k], nx = x+dx[k];
				if(ny<0 || ny>=h || nx<0 || nx>=w) {
					uf.unite(y*w+x, h*w);
				}else{
					if(a[ny][nx] <= i) uf.unite(y*w+x, ny*w+nx);
				}
			}
		}
		out(h*w+1-uf.size(h*w));
	}
}