...
int main() {
	auto [n,g] = Graph<>::input(true);
	vl ans(n, -1);

	minpq<tuple<ll,int>> pq;
	def(ll,k);
	rep(k){
		def(ll,p);
		ans[p-1] = 0;
		for(auto&[to,cost]:g.edge[p-1]){
			pq.emplace(cost, to);
		}
	}
	...
	rep(i,d){
		...
		minpq<tuple<ll,int>> pq2;
		while(!pq.empty()){
			auto [cost, p] = pq.top();
			if(cost > x)break;
			pq.pop();
			if(ans[p] == -1) pq2.emplace(cost, p);
		}
		while(!pq2.empty()){
			auto [c2, p2] = pq2.top(); pq2.pop();
			if(ans[p2] != -1)continue;
			ans[p2] = i+1;
			for(auto&[to,c]:g.edge[p2]){
				if(ans[to] != -1)continue;
				if(c2+c <= x) pq2.emplace(c2+c, to);
				else pq.emplace(c, to);
			}
		}
	}
