...
int main() { /*$1*/
	...
	// dp[i][j][k] マス(i,j)に到達し、そのマスまでの最高報酬額がkである場合の(ans, -所持金)
	vector<vector<HashMap<ll, pll>>> dp(n, vector<HashMap<ll, pll>>(n));
	dp[0][0][a[0][0]] = {0, 0};
	rep(i,n)rep(j,n){
		if(i+1<n){
			for(auto [money, reward] : dp[i][j]){
				auto [ans, have] = reward;
				have*=-1;
				auto cost = c[i][j];
				if(!dp[i+1][j].contains(max(money, a[i+1][j]))) {
					dp[i+1][j][max(money, a[i+1][j])] = {LINF,LINF};
				}
				if (cost>have) {
					auto rem = cost-have;
					auto cnt = (rem+money-1)/money;
					chmin(dp[i+1][j][max(money, a[i+1][j])], {(ans+1+cnt), -(have+money*cnt-cost)});
				} else {
					chmin(dp[i+1][j][max(money, a[i+1][j])], {(ans+1), -(have-cost)});
				}
			}
		}
		if(j+1<n){
			for(auto [money, reward] : dp[i][j]){
				...
				have*=-1;
				auto cost = r[i][j];
				if(!dp[i][j+1].contains(max(money, a[i][j+1]))) {
					dp[i][j+1][max(money, a[i][j+1])] = {LINF,LINF};
				}
				if (cost>have) {
					auto rem = cost-have;
					auto cnt = (rem+money-1)/money;
					chmin(dp[i][j+1][max(money, a[i][j+1])], {(ans+1+cnt), -(have+money*cnt-cost)});
				} else {
					chmin(dp[i][j+1][max(money, a[i][j+1])], {(ans+1), -(have-cost)});
				}
			}
		}
	}
