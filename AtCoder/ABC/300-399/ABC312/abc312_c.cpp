...
int main() {
	...
	vl tgt;
	rep(i,n)tgt.push_back(a[i]);
	rep(i,n)tgt.push_back(a[i]-1);
	rep(i,n)tgt.push_back(a[i]+1);
	rep(i,m)tgt.push_back(b[i]);
	rep(i,m)tgt.push_back(b[i]-1);
	rep(i,m)tgt.push_back(b[i]+1);
	...
	for(auto&p:tgt){
		// cnta = p以下のaの要素数
		ll cnta = upper_bound(all(sorta), p) - sorta.begin();
		// cntb = p以上のbの要素数
		ll cntb = sortb.end() - lower_bound(all(sortb), p);
		debug(p, cnta, cntb);
		if(cnta >= cntb) {
			chmin(ans, p);
		}
