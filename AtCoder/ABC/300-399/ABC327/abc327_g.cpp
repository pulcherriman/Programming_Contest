...
int main() { /*$1*/
	...
	// rep(i,n+1){
	// 	mymint val=0;
	// 	val += nCr(n,i) * mymint(n-i).pow(i) * nHr(n+i*(n-i), m-i);
	// 	ans+=val;
	// 	debug(i, val);
	// }
	rep(c,0,n+1){
		rep(a,1,n-c){
			auto b=n-a-c;
			// a,b個の頂点がある状態で、全ての頂点が次数を持つような最小のグラフ
			mymint val = 0;
			{
				int aa=a, bb=b;
				if(aa>bb) swap(aa,bb);
				rep(i,1<<aa){
					mymint cnt=mymint(__builtin_popcount(i)).pow(bb);
					if((aa-__builtin_popcount(i))%2 == 1) cnt=-cnt;
					val+=cnt;
				}
			}
			auto tmp = nCr(n,c) * nCr(n-c,a) * nHr(n+max(a,b), m-max(a,b)) * val;
			ans+=tmp;
			out(a,b,val, tmp);
		}
	}
