...
public:
	...
	SegmentTree(const vector<Val>& data_)
		: h(ceil(log2(data_.size()))), n(1 << h), data(n * 2, Monoids::id_val()), lazy(n * 2, Monoids::id_op()), input_n(data_.size()) {
	...
	friend ostream &operator<<(ostream &os, SegmentTree &st) {
		Val v;
		rep(i,st.input_n-1) os<<st[i]<<' ';
		return os<<st[st.input_n-1];
	}
};
...
int main() { /*$1*/
	...
	rep(i,200001) sort(all(a[i]));

	ll ans=0;
	SegmentTree<SegTreeUtil::Add_Max<ll>> st(400001, 0);
	rep(i,1,200001){
		for(auto&x:a[i]) st.update(x, x+w, 1);
		if(i-h>=0) for(auto&x:a[i-h]) st.update(x, x+w, -1);
		chmax(ans, st.query(1, 400001));
	}
	out(ans);
