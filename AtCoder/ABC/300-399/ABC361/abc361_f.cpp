...
ll getInvpow(ll n, ll b) {
	if(b > 60) return 1;
	if(memo.count(b)) return memo[b];
	ll r = floor(powl(n, 1.0l / b));
	while(true) {
		ll v = 1;
		bool ok=true;
		rep(b-1) {
			if (v >= LINF/(r+1)) {ok=false; break;}
			v *= r+1;
		}
		if(!ok) break;
		if (v <= n/(r+1)) r++;
...
int main() { /**/
	...
	rep(b,2,61) {
		...
		// 包除原理
		ans += getInvpow(n, b);
		rep(s,1, 1ll << sz(prSet)) {
			ll prProd = b;
			rep(i,sz(prSet)) {
				if(s & (1ll << i)) prProd *= prSet[i];
				if(prProd > 60) {
					break;
				}
			}
			ll cnt = getInvpow(n, prProd);
			if(__builtin_popcountll(s) & 1) ans -= cnt;
			else ans += cnt;
		}
		prSet.pb(b);
	}
