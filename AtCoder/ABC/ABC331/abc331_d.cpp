...
int main() { /*$1*/
	def(ll,n,q);
	vs f(n); in(f);
	vvl psum(n+1, vl(n+1, 0));
	rep(i,n)rep(j,n){
		psum[i+1][j+1] = psum[i+1][j] + psum[i][j+1] - psum[i][j] + (f[i][j]=='B');
	}

	auto calc = [&](int y1,int x1,int y2,int x2){
		return psum[y2][x2] - psum[y1][x2] - psum[y2][x1] + psum[y1][x1];
	};
	rep(q){
		def(ll,y1,x1,y2,x2);
		y2++; x2++;
		...
		if(y1%n!=0){
			if(x1%n!=0){
				ans += calc(y1%n, x1%n, n, n);
			}
			if(x2%n!=0){
				ans += calc(y1%n, 0, n, x2%n);
			}
			ll l=x1/n + (x1%n!=0 ? 1 : 0);
			ll r=x2/n;
			ans += calc(y1%n, 0, n, n) * (r-l);
		}
		if(y2%n!=0){
			if(x1%n!=0){
				ans += calc(0, x1%n, y2%n, n);
			}
			if(x2%n!=0){
				ans += calc(0, 0, y2%n, x2%n);
			}
			ll l=x1/n + (x1%n!=0 ? 1 : 0);
			ll r=x2/n;
			ans += calc(0, 0, y2%n, n) * (r-l);
		}
		ll t=y1/n + (y1%n!=0 ? 1 : 0);
		ll b=y2/n;
		if(b-t){
			if(x1%n!=0){
				ans += calc(0, x1%n, n, n) * (b-t);
			}
			if(x2%n!=0){
				ans += calc(0, 0, n, x2%n) * (b-t);
			}
			ll l=x1/n + (x1%n!=0 ? 1 : 0);
			ll r=x2/n;
			ans += calc(0, 0, n, n) * (b-t) * (r-l);
		}
		out(ans);
	}
}