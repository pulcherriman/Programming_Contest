...
int main() { /*$1*/
	...
	// dp[i][j][k] = i番目までの料理をj個食べて、合計の甘さ(a)がkの時の最小の辛さ(b)の合計
	vvl dp(n+1,vl(20001, LINF));
	dp[0][0] = 0;

	rep(i,n){
		auto dp2=vvl(n+1, vl(10001, LINF));
		rep(j,i+1){
			rep(k,10001){
				if(dp[j][k]==LINF) continue;
				chmin(dp2[j][k], dp[j][k]);
				if(k+a[i]<=x and dp[j][k]+b[i]<=y){
					chmin(dp2[j+1][k+a[i]], dp[j][k]+b[i]);
				}
			}
		}
		dp = dp2;	
