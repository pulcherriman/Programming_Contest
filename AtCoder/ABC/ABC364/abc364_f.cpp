...
int main() { /*$1*/
	...
	for(auto[cost,l,r]:v) {
		ll lv=l, rv=r, cnt=r-l+1;
		// [l,r] の中にいくつの区間があるか計算する
		auto itl = range.lower_bound({l+1, l+1});
		if(itl!=range.begin()){
			itl--;
			if(itl->second<l) {
				itl++;
		...
		auto outeritr = range.lower_bound({r+1, r+1}); 
		auto itr = outeritr; itr--;

		// out_f(l,r,cost);

		for(auto it=itl; it!=outeritr; it++) {
			auto [cl, cr] = *it;
			lv = min(lv, cl);
			rv = max(rv, cr);
			// out_f(cl,cr);
			
			ll dl = clamp(cl, l, r), dr = clamp(cr, l, r);
			cnt -= dr-dl;
		}
		range.erase(itl, outeritr);
		range.insert({lv, rv});

		ans += cnt * cost;
		// out_f("=",cnt,cost);
	}
	if(range.size() == 1 and range.begin()->first == 0 and range.begin()->second == n-1) {
		out(ans);
	}else{
		out(-1);
	}
