...
void Main(){
	...
	sort(all(a), [](vl&a, vl&b){return a[1]<b[1] or (a[1]==b[1] and a[0]>b[0]);});
	rep(i,m){
		int l=a[i][0],r=a[i][1], x=a[i][2];
		l--;r--;
		// puta(l,r,x);
		ll sum = st.prod(l,r+1).sum;
		ll need=x-sum;
		if(need<=0)continue;
		while(need){
			auto it=unused.lower_bound(-r);
			if(it==unused.end()){
				break;
			}
			int nextuse=-(*it);
	...
	vl ans;
	rep(i,n)ans.push_back(st.get(i).sum);
	puta(ans);
}
