...
int main() {
	...
	int l=0,r=0;
	while(true){
		if(r+1<a.size() and l%n!=(r+1)%n and (a[r+1]-a[r]==0 or a[r+1]-a[r]==1)){
			r++;
			continue;
		}
		if(r+1<a.size() and l%n!=(r+1)%n and a[r+1]==0 and a[r]==m-1){
			r++;
			continue;
		}
		chmin(ans, s - (b[r]-(l?b[l-1]:0)));
		debug(l,r, s, (b[r]-(l?b[l-1]:0)), ans);
		l=r=r+1;
		if(l>=a.size())break;
	}
	out(ans);
