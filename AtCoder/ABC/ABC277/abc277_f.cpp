...
int main() {
	...
	sort(all(b));
	rep(i,h-1){
		if(b[i].first.second > b[i+1].first.first){
			out("No");
			return 0;
		}
	}
	rep(i,h){
		a[i] = b[i].second;
		if(b[i].first.first == b[i].first.second){
			rep(j,w)if(a[i][j]==0)a[i][j]= b[i].first.first;
		}
	}
	Graph g(w);
	map<pll, pll> mp;
	auto getNode = [&](ll i, ll v){
		if(mp.count({i,v}))return mp[{i,v}];
		int id = g.addNode();
		int id2 = g.addNode();
		debug("!!", i, v, "->",id, id2);
		return mp[{i,v}] = {id, id2};
	};
	rep(ln,h){
		debug("====");
		auto&c=a[ln];
		vp p;
		rep(i,w)p.emplace_back(c[i], i);
		sort(all(p));
		rep(i,w){
			if(p[i].first==0)continue;
			auto [before, after] = getNode(ln, p[i].first);
			g.add(before, p[i].second, 1, true);
			g.add(p[i].second, after, 1, true);
			debug(before, p[i].second, after);
		}
		rep(i,w-1){
			if(p[i].first==0)continue;
			if(p[i].first < p[i+1].first){
				auto [_, prev_after] = getNode(ln, p[i].first);
				auto [nxt_before, __] = getNode(ln, p[i+1].first);
				g.add(prev_after, nxt_before, 1, true);
				debug(prev_after, nxt_before);
			}
		}
	}
	debug(g);
	auto[ok, ans] = g.topologicalSort();
	debug(g.size, ans.size());
	Yn(ok);
	debug(a);
	debug(ans);
...
/*
7 8 0 9 3  2 1 5 4  6 15
16 10  17 18 19 20 11 21 12 22 23 24 13 25 14  15
16 10  17 18 19 11 12 13 14  15
7 8 16 0 9 10 3 17 18 2 19 20 1 11 21 12 22 5 23 24 4 13 25 14 6 15
