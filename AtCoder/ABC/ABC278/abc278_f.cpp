...
int main() {
	...
	def(ll,n);
	vs s(n); in(s);
	vvl bitdp(1<<n, vl(n, -1));
	rep(i,n)bitdp[(1<<n)-1][i] = 1-n%2; //全ての単語が使われたなら、単語が奇数個なら先手勝ち
	rrep(i,1,1<<n) {
		// debug(bitset<6>(i), bitdp[i]);
		rep(j,n) {
			if(bitdp[i][j] == -1)continue;
			int pops=__builtin_popcount(i);
			int turn = 1-pops%2;
			if(turn == bitdp[i][j]){
				// このターン、turnのプレイヤーが単語s[j]を置いたら勝つ
				// →s[j][0]で終わる単語で回って来たら勝つ
				rep(k,n)if(j!=k and i>>k&1){
					if(s[k].back() == s[j][0]){
						bitdp[i^(1<<j)][k] = turn;
					}else{
						if(bitdp[i^(1<<j)][k] != turn) bitdp[i^(1<<j)][k] = 1-turn;
					}
				}
			}else{
				// このターン、turnのプレイヤーが単語s[j]を置いたら負ける
				// 単語を置けなくても負ける
				rep(k,n)if(j!=k and i>>k&1){
					if(bitdp[i^(1<<j)][k] != turn) bitdp[i^(1<<j)][k] = 1-turn;
				}
