...
int main() {
	...
	map<ll, vl> wall_tate;
	for(auto [y,x]:wall) {
		wall_yoko[y].push_back(x);
		wall_tate[x].push_back(y);
	}
	for(auto&[k,v]:wall_yoko) {
		v.push_back(0);
		v.push_back(w+1);
		v.push_back(-LINF);
		v.push_back(LINF);
	...
	rep(q){
		...
		def(ll,dist);
		if(c=='L'){
			if(wall_yoko[sy].empty()) {
				wall_yoko[sy].push_back(-LINF);
				wall_yoko[sy].push_back(0);
				wall_yoko[sy].push_back(w+1);
				wall_yoko[sy].push_back(LINF);
			}
			auto wl=*(lower_bound(all(wall_yoko[sy]),sx)-1);
			sx=max(wl+1,sx-dist);
		}
		if(c=='R'){
			if(wall_yoko[sy].empty()) {
			...
			auto wl=*lower_bound(all(wall_yoko[sy]),sx);
			sx=min(wl-1,sx+dist);
		}
		if(c=='U'){
			if(wall_tate[sx].empty()) {
				wall_tate[sx].push_back(-LINF);
				wall_tate[sx].push_back(0);
				wall_tate[sx].push_back(h+1);
				wall_tate[sx].push_back(LINF);
			}
			auto wl=*(lower_bound(all(wall_tate[sx]),sy)-1);
			sy=max(wl+1,sy-dist);
		}
		if(c=='D'){
			if(wall_tate[sx].empty()) {
			...
			auto wl=*lower_bound(all(wall_tate[sx]),sy);
			sy=min(wl-1,sy+dist);
		}
		out(sy,sx);
	}
