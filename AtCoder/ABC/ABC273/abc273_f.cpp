...
int main() {
	...
	Graph<ll> g(n*2+2);
	rep(i,n){
		g.add(i*2+1, i*2+2, abs(wall[i]-hammer[i]), true);
	}
	{
		vl w=wall;
		w.push_back(-LINF);
		...
		sort(all(w));
		// {
		// 	auto near_wall=*upper_bound(all(w), 0);
		// 	rep(i,n)if(hammer[i]>0 and hammer[i] < near_wall){
		// 		g.add(0, i*2+1, hammer[i], true);
		// 	}
		// }
		// {
		// 	auto near_wall=*(lower_bound(all(w), 0)-1);
		// 	rep(i,n)if(hammer[i]<0 and hammer[i] > near_wall){
		// 		g.add(0, i*2+1, -hammer[i], true);
		// 	}
		// }
		{
			auto near_wall=*(upper_bound(all(w), tgt)-1);
			if(near_wall<0){
				g.add(0, n*2+1,-near_wall, true);
			}else{
				rep(i,n)if(wall[i]==near_wall){
					g.add(i*2+2, n*2+1, tgt-near_wall, true);
				}
			}
		}
	}
	{
		vp mp;
		mp.emplace_back(0,0);
		mp.emplace_back(tgt,n*2+1);
		rep(i,n){
			mp.emplace_back(hammer[i],i*2+1);
			mp.emplace_back(wall[i],i*2+2);
		}
		sort(all(mp));
		rep(i,n){
