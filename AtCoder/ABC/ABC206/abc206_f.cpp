...
void Main(){
	...
	rep(t){
		...
		rep(i,n)rep(j,i+1,n){
			bool ok=false;
			if(p[i].first<=p[j].first and p[j].first<p[i].second)ok=true;
			if(p[i].first<p[j].second and p[j].second<=p[i].second)ok=true;
			if(p[i].first>p[j].first and p[j].second>=p[i].second)ok=true;
			if(ok)g.add(i,j,1);
		}
		...
		ll cnt=0;
		rep(i,n)if(!vis[i]){
			auto dd=g.dfs(i).distance;
			rep(j,n)if(dd[j]==LINF)dd[j]=-1;
			auto[in,v]=getMaxAndIndex(dd);
			auto d=g.dfs(in).distance;
			// puta(i,d);
			rep(j,n)if(d[j]!=LINF)vis[j]=1;else d[j]=-1;
			// puta(*max_element(all(d))+1);
			// puta(vis);
			cnt++;
		}
		if(cnt==2)puta("Bob");
