...
int main(){
	...
	cin>>s;
	int n=s.size();
	vm a(n+1,0),b(n+1,0),c(n+1,0),q(n+1,0),pow3(n+1,1);
	rep(i,n){
		(s[i]!='A'?s[i]!='B'?s[i]!='C'?q:c:b:a)[i+1]=1;
	}
	partial_sum(all(a),a.begin());
	partial_sum(all(b),b.begin());
	partial_sum(all(c),c.begin());
	partial_sum(all(q),q.begin());
	...
	rep(i,n){
		...
		mint aa=a[i]*pow3[q[i]];
		if(q[i])aa+=q[i]*pow3[q[i]-1];
		mint cc=(c[n]-c[i+1])*pow3[q[n]-q[i+1]];
		if(q[n]-q[i+1])cc+=(q[n]-q[i+1])*pow3[q[n]-q[i+1]-1];
		ans+=aa*cc;
	}
	cout<<ans<<endl;
	return 0;
