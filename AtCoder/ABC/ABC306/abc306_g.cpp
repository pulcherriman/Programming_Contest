...
template<class T=ll>struct Graph{
	...
	constexpr vector<vector<T>> bfs2(int s){
		vector<T> ret(size,INF_VAL);
		vector<vector<T>> rets(size);
		deque<int> q;
		ret[q.emplace_back(s)]=0;
		while(!q.empty()){
			int p=q.front(); q.pop_front();
			for(auto[to,cost]:edge[p])if(chmin(ret[to],ret[p]+cost)){
				if(cost==0)q.emplace_front(to); else q.emplace_back(to);
				rets[to].emplace_back(ret[p]+cost);
			}else{
				rets[to].emplace_back(ret[p]+cost);
			}
		}
	...
	vector<vector<pair<int,T>>> inverse;
	constexpr void inputEdge(int edgeCount, bool weighted, bool directed){
		...
		rep(i,edgeCount){
			...
			inverse[b-1].emplace_back(a-1,c);
		}
	}
...
void solve(){
	auto [n,g] = Graph<ll>::input(false, true);
	Graph<ll> g2(n);
	rep(i,n)for(auto&[to,c]:g.inverse[i])g2.add(i,to,c,true);
	auto dist = g.bfs(0);
	auto back = g2.bfs(0);
	auto dist2 = g.bfs2(0);
	auto back2 = g2.bfs2(0);
	ll gc=0;
	rep(i,1,n){
		if(dist[i] == g.INF_VAL || back[i] == g.INF_VAL) continue;
		if(gc==0)gc=dist[i]+back[i];
		else gc=gcd(gc,dist[i]+back[i]);
		for(auto&c1:dist2[i])for(auto&c2:back2[i]) gc = gcd(gc, c1+c2);
	}
	if(gc==0) gc=3;
	while(gc%2==0) gc/=2;
	while(gc%5==0) gc/=5;
