...
int main() { /*$1*/
	def(ll,n,q);
	ll vmax = 200005;
	SegmentTree<SegTreeUtil::Update_Min<ll>> st(vmax);
	vl a(n); in(a);
	map<ll,ll> cnt;

	rep(i,vmax) st.update(i,i);
	for(auto&v:a){
		cnt[v]++;
		if(v<vmax)st.update(v,LINF);
	}
	rep(i,q){
		def(ll,b,c);
		b--;
		if(a[b]<vmax and cnt[a[b]]==1)st.update(a[b],a[b]);
		cnt[a[b]]--;
		a[b]=c;
		cnt[a[b]]++;
		if(c<vmax) st.update(a[b],LINF);
		out(st.query(0,vmax));
	}
