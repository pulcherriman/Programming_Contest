...
void Main(){
	...
	rep(s,1,1<<20){
		vvl h(4,vl(5,0));
		rep(i,20)h[i/5][i%5]=(s>>i)&1;
		bool ok=true;
		vl total(4,0);
		rep(i,4)total[i]=sum(h[i]);
		rep(i,4)if(total[i]%2!=0)ok=false;
		if(!ok)continue;
		vvl f(4,vl(4,0));
		rep(i,4){
			int t=h[i][0];
		...
		rep(i,4)rep(j,4)if(a[i][j] and !f[i][j]) ok=false;
		if(!ok)continue;
		{
			vvl vis(6,vl(6,0));
			queue<pll> q;
			rep(i,4)rep(j,4)if(f[i][j]){
				q.emplace(i,j);
				i=j=4;
				break;
			}
			while(!q.empty()){
				...
				rep(v,4){
					...
					if(f[ny][nx] and !vis[ny+1][nx+1])q.emplace(ny,nx);
				}
			}

			rep(i,4)rep(j,4)if(f[i][j] and !vis[i+1][j+1])ok=false;
			if(!ok)continue;

			rep(i,3)rep(j,3){
				if(f[i][j] and !f[i+1][j] and !f[i][j+1] and f[i+1][j+1])ok=false;
				if(!f[i][j] and f[i+1][j] and f[i][j+1] and !f[i+1][j+1])ok=false;
			}
			if(!ok)continue;
			...
			while(!q.empty()){
				...
				rep(v,4){
					int ny=y+dy[v], nx=x+dx[v];
					if(ny<0 or nx<0 or ny>=6 or nx>=6)continue;
					if(!vis[ny][nx])q.emplace(ny,nx);
				}
			}
			rep(i,1,5)rep(j,1,5)if(!vis[i][j])ok=false;
			if(!ok)continue;
