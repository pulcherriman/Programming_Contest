...
int main() {
	...
	vl swaps(n,0); iota(all(swaps),0);
	rep(i,m){
		swap(swaps[a[i]-1],swaps[a[i]]);
	}
	vl mp(n);
	rep(i,n) mp[swaps[i]] = i;
	...
	rep(i,m){
		int tgt=a[i];
		int v1=ans[tgt-1]-1, v2=ans[tgt]-1;
		v1=a[i]-1, v2=a[i];
		swap(swaps[mp[v1]], swaps[mp[v2]]);
		swap(mp[v1], mp[v2]);
		out(mp[val]+1);
		if(ans[tgt-1]==1){
			val++;
