		...
		if(isGone[v]){
			...
			if(edge[v].size()==1 and v!=0){
				...
				for(auto&[to,cost]:edge[v]){
					if(to!=par[v]){
						ans+=max(dp0[to], dp1[to]);
						if(dp1[to]==-LINF)continue;
						ll toku=dp1[to]+cost - max(dp0[to], dp1[to]);
						if(toku>0) w.emplace_back(toku);
					}
				}
				if(w.empty() or d[v]==0){
					dp0[v]=dp1[v]=ans;
				...
				dp0[v] = ans + w[min(d[v], sz(w))-1];
				debug(d[v], sz(w), min(d[v]-1, sz(w))-1)<<flush;
				if(v!=0 and d[v]>1) dp1[v] = ans + w[min(d[v]-1, sz(w))-1];
				if(v!=0 and d[v]==1) dp1[v] = ans;
			}
			debug("=>");
			debug(dp0);
			debug(dp1);
