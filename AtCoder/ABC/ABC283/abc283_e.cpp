...
int main() {
	...
	def(ll,h,w);
	vvl a(h,vl(w)); in(a);
	vvl dp(h+1, vl(4, LINF));

	vvb isolate(h,vb(w,false));
	rep(i,h)rep(j,w){
		bool b=true;
		if(j>0 and a[i][j-1]==a[i][j]) b=false;
		if(j<w-1 and a[i][j+1]==a[i][j]) b=false;
		isolate[i][j]=b;
	}
	...
	rep(a0,2)rep(a1,2){
		bool ok=true;
		rep(j,w){
			if(isolate[0][j] and (a[0][j]^a0) != (a[1][j]^a1)) ok=false;
		}
		if(ok)dp[2][a0*2+a1]=a0+a1;
	}
	debug(dp[2]);
	rep(i,2,h)rep(k,4)if(dp[i][k]<LINF){
		bool ap = k%2==1;
		bool app = k/2==1;
		rep(an,2){
			bool ok=true;
			rep(j,w){
				if(isolate[i-1][j] and (a[i-2][j]^app) != (a[i-1][j]^ap) and  (a[i-1][j]^ap)!=(a[i][j]^an)) ok=false;
				if(i+1==h and isolate[i][j] and (a[i-1][j]^ap) != (a[i][j]^an)) ok=false;
			}
			if(ok)chmin(dp[i+1][ap*2+an], dp[i][k]+an);
		}
	}
