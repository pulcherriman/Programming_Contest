...
inline int hashize(int sy, int sx, int ty, int tx) {
	return (sy*w+sx+1)*h*w+(ty*w+tx);
}

// memo[hash][cutCount][minVal] = minimize(maxVal)
vb visited(10000, false);
vector<vector<HashMap<ll,ll>>> memo;
ll solve(int sy, int sx, int ty, int tx) {
	int hsh = hashize(sy,sx,ty,tx);
	if(visited[hsh]) return 0;
	visited[hsh] = true;

	debug("CALL", sy,sx,ty,tx);

	if(sy==ty&&sx==tx) {
		memo[hsh][0][ichigo[sy][sx]] = ichigo[sy][sx];
		return 0;
	}
	// kiranai
	{
		ll val=0;
		rep(i,sy,ty+1)rep(j,sx,tx+1)val+=ichigo[i][j];
		memo[hsh][0][val] = val;
	}

	// yokonikiru 
	rep(cy,sy+1, ty+1){
		int hsh1 = hashize(sy,sx,cy-1,tx);
		int hsh2 = hashize(cy,sx,ty,tx);
		// debug("!",hsh,"->", hsh1, hsh2);
		solve(sy,sx,cy-1,tx);
		solve(cy,sx,ty,tx);
		rep(cut, 1, t+1){
			rep(cut1, 0, cut){
				int cut2 = cut - cut1-1;
				// debug(cut, cut1, cut2);
				for(auto&[minVal1, maxVal1]:memo[hsh1][cut1]){
					for(auto&[minVal2, maxVal2]:memo[hsh2][cut2]){
						ll minVal = min(minVal1, minVal2);
						ll maxVal = max(maxVal1, maxVal2);
						if(memo[hsh][cut].find(minVal)==memo[hsh][cut].end())
							memo[hsh][cut][minVal] = maxVal;
						else
							chmin(memo[hsh][cut][minVal], maxVal);
					}
				}
			}
		}
	...
	// tatekiru
	rep(cx,sx+1, tx+1){
		int hsh1 = hashize(sy,sx,ty,cx-1);
		int hsh2 = hashize(sy,cx,ty,tx);
		// debug("!",hsh,"->", hsh1, hsh2);
		solve(sy,sx,ty,cx-1);
		solve(sy,cx,ty,tx);
		rep(cut, 1, t+1){
...
int main() {
	/*$1*/
	cin>> h>> w>> t;
	ichigo.resize(h,vl(w));
	in(ichigo);
	memo = vector<vector<HashMap<ll,ll>>>(2000, vector<HashMap<ll,ll>>(t+1));

	// rep(i,h)rep(j,w){
	// 	rep(k,i,h)rep(l,j,w){
	// 		out(i,j,"->",k,l, hashize(i,j,k,l));
	// 	}
	// }
	...
	ll ans = LINF;
	for(auto&[minVal, maxVal]:memo[hashize(0,0,h-1,w-1)][t]){
		chmin(ans, maxVal-minVal);
	}
	out(ans);
}