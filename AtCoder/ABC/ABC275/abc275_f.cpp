...
int main() {
	...
	vector<vvl> dp(3, vvl(n+1,vl(m+1,LINF)));
	dp[0][0][0] = 0;
	rep(i,n) {
		rep(j,m+1) {
			if(dp[0][i][j] != LINF) { //区間が閉じていて、開くことができる
				chmin(dp[1][i+1][j], dp[0][i][j]+1); //空ける
				chmin(dp[2][i+1][j], dp[0][i][j]+1); //空けて、長さ1で閉じる
				if(j+a[i] <=m) chmin(dp[0][i+1][j+a[i]], dp[0][i][j]); //空けない
			}
			if(dp[1][i][j] != LINF) {//区間が開いている
				chmin(dp[1][i+1][j], dp[1][i][j]);//あけっぱなし
				chmin(dp[2][i+1][j], dp[1][i][j]); //閉じる
			}
			if(dp[2][i][j] != LINF) {//区間が開いていて、空けることができない
				if(j+a[i] <=m) chmin(dp[0][i+1][j+a[i]], dp[2][i][j]); //空けられるようになる
			}
		}
	}
	rep(j,1,m+1){
		ll val = min({dp[0][n][j], dp[1][n][j], dp[2][n][j]});
		if(val==LINF)val=-1;
		out(val);
	}
