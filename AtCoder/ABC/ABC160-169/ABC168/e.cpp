...
int main(){
    ...
    for(auto&d:p){
        if(d.sc<0)continue;
        ll a=d.fs.sc, b=d.fs.fs;
        if(a==0 and b!=0){az+=d.sc;continue;}
        if(b==0 and a!=0){bz+=d.sc;continue;}
        if(a==0 and b==0){zz+=d.sc;continue;}
        if(b<0){b*=-1;a*=-1;}
        a*=-1;
        if(p.find(pll{a,b})!=p.end()){
            ans*=(2_m^d.sc)+(2_m^p[pll{a,b}])-1;
            d.sc*=-1;
            p[pll{a,b}]*=-1;
        }
    }
