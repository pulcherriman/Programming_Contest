...
int main(){
    ...
    auto solve_small_count=[&](){
        ...
        rep(s,1<<(n-n/2)){
            ...
            b.emplace_back(wc,vc);
        }
        b.emplace_back(LINF,0);
        sort(all(b));
        rep(i,b.size()-1){
        ...
        for(auto [wc,vc]:t){
            ll wleft=wmax-wc;
            if(wleft<0)continue;
            auto [wtgt, vtgt]=*(--lower_bound(all(b),pll{wleft+1,0}));
            chmax(ans, vc+vtgt);
        }
        return ans;
    };
    auto solve_small_weight=[&](){
        ll WMAX=200000;
        vvl dp(n+1,vl(WMAX+1,0));   //i番目まで見て重さj以下で達成できる最高
        rep(i,n){
            rep(j,WMAX){
                chmax(dp[i+1][j],dp[i][j]);
                if(j+w[i]<=wmax)chmax(dp[i+1][j+w[i]],dp[i][j]+v[i]);
            }
        }
        return max(dp[n]);
    };
    auto solve_small_value=[&](){
        ll VMAX=200000;
        vvl dp(n+1,vl(VMAX+1,LINF));   //i番目まで見て価値jで達成できる最小の重さ
        dp[0][0]=0;
        rep(i,n){
            rep(j,VMAX){
        ...
        rrep(i,VMAX+1){
            if(dp[n][i]<=wmax)return i;
        }
        return 0ll;
    };

    if(n<=30){
        puta(solve_small_count());
    }else if(max(w)<=1000){
        puta(solve_small_weight());
    }else{
        puta(solve_small_value());
    }
