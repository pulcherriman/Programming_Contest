...
int main() {
	/*$1*/
	def(ll,n,m);
	vvl edge(m, vl(3)); vector<vector<tl3>> con(n);
	rep(i,m){
		cin>>edge[i];
		con[edge[i][0]-1].emplace_back(edge[i][1]-1,edge[i][2],i+1);
		con[edge[i][1]-1].emplace_back(edge[i][0]-1,edge[i][2],i+1);
	}
	vl dist(n,LINF);
	vb vis(n,0);
	dist[0]=0;
	...
	while(!q.empty()){
		auto [w,p,use]=q.top(); q.pop();
		if(vis[p])continue;
		if(use!=-1)ans.emplace_back(use);
		vis[p]=1;
		for(auto&[to,cost,id]:con[p]){
			if(!vis[to] and dist[to]>w+cost){
				dist[to]=w+cost;
				q.emplace(w+cost,to,id);
			}
		}
	}
	out(ans);
