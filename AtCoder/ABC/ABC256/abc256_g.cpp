...
int main() {
	...
	def(ll,n,d);
	vm w(d,0);
	rep(i,d)w[i]=nCr(d-1,i);


	mymint ans=0;
	rep(white,d+2){
		Matrix<mymint> m(2,2);
		m[0][0] = white<=d-1 ? w[white] : 0;
		m[0][1] = white>=1 and white-1<=d-1 ? w[white-1] : 0;
		m[1][0] = white>=1 and white-1<=d-1 ? w[white-1] : 0;
		m[1][1] = white>=2 and white-2<=d-1 ? w[white-2] : 0;
		m=m.pow(n);
