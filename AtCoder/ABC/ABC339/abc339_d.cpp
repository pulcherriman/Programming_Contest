...
int main() { /**/
	...
	rep(i,n)rep(j,n)if(f[i][j]=='P') {
		if(y1==-1) y1=i,x1=j;
		else y2=i,x2=j;
	}
	auto solve=[&n, &f](int y1, int x1, int y2, int x2){

		queue<array<ll,9>> q;
		q.push({y1,x1,y2,x2,y1,x1,y2,x2,0});
		vl dist(n*n*n*n, LINF);
		while(!q.empty()){
			auto [py1,px1,py2,px2,y1,x1,y2,x2,d]=q.front(); q.pop();
			y1=clamp(y1,0ll,n-1);
			x1=clamp(x1,0ll,n-1);
			y2=clamp(y2,0ll,n-1);
			x2=clamp(x2,0ll,n-1);
			...
			int enc = y1*n*n*n+x1*n*n+y2*n+x2;
			if(dist[enc]!=LINF) continue;
			dist[enc]=d;

			q.push({y1,x1,y2,x2,y1+1,x1,y2+1,x2,d+1});
			q.push({y1,x1,y2,x2,y1-1,x1,y2-1,x2,d+1});
			q.push({y1,x1,y2,x2,y1,x1+1,y2,x2+1,d+1});
			q.push({y1,x1,y2,x2,y1,x1-1,y2,x2-1,d+1});
		}
		return dist;
	};
	...
	auto dist = solve(y1,x1,y2,x2);
	ll ans = LINF;
	rep(i,n)rep(j,n) {
		chmin(ans, dist[i*n*n*n+j*n*n+i*n+j]);
	}
	out(ans==LINF?-1:ans);
}