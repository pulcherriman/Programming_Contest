...
void Main(){
	auto[n,d]=Graph<ll>::input(true, true);
	n=400;
	d=Graph(n);
	rep(i,n)rep(j,n)d.add(i,j,Random(1,1000000), true);

	vvl adj(n,vl(n,LINF));
	rep(i,n)for(auto [j,c]:d.edge[i])adj[i][j]=c;
	...
	rep(k,n){
		rep(s,n){
			...
			while(!q.empty()){
				...
				if(r[p]!=LINF)continue;
				r[p]=c;
				if(p!=s and p>k)continue;
				rep(nxt,n)if(r[nxt]==LINF and adj[p][nxt]!=LINF){
					q.emplace(c+adj[p][nxt], nxt);
				}
			}
			rep(t,n)if(r[t]!=LINF)ans+=r[t];
		}
	}
	puta(ans);
}
