...
vvi getPredict2() {
	...
	rep(i,beforeSize)rep(j,beforeSize){
		compressed[i][j] = decode(compressed[i][j]);
	}
	// debug(compressed);
	...
	rep(i,afterSize)rep(j,afterSize){
		...
		int by = upper_bound(all(indices), i) - indices.begin();
		int bx = upper_bound(all(indices), j) - indices.begin();
		...
		// out("=========");
		// out(i,j, y, x, by, bx);
		rep(ny, by-3, by+3)rep(nx, bx-3, bx+3){
			if(ny<0 || ny>=beforeSize || nx<0 || nx>=beforeSize)continue;
			ld nypos = indices[ny] / (ld)afterSize * (beforeSize-1);
			ld nxpos = indices[nx] / (ld)afterSize * (beforeSize-1);
			ld d = sqrt((nypos-y)*(nypos-y) + (nxpos-x)*(nxpos-x));
			predict_raw[i][j] += compressed[ny][nx] * weightByDistance(d);
			// out("   to:", ny,nx,nxpos,nypos,":",d, compressed[ny][nx] * weightByDistance(d));
		}
		// out("= predict ========");
		// out(predict_raw[i][j]);
		// out("");
	...
	rep(i,afterSize)rep(j,afterSize){
		predict[i][j] = round((predict_raw[i][j] - minValue)  * (5000-10) / (maxValue - minValue) + 10);
