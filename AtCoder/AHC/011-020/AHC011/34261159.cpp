...
void display(Field&f){
	...
	// // }
	// rep(i,n){
	// 	rep(j,n) cerr << right << setw(2) << hex << f.get(i,j);
	// 	cerr<<dec;
	// 	debug_f("");
	// }
}

void displayLine(ostream&os, ll tiles){
	// if (!_TEMPORARY_DEBUG) return;
	// const static int d[3][3]={
	// 	{0,2,0},
	// 	{1,15,4},
	// 	{0,8,0}
	// };
	// rep(i,n)rep(w,4)if(w==0)cout<<'+'; else cout<<"--";
	// cout<<"+\n";
	// rep(h,3){
	// 	rep(i,n){
	// 		cout<<'|';
	// 		rep(w,3){
	// 			if((tiles>>(i*4)) & d[h][w])os<<"##";
	// 			else os<<"  ";
	// 		}
	// 	}
	// 	os<<"|\n";
...
struct AnswerObserver {
	...
	void inject(Field&f) { answer = from = f; }
	void createMove(Field mst){
		challenge++;
...
//		debug("To...");
		display(mst);
		try {
			if(solveAtoB(fCopy, mst)){
	//			debug(challenge, "SUCCESS!!!");
				if(!isSuccess or answer.mvLength > fCopy.mvLength){
					isSuccess = true;
					answer = fCopy;
				}
			}else{
	//			debug(challenge, "FAILED....");
				if(!isSuccess){
					if(chmax(maxScore, calculate(fCopy))){
						answer = fCopy;
					}
				}
			}
...
void findMST(Field&f) {
	int calcCount=0;
	TileZaiko masterTileCount;
	masterTileCount.fill(0);
	rep(i,n)rep(j,n) masterTileCount[f.get(i,j)]++;
	...
	try {
		...
		while(!q.empty()){
			...
			TileZaiko zaiko = masterTileCount;
			rep(i,cur.nextLineIndex)rep(j,n)zaiko[cur.get(i,j)]--;
			if(cur.nextLineIndex+1==n){
				zaiko[0]=1; // (H):0の位置を最下段に固定
			}else{
				zaiko[0]=0; // (H):0の位置を最下段に固定
			}
			...
			for(auto&bottom:(cur.nextLineIndex+1==n)?vi(1,0):bottoms){
				...
				for(auto&placedTiles: result){
					...
					if(cur.nextLineIndex+1==n){
						// 発見！
						if(nxt.get(n-1,n-1) == 0) {
							answerObserver.createMove(nxt);
						}
						continue;
					}
...
//				debug(distMap);
	...
	void moveTile(int tilePos, int targetPos) {
...
//		debug("  decide finish :", "move to", decode(targetPos), "from", decode(minTilePos), "=", minDist);
	...
	void show(){
		...
		// rep(i,n){
		// 	rep(j,n){
		// 		if(isFixed(i,j)) cerr<<"#"<<hex<<f.get(i,j)<<dec<<"#";
		// 		else if(kouho.find(encode(i,j)) != kouho.end()) cerr<<"["<<hex<<f.get(i,j)<<dec<<"]";
		// 		else cerr<<" "<<hex<<f.get(i,j)<<dec<<" ";
		// 	}
		// 	if(i==cRow) cerr<<" <";
		// 	cerr<<"\n";
		// }
		// rep(i,n)if(cColumn==i)cerr<<" ^ "; else cerr<<"   ";
		// cerr<<"\n";
	}

	bool Last2x2(){
//		debug("LAST 2x2!");
		if(f.by+1!=n)f.move(DIR::DOWN);
		if(f.bx+1!=n)f.move(DIR::RIGHT);
		vl to={target.get(n-2,n-2), target.get(n-2,n-1), target.get(n-1,n-2)};
		vl from={f.get(n-2,n-2), f.get(n-2,n-1), f.get(n-1,n-2)};
...
//				debug("Special Operation (Column):", decode(p[0]), "and", decode(p[1]));
//				debug(cRow, cColumn);
				display(target);
				show();
//				debug("  Step 1");
...
//					debug("  tsumi.");
					...
					vector<DIR> dirs=side1 ? vector<DIR>{
...
//				debug("  Step 3");
		...
		{ // 行
			...
			if(cnt==2 and p[0] != encode(n-2, n-2)){
//				debug("Special Operation (Row):", decode(p[0]), "and", decode(p[1]));
//				debug(cRow, cColumn);
				display(target);
				show();
				// p[1](右)の位置にp[0]に置きたいものを持ってくる
...
void solve(Field&f){
	findMST(f);
	out(answerObserver.answer.getString());
	debug_f("found:", answerObserver.challenge, " n =",n);
	debug_f(answerObserver.answer.getString().size());
}
