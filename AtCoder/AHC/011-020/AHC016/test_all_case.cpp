...
struct StrategyBase {
	...
	virtual void CreateGraphs(int DEF_N=100) = 0;
...
struct NoErrorStrategy : public StrategyBase {
	...
	void CreateGraphs(int DEF_N=100) override {
		if(M <= 11) N = 4;
		else if(M <= 34) N = 5;
		else N = 6;
		GraphEnumerate(N);
...
struct EdgeCountStrategy : public StrategyBase {
	...
	void CreateConstTable(int n){
		...
		comb_map = vector<vector<long double>>(n+1, vector<long double>(n+1, 0));
		pow_damage_map = vector<long double>(n+1, 1);
		pow_not_damage_map = vector<long double>(n+1, 1);
	...
	vector<vector<pair<int, long double>>> PREDICT_TABLE;
	// n頂点と決め打った時の正解数の期待値
	long double getExpectedCount(int n){
		...
		vector<vector<pair<int, long double>>> predict_table(edge_count+1);
		rep(j, edge_count+1){
			long double prob_sum=0, prob_max=0; int prob_index=0;
			rep(i,M){
				prob_sum += probArray[i][j];
				predict_table[j].push_back({i, probArray[i][j]});
			}
			rep(i,M){
				predict_table[j][i].second /= prob_sum;
			}
			sort(all(predict_table[j]), [](const pair<int, long double> &a, const pair<int, long double> &b){return a.second > b.second;});
		}

		// expected_count[i] = 正解にグラフiが使われたときの正解確率
		vector<long double> expected_table(M, 0);
		rep(j, edge_count+1){
			auto [predicted, _] = predict_table[j][0];
	...
	int PredictGraph(Graph&damaged) override {
		return PREDICT_TABLE[damaged.getEdgeCount()][0].first;
	}
	pair<int,long double> PredictInfo(Graph&damaged, int deletion) {
		rep(i,M){
			if(PREDICT_TABLE[damaged.getEdgeCount()][i].first >= deletion){
				return PREDICT_TABLE[damaged.getEdgeCount()][i];
			}
		}
	}
};
...
struct DualStrategy : public StrategyBase {
	...
	void CreateGraphs(int DEF_N=100) override {
		...
		chmin(FRONT_DELETION, siz(graphs));
	...
	int PredictGraph(Graph&damaged) override {
		if(!dual2.graphs.empty()){
			auto [predicted, prob] = dual2.PredictInfo(damaged, FRONT_DELETION);
			if(prob >= 2.0f / M) return Pattern1 + predicted - FRONT_DELETION;
		}
		{// Pattern 1
...
map<tuple<string, int>, pair<int, StrategyBase*>> strategyMap;

template<class T>
void RunStrategy(int m, int e, int n) {
	cerr<<"Selected: "<<typeid(T).name()<<endl;
	StrategyBase* strategy = strategyMap[{typeid(T).name(), n}].second;
...
void NoError(int m, int e, int n=100){
	RunStrategy<NoErrorStrategy>(m, e, n);
}

void EdgeCount(int m, int e, int n=100){
	RunStrategy<EdgeCountStrategy>(m, e, n);
}

void FailSafe(int m, int e, int n=100){
	RunStrategy<FailSafeStrategy>(m, e, n);
}

void Dual(int m, int e, int n=100){
	RunStrategy<DualStrategy>(m, e, n);
}

template<class T>
int TestStrategy(int m, int e, int n=100, int repeat=10) {
	T* strategy = new T(m, e);
	strategy->CreateGraphs(n);
	int score = strategy->Test(repeat);
	cerr<<"Test: "<<typeid(T).name()<<" -> "<<score<<endl;

	auto key = make_tuple(typeid(T).name(), n);
	if(!strategyMap.count(key) || strategyMap[key].first < score){
		strategyMap[key] = make_pair(score, strategy);
	}
	return strategyMap[key].first;
}


vector<pair<int, int>> results;
int TestDual(int m, int e, int n){
	if(Mgr::TLE >= Mgr::timer.get()+n*12){
		results.emplace_back(TestStrategy<DualStrategy>(m, e, n), n);
...
char SelectBest(int m, int e) {
	// results.emplace_back(TestStrategy<FailSafeStrategy>(m, e), 0);
	// results.emplace_back(TestStrategy<NoErrorStrategy>(m, e), 1);
	// results.emplace_back(TestStrategy<EdgeCountStrategy>(m, e), 2);

	vi isTry(100-4+1); iota(all(isTry), 4); shuffle(begin(isTry), end(isTry)-1, Random);
	reverse(all(isTry));
	rep(1){
		for(auto&i:isTry)if(i){
			if(TestDual(m, e, i) < 6640) i = 0;
		}
	}
...
int main() {
	...
	rrep(e,0,41)rep(m,10,101){
		Mgr::timer = Timer(4600-100);
		EdgeCountStrategy::calced_e = -1;
		EdgeCountStrategy::calced_n = -1;
		results.clear();
		strategyMap.clear();

		int n = SelectBest(m,e);
		int score = TestStrategy<DualStrategy>(m, e, n);
		if(score<6640) break;
		out(m,e,n,-1,score,-1);
	}

	// int strategy = SelectBest(M,E);

	// switch(strategy){
	// 	case 0: FailSafe(M,E); break;
	// 	case 1: NoError(M,E); break;
	// 	case 2: EdgeCount(M,E); break;
	// 	default: Dual(M,E,strategy); break;
	// }
	return 0;
}