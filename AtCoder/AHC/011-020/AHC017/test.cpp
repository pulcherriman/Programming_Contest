...
template<typename T> struct floyd_warshall_naive {
	using value_t = T;
	static constexpr T INF = 1000000000;
...
float calcScoreOfDay(int day, vi&constructed) {
	WarshallFloyd<int32_t> wf(N);
	rep(j,M)if(constructed[j] != day+1){
	...
	float total = 0;
	rep(j,N)rep(k,j+1,N) total += dist[j][k] - initDist[j][k];
...
void initialize() {
	...
	rep(i,M){
		def(int,a,b,w);
		AdjList[a-1].push_back({b-1,w,i});
		AdjList[b-1].push_back({a-1,w,i});
		Edges[i] = {a-1,b-1,w};
...
tuple<ll, vi, vf> solve() {
	...
	double result = 0;
	rep(i,D){
		result += scores[i] = calcScoreOfDay(i, constructed);
	}

	return {round(1e3 * result / D), constructed, scores};
}
...
int main() {
	...
	rep(turn,D){
		...
		int eCount = M/D + (M-M/D*D > turn);
		debug("Start turn", turn+1);

		vb locked(M,false);
		{
			vector<tuple<int, int, vi>> diffs(M);

			rep(i,M)if(constructed[i] == 0 and !locked[i]){
				//辺i=(s,t)を使わないときの(s,t)間の距離
				const auto [s,t,w] = Edges[i];
				...
				while(!pq.empty()){
					...
					dist[v] = d;
					if(v == t) break;
					for(auto [u, w, j]: AdjList[v]){
						if(j == i) continue;
						if(constructed[j]==turn+1)continue; 
				...
				vi second_path;
				second_path.push_back(t);
				int cur = t;
				while(cur != s){
					for(auto [u, w, j]: AdjList[cur]){
						if(j == i) continue;
						if(constructed[j]==turn+1)continue; 
						if(dist[u] + w == dist[cur]){
							second_path.push_back(u);
							cur = u;
							break;
						}
					}
				}
				diffs[i] = {dist[t]-w, i, second_path};
			}
			sort(rall(diffs));
			rep(i,eCount/2){
				auto [diff, index, second_path] = diffs[i];
				constructed[index] = turn+1;
				constructCount++;

				rep(j,sz(second_path)-1) {
					locked[AdjMatrixToIndex[second_path[j]][second_path[j+1]]] = true;
				}
			}
		}

		while(eCount > constructCount){
			vi second_path;
			int minDiff = INF, edgeIndex = -1;
			...
			rep(i,M)if(constructed[i] == 0 and !locked[i]){
				...
				// out(i, dist[t]-w, dist[t], w);
				// out("dijkstra end", i);

				if(dist[t]-w < 0){
					constructed[edgeIndex] = turn+1;
					constructCount++;
					rep(j,sz(second_path)-1) {
						locked[AdjMatrixToIndex[second_path[j]][second_path[j+1]]] = true;
					}
					if(constructCount == eCount) break;
					continue;
				}

				if(chmin(minDiff, dist[t]-w)) {
					edgeIndex = i;
					second_path.clear();
					second_path.push_back(t);
					int cur = t;
			...
			rep(i,sz(second_path)-1) {
				locked[AdjMatrixToIndex[second_path[i]][second_path[i+1]]] = true;
			}
			// out("Shortest");
			// out(edgeIndex, minDiff);
			// out(Edges[edgeIndex]);
			// out(second_path);
		}
		if(eCount > constructCount){
			rep(i,M)if(constructed[i] == 0){
				constructed[i] = turn+1;
				constructCount++;
				if(constructCount == eCount) break;
			}
		}
	...
	vf scores(D);
	rep(i,D){
		scores[i] = calcScoreOfDay(i, constructed);
	}
	ll score = round(1e3 * sum(scores) /D);

	int succeeded = 0, failed = 0;
	while(Mgr::timer){
		int a = -1, b = -1;
		{
			double maxScore = -INF; int maxDay=0;
			rep(i,D)if(chmax(maxScore, scores[i])){
				maxDay = i;
			}
			do{
				rep(i,Random(0,M), M)if(constructed[i]==maxDay+1){
					a=i;
					break;
				}
			}while(a==-1);
		...
		do {
			b=Random(0,M);
		}while(constructed[a]==constructed[b]);

		swap(constructed[a], constructed[b]);
		float oldScore = scores[constructed[a]-1] + scores[constructed[b]-1];
		float newScoreA = calcScoreOfDay(constructed[a]-1, constructed);
		float newScoreB = calcScoreOfDay(constructed[b]-1, constructed);
		float newScore = newScoreA + newScoreB;
		if(newScore < oldScore){
			score = round((score*D/1e3 - oldScore + newScore)/D*1e3);
			scores[constructed[a]-1] = newScoreA;
			scores[constructed[b]-1] = newScoreB;
			debug("Upgraded:", oldScore, "->", newScore,"(",oldScore-newScore,")");
			succeeded++;
		}else{
			swap(constructed[a], constructed[b]);
			failed++;
	...
	debug(succeeded, failed);
	debug(score);
	out(constructed)<<flush;
	return 0;

	// auto [score, constructed, scores] = solve();
	// while(Mgr::timer.get() <= 1500){
	// 	auto [_score, _constructed, _scores] = solve();
	// 	if(chmin(score, _score)){
	...
	// 	int a = 0, b = 0;
	// 	do {
	// 		a=Random(0,M), b=Random(0,M);
	// 	}while(a==b or constructed[a]==constructed[b]);
