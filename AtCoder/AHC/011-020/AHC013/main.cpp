...
vvb edge;
pair<int, vector<ConnectKey>> createTree(char c='1', int remains=0){
	int score=0;
	vector<ConnectKey> ret;
	UnionFind uf((n+2)*(n+2));

	rep(ii,1,n+1){
		int i=(n/k*(c-'1')+ii-1)%n+1;
		// i=ii;
		rep(j,1,n+1){
			if(field[i][j]==c){
				int xMax=j;
...
pair<int, int> calcConnectivity(string numbers, int moved=0){
	rep(i,1,n+1)rep(j,1,n+1)edge[i][j]=false;
	int score=0, conSize=0, remains=100*k-moved;

	for(auto&c:numbers){
		auto[sc, con]=createTree(c, remains);
		score+=sc;
		conSize+=con.size();
		remains-=con.size();
	}
...
pair<int, vector<ConnectKey>> getConnectivity(string numbers, int moved=0){
	rep(i,1,n+1)rep(j,1,n+1)edge[i][j]=false;

	int score=0, remains=100*k-moved;
	vector<ConnectKey> connect;
	connect.reserve(remains);
...
tuple<int, vector<ConnectKey>, vector<ConnectKey>> yakinamashi(int initScore){
		...
		rep(i,num.size()){
			...
			for(auto&v:dir){
				...
				rep(len,1,maxLen+1) {
					...
					auto[nxtScore, nxtConSize]=calcConnectivity(numbers, moves.size()+len);
					...
					if(score+threshold < nxtScore 
						or (conSize > nxtConSize+len and score-2 <= nxtScore)){
						debug(Mgr::timer.get(), test, score,"(",i,field[ny][nx],") =>",nxtScore-score);
						score=nxtScore;
						rep(d,len){
	...
	vector<ConnectKey> connect;
	tie(score, connect)=getConnectivity(numbers, moves.size());
