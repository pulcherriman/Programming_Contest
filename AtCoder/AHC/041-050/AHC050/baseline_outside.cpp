...
int main() { /**/
	...
	sort(all(ans), [&N](const pll&l, const pll&r) {
		int distOfCenterL = abs(l.first - N/2) + abs(l.second - N/2);
		int distOfCenterR = abs(r.first - N/2) + abs(r.second - N/2);
		if(distOfCenterL != distOfCenterR) return distOfCenterL > distOfCenterR;
