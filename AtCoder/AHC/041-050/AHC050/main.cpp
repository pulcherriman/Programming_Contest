...
// lib.rsと完全に同じスコア計算アルゴリズム
vector<pii> strategy_probability_based(const vs& field, int N, int M) {
	vector<pii> result;
...
int main() { /**/
	...
	vector<pii> ans;
	// 確率ベース戦略を使用
	ans = strategy_probability_based(field, N, M);
