...
// lib.rsと同じロボット確率分布計算
vvd simulateRobotProbability(const vs& field, int N) {
	vvd prob(N, vd(N, 0.0));
...
// 確率フロー解析: 将来k回の移動後の確率分布を予測
vvd predictProbabilityFlow(const vs& field, int N, int k) {
	vvd prob(N, vd(N, 0.0));
...
// フロー重要度の計算: 確率の流れの収束性を測定
double calculateFlowImportance(const pii& pos, const vs& field, int N, int lookaheadSteps) {
	vs testField = field;
	auto [r, c] = pos;
	testField[r][c] = '#';
	
	// 元の場合と岩を置いた場合の確率分布の差を測定
	vvd originalFlow = predictProbabilityFlow(field, N, lookaheadSteps);
	vvd modifiedFlow = predictProbabilityFlow(testField, N, lookaheadSteps);
	
	double importance = 0.0;
	rep(i, N) rep(j, N) {
		if(field[i][j] == '.') {
			double diff = abs(originalFlow[i][j] - modifiedFlow[i][j]);
			importance += diff;
		}
	}
...
// アトラクター解析: 確率が集中する点を検出
vector<pii> findProbabilityAttractors(const vs& field, int N, int lookaheadSteps) {
	vvd futureProb = predictProbabilityFlow(field, N, lookaheadSteps);
	
	vector<pair<double, pii>> candidates;
	rep(i, N) rep(j, N) {
		if(field[i][j] == '.') {
			candidates.push_back({futureProb[i][j], {i, j}});
		}
	}
	
	sort(rall(candidates)); // 確率の高い順
	
	vector<pii> attractors;
	rep(i, min(10, (int)candidates.size())) {
		attractors.push_back(candidates[i].second);
	}
	
	return attractors;
}

// フロー効率性の計算: 岩の配置がフローに与える効果を評価
double calculateFlowEfficiency(const pii& pos, const vs& field, const vvd& currentProb, int N) {
	auto [r, c] = pos;
	double hitProb = currentProb[r][c];
	
	// 基本スコア（main.cppと同じ）
	double baseScore = hitProb;
	
	// フロー遮断効果: この位置に岩を置くことで、将来の確率集中をどれだけ阻害できるか
	double flowDisruption = calculateFlowImportance(pos, field, N, 3);
	
	// 周辺影響度: 周辺のマスへの確率流入をどれだけ変化させるか
	double neighborInfluence = 0.0;
	vector<pii> dirs = {{-1,0}, {1,0}, {0,-1}, {0,1}};
	for(auto [di, dj] : dirs) {
		int ni = r + di, nj = c + dj;
		if(ni >= 0 && ni < N && nj >= 0 && nj < N && field[ni][nj] == '.') {
			neighborInfluence += currentProb[ni][nj];
		}
	}
	
	// 統合評価関数
	return baseScore + flowDisruption * 0.1 + neighborInfluence * 0.05;
}

// 確率フロー解析ベースの戦略
vector<pii> strategy_flow_analysis(const vs& field, int N, int M) {
	vector<pii> result;
	...
	while(true) {
		...
		rep(i, N) rep(j, N) {
			if(currentField[i][j] == '.') {
				double score = calculateFlowEfficiency({i, j}, currentField, prob, N);
				if(score > bestScore) {
					bestScore = score;
					bestPos = {i, j};
				}
		...
		// アトラクター分析結果をデバッグ出力
		if(turn % 10 == 0) {
			auto attractors = findProbabilityAttractors(currentField, N, 3);
			debug("Turn:", turn, "Attractors:", attractors.size(), 
				  "FlowScore:", bestScore);
		}
...
int main() {
	...
	vector<pii> ans;
	// 確率フロー解析戦略を使用
	ans = strategy_flow_analysis(field, N, M);
