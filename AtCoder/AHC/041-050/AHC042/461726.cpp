...
auto escapeVertical(int r, int vec, int dist) {
	int val = vec==1 ? N-1 : 0;
	vi oniMap(N,-1);
	rep(d,demons.size()) {
		if(demons[d].c==r && demons[d].alive) {
			oniMap[demons[d].r] = d;
		}
	}
	rep(dist) {
		if(oniv[r].contains(val)) {
			oniv[r].erase(val);
			onih[val].erase(r);
	...
	while(0<=val && val<N) {
		if(oniMap[val]!=-1) {
			...
			oniv[r].erase(val);
			oniv[r].insert(val+dist*vec);
			onih[val].erase(r);
			onih[val+dist*vec].insert(r);
		}else if(fukuv[r].contains(val)) {
			fukuv[r].erase(val);
			fukuv[r].insert(val+dist*vec);
			fukuh[val].erase(r);
			fukuh[val+dist*vec].insert(r);
		}
		val-=vec;
	}
}

auto escapeHorizontal(int c, int vec, int dist) {
	int val = vec==1 ? N-1 : 0;
	vi oniMap(N,-1);
	rep(d,demons.size()) {
		if(demons[d].r==c && demons[d].alive) {
			oniMap[demons[d].c] = d;
		}
	}
	...
	while(0<=val && val<N) {
		if(oniMap[val]!=-1) {
			...
			onih[c].erase(val);
			onih[c].insert(val+dist*vec);
			oniv[val].erase(c);
			oniv[val+dist*vec].insert(c);
			// out("moved!", val, val+dist*vec);
		}else if(fukuh[c].contains(val)) {
			fukuh[c].erase(val);
			fukuh[c].insert(val+dist*vec);
			fukuv[val].erase(c);
			fukuv[val+dist*vec].insert(c);
		}
		val-=vec;
	}
}
...
int main() {
	...
	for(auto [idx, d, dist] : bestActions) {
		rep(dist){
			if(d%2==0) {
				out_f((d==0 ? 'U' : 'D'), idx);
			} else {
