...
const int MAX_PRECOMPUTE_TIME_MS = 800;  // 前処理制限時間（0.8秒に短縮）
int K, T, D;
...
public:
    void solve() {
        ...
        // H*2が概算の最小ターン数。制約が厳しい場合のみペナルティを適用
        int estimated_min_turns = targets.size() * 2;
        if (estimated_min_turns > T * 0.8) {
            turn_penalty_weight = D * 1.0;  // 簡単なペナルティ適用
            cerr << "Turn constraint detected, applying penalty weight: " << turn_penalty_weight << endl;
        }
