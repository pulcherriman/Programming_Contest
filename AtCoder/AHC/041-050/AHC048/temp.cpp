...
const int MAX_PRECOMPUTE_GRAMS = 8;  // 前処理で計算する最大グラム数
int K, T, D;
...
public:
    AdvancedPalette palette;
      void solve() {
        ...
        cerr << "Average per target: " << (double)main_duration.count() / _H << " ms" << endl;
        ...
        cerr << "Calls per target: " << (double)findClosestMix_calls / _H << endl;
    }
};
