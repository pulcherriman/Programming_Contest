...
const int MAX_PRECOMPUTE_TIME_MS = 1000;  // 前処理制限時間（1秒）
int K, T, D;
...
public:
    ...
    Action findBestAction(const Color& target, double turn_penalty_weight = 0.0) {
        Action bestAction;
        ...
        rep(wellId, GRID_ROWS * GRID_COLS) {
            ...
            for (int add_grams = 1; add_grams <= std::min(MAX_WELL_CAPACITY, actual_max_precompute_grams); add_grams++) {
                // 現在のウェル容量制限チェック
            ...
            if (!well.isEmpty()) {
                for (int dispose_grams = 1; dispose_grams <= well.currentGrams; dispose_grams++) {
                    ...
                    for (int add_grams = 1; add_grams <= std::min(MAX_WELL_CAPACITY - remaining_grams, actual_max_precompute_grams); add_grams++) {
                        // 廃棄後の理想の追加色ベクトルを計算
                        Color ideal_add_color = computeIdealAdditionColor(
...
struct SolverResult {
    ...
    bool withinTurnLimit;
    
    SolverResult() : totalTurns(0), totalScore(0), penalty_weight(0), withinTurnLimit(false) {}
};
...
public:
    ParameterizedSolver(double penalty_weight) : turn_penalty_weight(penalty_weight) {}
    
    // ドライラン（出力なしでシミュレーション）
    SolverResult simulateSolution() {
        SolverResult result;
        result.penalty_weight = turn_penalty_weight;
        
        AdvancedPalette test_palette;
        int current_turns = 0;
        
        rep(i, targets.size()) {
            AdvancedPalette::Action best_action = test_palette.findBestAction(targets[i], turn_penalty_weight);
            
            // シミュレーション内でのアクション実行（出力なし）
            if (best_action.wellId >= 0) {
                Well& well = test_palette.wells[best_action.wellId];
                
                // 事前廃棄
                if (best_action.disposalAmount > 0) {
                    well.dispose(best_action.disposalAmount);
                }
                
                // 絵の具を追加
                for (auto [tubeId, grams] : best_action.additions) {
                    well.addPaint(tubeId, grams);
                }
                ...
                current_turns += best_action.turnCount;
                result.totalScore += best_action.score;
                result.actions.push_back(best_action);
            }
        }
        
        result.totalTurns = current_turns;
        result.withinTurnLimit = (current_turns <= T);
    ...
    void outputSolution() {
        ...
        rep(i, targets.size()) {
            AdvancedPalette::Action bestAction = palette.findBestAction(targets[i], turn_penalty_weight);
...
public:
    void solve() {
        ...
        cerr << "Precomputation completed: " << (precompute_completed ? "Yes" : "No (time limit)") << endl;
        cerr << "Actual max precompute grams: " << actual_max_precompute_grams << endl;
        ...
        // 複数のペナルティ重みでシミュレーション
        vector<double> penalty_weights = {0.0, D * 0.1, D * 0.5, D * 1.0, D * 2.0, D * 5.0, D * 10.0};
        vector<SolverResult> results;
        
        cerr << "Testing multiple penalty weights..." << endl;
        
        for (double weight : penalty_weights) {
            ParameterizedSolver solver(weight);
            SolverResult result = solver.simulateSolution();
            results.push_back(result);
            
            cerr << "Weight " << weight << ": turns=" << result.totalTurns 
                 << "/" << T << ", score=" << result.totalScore 
                 << ", within_limit=" << (result.withinTurnLimit ? "Yes" : "No") << endl;
        }
        
        // 最適なペナルティ重みを選択
        SolverResult best_result;
        best_result.totalScore = 1e18;
        
        for (const auto& result : results) {
            if (result.withinTurnLimit && result.totalScore < best_result.totalScore) {
                best_result = result;
            }
        }
        
        // 制約を満たすものがない場合、最もターン数が少ないものを選択
        if (best_result.totalScore == 1e18) {
            int min_turns = 1e9;
            for (const auto& result : results) {
                if (result.totalTurns < min_turns) {
                    min_turns = result.totalTurns;
                    best_result = result;
                }
            }
            cerr << "Warning: No solution within turn limit. Using minimum turn solution." << endl;
        }
        
        cerr << "Selected penalty weight: " << best_result.penalty_weight << endl;
        cerr << "Final solution: turns=" << best_result.totalTurns 
             << "/" << T << ", score=" << best_result.totalScore << endl;
        
        // 最適なペナルティ重みで実際に実行
        auto start_main = chrono::high_resolution_clock::now();
        
        ParameterizedSolver final_solver(best_result.penalty_weight);
        final_solver.outputSolution();
