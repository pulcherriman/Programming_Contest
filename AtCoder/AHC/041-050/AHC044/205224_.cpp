...
int solve2(vi&t, vi&Tgt){
	...
	rep(turn,L){
		...
		if(cur==0 && !flag(vis, 0)) {
			...
			int remain = L - turn - 1;
			int loopReq = remain / loopTurn;
			rep(i,N) cnt[i] *= (loopReq+1);
			turn += loopTurn * loopReq;
		}
	}

	int res=0;
	rep(i,N) res+=abs(cnt[i]-Tgt[i]);
	return res;
}
...
int main() {
	...
	sort(T.rbegin()+1, T.rend());
	
	vi best; int bestScore = 1000000;
	int challenge=0;
	...
	while(Mgr::timer){
		...
		int tgt = Random(0, N);
		int prev = t[tgt];
		t[tgt] = Random(0, tgt+1);
		int score = solve2(t,T);
