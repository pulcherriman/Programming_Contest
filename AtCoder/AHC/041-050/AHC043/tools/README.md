...
rustup update
```
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc043/de43f43a9c_windows.zip).
...
- `-M=100` Fix the `M` value of the generated inputs to the specified one.
- `-K=15000` Fix the `K` value of the generated inputs to the specified one.
...
You can also use a [web visualizer](https://img.atcoder.jp/ahc043/de43f43a9c.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc043/de43f43a9c_windows.zip)も用意してあります。
...
- `--M=100` 生成される入力の `M` の値を指定されたものに固定
- `--K=15000` 生成される入力の `K` の値を指定されたものに固定
...
より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc043/de43f43a9c.html?lang=ja)も利用可能です。
