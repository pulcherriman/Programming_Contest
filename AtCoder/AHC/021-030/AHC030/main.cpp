...
bool isFieldContained(Field parent, Field&child) {
    for(size_t i = 0; i < parent.size(); ++i) {
        if(max(parent[i], (Cell)0) < child[i]) {
            return false;
        }
    }
    return true;
...
struct Plobrem: PlobremInterface {
	...
	struct Overlap {
		...
		bool decideNoExist(int idx) {
			if(sum[idx] == 0) return false;
			rrep(i, sz(states)) if(states[i][idx]) {
				removeAt(i);
				if(sum[idx] == 0) return true;
			}
			return true;
		}
		bool decideExist(int idx) {
			if(sum[idx] == sz(states)) return false;
			rrep(i, sz(states)) if(states[i][idx]==0) {
				removeAt(i);
				if(sum[idx] == sz(states)) return true;
			}
			return true;
		}
	...
	bool turn(){
		...
		{
			// dp2[i][j][k] = i番目までの油田まで見て、j個の油田がマスkにある確率
			vector<vector<valarray<float>>> dp2(m+1, vector<valarray<float>>(m+1, valarray<float>(0.f, n*n)));
			{
			...
			vector<float> odds(n*n, 1);
			static const vi dy={-1,0,1,0}, dx={0,1,0,-1};
			rep(i,n*n)if(diggedCount[i] - allocatedCount[i] >= 1) {
				rep(j,4){
					int ny=i/n+dy[j], nx=i%n+dx[j];
					if(ny<0 or nx<0 or ny>=n or nx>=n) continue;
					odds[ny*n+nx] /= pow(diggedCount[i] - allocatedCount[i] + 1, 1);
					// odds[ny*n+nx] /= pow(2, diggedCount[i] - allocatedCount[i]);
					// odds[ny*n+nx] *= dp2[m][0][ny*n+nx];
				}
			}

			rep(i,n*n)if(diggedCount[i]==-1  and allocatedCount[i] == 0){
				float prob = dp2[m][0][i] * odds[i];
				float entropy = prob ? -prob*fasterlog2(prob) : 0;
	...
	vector<vector<float>> value_count;
	bool turn_greedy(){
		if(dp_pool.empty()) {
			value_count.resize(n*n, vector<float>(m+1, 0));

			vector<Field> dp(1, allocatedCount), ndp;
			dp.reserve(GREEDY_SPEC);
			ndp.reserve(GREEDY_SPEC);
			dp_pool.reserve(GREEDY_SPEC);
			vector<int> searchIndex(m); iota(all(searchIndex), 0);
			sort(all(searchIndex), [&](int a, int b){
				return sz(overlaps[a].states) < sz(overlaps[b].states);
			});


			vector<Field> rev_max_placeCount(m+1);
			rev_max_placeCount[m].resize(n*n, 0);
			rrep(i,m) {
				rev_max_placeCount[i] = rev_max_placeCount[i+1];
				if(!overlaps[searchIndex[i]].isDecided){
					Field max_placeCount; max_placeCount.resize(n*n, 0);
					for(auto&state: overlaps[searchIndex[i]].states) {
						max_placeCount |= state;
					}
					rev_max_placeCount[i] += max_placeCount;
				}
			}

			vi enableIdx;
			rep(i,n*n) if(diggedCount[i] != -1) enableIdx.push_back(i);

			rep(i,m-1)if(!overlaps[searchIndex[i]].isDecided) {
				ndp.clear();
				for(auto&state : overlaps[searchIndex[i]].states) {
					for(auto&f: dp) {
						bool ok=true;
						for(auto l: enableIdx){
						// rep(l,n*n)if(diggedCount[l] != -1){
							Cell newValue = f[l] +state[l];

							// NOTE: なんかこれ入れたらスコア下がって草。なんで？？
							if(newValue+rev_max_placeCount[i+1][l] < diggedCount[l]) { // あと全部置けても届かない TODO 置ける個数の最大値は逆向きからの累積MAXでもうちょい絞れる
								ok=false;
								break;
							}
							if(diggedCount[l] < newValue) { // 超えちゃった
								ok=false;
								break;
							}
						}
						if(ok) ndp.push_back(move(f + state));
					}
				}
				swap(dp, ndp);
			}
			if(!overlaps[searchIndex[m-1]].isDecided) {
				for(auto&state : overlaps[searchIndex[m-1]].states) {
					for(auto&f: dp) {
						bool ok=true;
						for(auto l: enableIdx){
							Cell newValue = f[l] +state[l];
							if(newValue != diggedCount[l] ) {
								ok=false;
								break;
							}
						}
						if(ok) dp_pool.push_back(move(f + state));
					}
				}
			} else swap(dp_pool, dp);
			for(auto&f: dp_pool) {
				rep(i,n*n) value_count[i][f[i]]+=1;
			}
		} else {
			auto p = query.history.back();
			rep(i,sz(dp_pool)) {
				if(i >= sz(dp_pool)) break;
				if(dp_pool[i][p] != diggedCount[p]) {
					rep(j,n*n) value_count[j][dp_pool[i][j]]-=1;
					swap(dp_pool[i], dp_pool.back());
					dp_pool.pop_back();
					i--;
				}
			}
		}
		// debug("!greedy", sz(dp_pool));

		// TODO: C4.5の実装

		/* 最も掘ることで情報が大きそうなマスを掘る */
		pair<float, pair<int, int>> maxEntropy = {0, {-1, -1}};
		vector<float> probList(m+1, 0);


		int tgtMin = INF, tgtMax = INF;
		rep(i,n*n)if(diggedCount[i]==-1 and allocatedCount[i] == 0){
			// 属性：マスiを掘る
			vector<float> values = value_count[i];
			rep(j,m+1) values[j] /= sz(dp_pool);

			float entropy = 0;
			int tgtCount=0, minAfterValue=INF, minAfterValueCnt = -1, secondAfterValue=INF, secondAfterValueCnt = -1;
			rep(k,m+1)if(values[k]){
				tgtCount++;
				if(chmin(secondAfterValue, value_count[i][k])) {
					secondAfterValueCnt = k;
				}
				if(secondAfterValue < minAfterValue){
					swap(minAfterValue, secondAfterValue);
					swap(minAfterValueCnt, secondAfterValueCnt);
				}
				entropy -= values[k]*fasterlog2(values[k])/fasterlog2(m);
			...
			bool isDirectAnswer = tgtCount==2 and minAfterValue==1;
			if(chmax(maxEntropy, {entropy, {i, isDirectAnswer}})){
				probList = values;
				tgtMin = minAfterValueCnt;
				tgtMax = secondAfterValueCnt;
			}
		}
		if(maxEntropy.second.first == -1) return false;

		if (maxEntropy.second.second) {
			vi answer;
			for(auto&f : dp_pool) {
				if(f[maxEntropy.second.first] == tgtMin) {
					rep(i,n*n) if(f[i] > 0) {
						answer.push_back(i);
					}
					break;
			...
			auto res = query.sendAnswer(answer);
			if(res == 1) {
				exit(0);
			} else {
				diggedCount[maxEntropy.second.first] = tgtMax;
				return true;
			}
		} else {
			auto cnt = query.singleDig(maxEntropy.second.first);
			diggedCount[maxEntropy.second.first] = cnt;
			return true;
		}
	...
	bool shrink() {
		...
		// 			else cout<<diggedCount[i*n+j] - allocatedCount[i*n+j];
		// 		}
		// 		cout<<endl;
		// 	}
		// 	rep(i,m) out("#  ", i, sz(overlaps[i].states));
		// 	overlaps[6].output();
		// 	out("#====#");
		// }
		rep(i,n*n){
			...
			// 既に個数が分かっているマスについて、
			rep(j,m)if(!overlaps[j].isDecided){
				// もしあり得る油田の個数の最小値が個数通りなら、他のすべての油田はここには無い
				if(diggedCount[i]==l){
					if(!overlaps[j].isExist(i)) {
						if(overlaps[j].decideNoExist(i)) {
							changed = true;
						}
					}
				}
	...
	bool check() {
		...
		// 同型の要素がX個ある時、それぞれがX通りしかないなら割り当ててよい
		{
			...
			for(auto [_, g]: uf.groups()) if(sz(g) >= 2){
				bool ok=true;
				rep(i,1,sz(g)) ok&= isFieldEqual(overlaps[g[0]].sum , overlaps[g[i]].sum);
				if(!ok) continue;
		...
		// cout<<"#================================="<<endl;
		{
			...
			debug(query.queryCount, "\t", Mgr::timer.get(), "\t", pow(10, c));
		}
		...
		// 島についての議論
		{
			...
			// 		cout<<"#    ";
			// 		rep(j,n){
			// 			if(island[i*n+j] == 0) cout<<" ";
			// 			else cout<<island[i*n+j];
			// 		}
			// 		cout<<endl;
			// 	}
			// }
			...
			for(auto&island : islands){
				...
				// 特定のマスをカバー可能な油田がただ1つの形状であれば確定
				{
					...
					rep(i,n*n) if(sz(placeableShapeSet[i]) == 1) {
						...
						if(overlaps[minoIdx].decideExist(i)) {
							// out("# Decided Exist", minoIdx, i/n ,i%n, "=", sz(overlaps[minoIdx].states));
							// overlaps[minoIdx].output();
							isChanged = true;
						}
						if(overlaps[minoIdx].isDecided) {
							// out("#Bloken");
				...
				// 特定のマスをカバー可能な油田がただ1つの形状であれば確定
				{
					...
					rep(i,n*n) if(diggedCount[i]-allocatedCount[i] >= 1) if(sz(placeableShapeSet[i]) == 1) {
						...
						if(sz(stateMatrix[minoIdx][i]) == 1){
							...
							// out("# Loose Decided", minoIdx, i/n ,i%n);
							auto [isValid, afterPlaced] = getStrictIslands(island-overlaps[minoIdx].states[0]);
							islands.insert(islands.end(), all(afterPlaced));
							isChanged = true;
							// return true;
							...
							for(auto idx: stateDeMatrix[minoIdx][i] | ranges::views::reverse) {
								// out("# Loose Removed 2", minoIdx, i/n ,i%n, idx);
	...
	void solve() {
		...
		// {
		// 	ans.clear();
		// 	rep(i,n*n) if(dp_pool[0][i] > 0) ans.push_back(i);
		// }
