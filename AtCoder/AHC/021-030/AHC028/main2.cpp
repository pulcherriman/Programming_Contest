...
int main() { /**/
	...
	Route ansRoute = {INF, {}};
	while(Mgr::timer){
		Route bestRoute = {INF, {}}, innerAnsRoute = {INF, {}};
		int i=0,j=1;
		bool modified = false;
		auto chYakinamashi = [&innerAnsRoute](Route&minCost, Route cost) {
			if(cost<=minCost) {
				if(cost<minCost) innerAnsRoute=cost;
				minCost=cost; return true;
			}
			auto thres = (1-Mgr::timer.persentile())/10/(cost.first-minCost.first);
		...
		while(Mgr::timer){
			...
			if(!chYakinamashi(bestRoute, *min_element(all(routes)))) {
				swap(covers[i], covers[j]);
				// debug("0 NG!");
			}else {
