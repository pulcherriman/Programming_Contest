	...
	string HumansAction() {
...

		{
			...
			for(auto&human:humans){
				...
				if(human.role == 1){
					while(IsExistWall(human.targets.front())){
						human.targets.pop();
					}
					auto target = human.targets.front();
					// 置けるかな
					if (IsPlaceable(human, target)) {
						...
						if(target.x - target.y >= 4) {
							human.targets.push(target.Left().Down());
						} else if(target.x - target.y <= -3) {
							human.targets.push(target.Right().Up());
						}
	...
	int CalculateScore() {
		...
		rep(i,humanCount)if(scores[i]==0){
			...
			for(auto&humanId : containHumans) {
				...
				cout<<"#   "<<humans[humanId].role<<" : "<<humans[humanId].targets.front().y<<","<<humans[humanId].targets.front().x<<endl;
			}
		}
