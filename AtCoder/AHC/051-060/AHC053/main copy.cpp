...
vector<int> simulated_annealing(vector<ll>& A, const vector<ll>& B, vector<int> initial_X, int time_limit_ms) {
    ...
    // 温度スケジュール設定
    double start_temp = current_score * 0.1;  // 初期解の10%
    double end_temp = 500;  // 初期解の0.1%
    int iteration = 0;
    ...
    while(opt_timer) {
        // 時間ベース温度スケジュール
        double time_ratio = (double)opt_timer.get() / time_limit_ms;
        double temperature = start_temp * pow(end_temp / start_temp, time_ratio);
        ...
        if(delta <= 0) {
            // 改善解は常に受容
            accept = true;
        } else if(temperature > end_temp) {
            // 悪化解を確率的に受容
            double prob = exp(-delta / temperature);
            accept = (rnd(0.0, 1.0) < prob);
        }
        ...
        if(iteration % 10000 == 0) {
            debug("Iteration:", iteration, "Current:", current_score, "Best:", best_score, "Temp:", temperature);
        }
    }

    debug("SA completed. Iterations:", iteration, "Best score:", best_score);
    return best_X;
}
...
int main() {
    ...
    rep(i, large_count) {
        ll range = ( u-l) * 0.2;
        ll base_value = l + (ll)i * range / (large_count - 1);
        A[idx++] = min(max(base_value, 1LL), U);
    }

	auto max_exp = log10(u-l);
	auto threshold = 3;
    while(idx < N) {
		double percentile = (double)(idx - large_count) / (N - large_count);
		ll val = pow(10, threshold + (max_exp - threshold + 0.1) * percentile);
		A[idx++] = min(val, U);
    }
    ...
    // 2. シミュレーテッドアニーリングで改善
    debug("Optimizing with simulated annealing...");
    X = simulated_annealing(A, B, X, 1800);  // 1.8秒で最適化
