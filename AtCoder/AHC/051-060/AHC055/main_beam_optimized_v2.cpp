...
struct BeamParams {
    int beam_width = 41;           // ビーム幅：Optuna最適化結果
    int max_depth = 18;            // 探索深度：Optuna最適化結果
    int expand_count = 12;          // 展開候補数：Optuna最適化結果
    double weapon_coeff = 0.8092522845188599;     // 武器価値係数：Optuna最適化結果
    double progress_coeff = 1.629302790259697;   // 進捗係数：Optuna最適化結果
    double cost_coeff = 2.2360948336684814;       // コスト係数：Optuna最適化結果
    double completion_bonus = 76414.6051882853; // 完了ボーナス：Optuna最適化結果
};
    ...
    for (int depth = 0; depth < params.max_depth && !beam.empty(); depth++) {
        ...
        for (const auto& state : beam) {
            ...
            for (int i = 0; i < input.N; i++) {
                ...
                double early_bonus = (8.0 - cost) * 1.390291576875411;

                // 高耐久武器ボーナス - Optuna最適化結果
                double durability_bonus = input.C[i] * 1.3113920380258204;
