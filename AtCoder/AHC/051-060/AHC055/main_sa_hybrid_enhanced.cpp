/**
 * AHC055 - Enhanced Hybrid Structural Simulated Annealing Solution
 * 強化版ハイブリッド構造的近傍操作（HARDNESS_BASED_MOVE改良版）
 */
...
// 貪欲法による残り順序生成
vector<int> generateGreedyOrderFromPoint(const Input& input, const vector<int>& current_order, int start_idx) {
    if (start_idx >= current_order.size()) return current_order;

    vector<int> new_order(current_order.begin(), current_order.begin() + start_idx);
    vector<bool> used(input.N, false);

    // 既に使用済みの宝箱をマーク
    for (int i = 0; i < start_idx; i++) {
        used[current_order[i]] = true;
    }
    ...
    // 新しい順序に追加
    for (int box : remaining) {
        new_order.push_back(box);
    }
...
// 強化版ハイブリッド構造的近傍生成
vector<int> generateEnhancedHybridNeighbor(const vector<int>& order, const Input& input, mt19937& rng, NeighborType type) {
    ...
    switch (type) {
        ...
        case HARDNESS_BASED_MOVE: {
            // 強化版耐久力ベース移動：硬さの小さい宝箱を前方に移動し、その後貪欲法で再生成
            ...
            if (min_idx != -1 && min_idx > 0) {
                ...
                // 移動後の位置から貪欲法で残りを再生成
                new_order = generateGreedyOrderFromPoint(input, new_order, target_pos + 1);
            }
            break;
        }
    }
...
// 強化版ハイブリッド焼きなまし法（統計追跡版）
vector<pair<int, int>> enhancedHybridStructuralSimulatedAnnealingSolution(const Input& input, const SAParams& params = SAParams()) {
    // 1. v2.1解を生成
    ...
    // 近傍操作の重み（SWAP: 55%, INSERT: 40%, HARDNESS_BASED_MOVE: 5%）
    vector<NeighborType> neighbor_types = {SWAP, INSERT, HARDNESS_BASED_MOVE};
    vector<double> neighbor_weights = {0.55, 0.40, 0.05};
    ...
    while (true) {
        ...
        auto new_order = generateEnhancedHybridNeighbor(current_order, input, rng, neighbor_type);
    ...
    // 統計出力
    cerr << "=== Enhanced SA Statistics ===" << endl;
    ...
    vector<string> neighbor_names = {"SWAP", "INSERT", "ENHANCED_HARDNESS_BASED_MOVE"};
    for (int i = 0; i < 3; i++) {
...
void solve() {
    ...
    auto attacks = enhancedHybridStructuralSimulatedAnnealingSolution(input);
