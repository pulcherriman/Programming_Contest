/**
 * AHC055 - Simple Structural Simulated Annealing Solution
 * 軽量化された構造的近傍操作
 */
...
// 簡略化された焼きなまし法
vector<pair<int, int>> simpleStructuralSimulatedAnnealingSolution(const Input& input, const SAParams& params = SAParams()) {
    // 1. v2.1解を生成
    ...
    if (initial_order.empty()) {
        cerr << "Warning: Empty initial order, returning v2.1 solution" << endl;
        return v21_solution;
    }
    ...
    vector<double> neighbor_weights = {0.5, 0.3, 0.2};  // SWAPを重視
...
void solve() {
    ...
    auto attacks = simpleStructuralSimulatedAnnealingSolution(input);
