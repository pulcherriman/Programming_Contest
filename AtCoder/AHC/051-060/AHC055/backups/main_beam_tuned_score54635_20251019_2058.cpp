...
struct BeamParams {
    int beam_width = 20;           // ビーム幅：多様性確保のため増加
    int max_depth = 15;            // 探索深度：序盤重要箱をカバー
    int expand_count = 5;          // 展開候補数：選択肢の多様性
    double weapon_coeff = 1.0;     // 武器価値係数：実削減攻撃数
    double progress_coeff = 4.5;   // 進捗係数：平均硬さ相当
    double cost_coeff = 1.0;       // コスト係数：実攻撃コスト
    double completion_bonus = 50000.0; // 完了ボーナス：十分大きく設定
};
    ...
    for (int depth = 0; depth < params.max_depth && !beam.empty(); depth++) {
        ...
        for (const auto& state : beam) {
            ...
            for (int i = 0; i < input.N; i++) {
                ...
                // 早期開封ボーナス（硬さが低い）
                double early_bonus = (8.0 - cost) * 0.5;

                // 高耐久武器ボーナス
                double durability_bonus = input.C[i] * 0.3;
