...
int main() {
	...
	vvi pathList(T);
	vvi pathListByAddOrder;
	vi laLists;
	{
		vi idx(1,0); rep(i,N) idx.push_back(tList[i]);
		vb used(T, false);

		vector<HashSet<int>> edgeMap0(N), edgeMap1(N);
		rep(i,N) rep(j,N) if(edge[i][j]) edgeMap1[i].insert(j);

		while(true){
			int maxDist = -1; int maxDistIdx = -1;
			vi maxPrevMap;
			rep(i,T) if(!used[i]) {
				deque<tuple<int,int,int>> q; // index, dist, prev
				q.push_back({idx[i], 0, -1});
				vb visited(N, false);
				vi prevs(N, -1);
				while(!q.empty()) {
					auto [p, d, prev] = q.front(); q.pop_front();
					...
					for(auto to: edgeMap0[p]) {
						q.push_front({to, d+0, p});
					}
					for(auto to: edgeMap1[p]) {
						q.push_back({to, d+1, p});
					}
				}
			}
			if(maxDistIdx == -1) break;
			used[maxDistIdx] = true;

			// idx[maxDistIdx] ~ idx[maxDistIdx+1] のパスを復元し、edgeのコストを0に変更
			{
				vi maxPath(1, idx[maxDistIdx+1]);
				int current = idx[maxDistIdx+1];
				while(current != idx[maxDistIdx]) {
					int prev = maxPrevMap[current];
					maxPath.push_back(prev);
					current = prev;
				}
				reverse(all(maxPath));
				// out(idx[maxDistIdx], idx[maxDistIdx+1],":", maxPath);
				pathList[maxDistIdx] = maxPath;
				pathListByAddOrder.push_back(maxPath);

				rep(i, sz(maxPath)-1) {
					int c = maxPath[i], d = maxPath[i+1];
					if(edgeMap1[c].contains(d)) {
						edgeMap1[c].erase(d);
						edgeMap0[c].insert(d);
					}
					if(edgeMap1[d].contains(c)) {
						edgeMap1[d].erase(c);
						edgeMap0[d].insert(c);
					}
				}
	...
	Graph<int> g(N, INF);
	vvi tmpEdge(N, vi(N, 0));
	vi alreadyExist(N, 0);
	rep(i,T){
		auto&path = pathListByAddOrder[i];
		rep(j,sz(path)-1){
			int from = path[j], to = path[j+1];
			if(tmpEdge[from][to]) {
				if(!alreadyExist[to]) {
					alreadyExist[to] = 1;
					laLists.push_back(to);
				}
				continue;
			}
			tmpEdge[from][to] = tmpEdge[to][from] = 1;
			g.add(from, to, 1);
			alreadyExist[to]=1;
			laLists.push_back(to);
		}
	...
	// rep(i, N) rep(j, N) if (edge[i][j]) g.add(i, j, 1);

	vvi distMap(N, vi(N, INF));

	rep(i,T){
		auto path = pathList[i];
		rep(j,sz(path)-1) rep(k,j+1,sz(path)) {
			distMap[path[j]][path[k]] = distMap[path[k]][path[j]] = k-j;
		}
	}


	// 操作DB
	// auto ord = g.dfs(0).preorder;
	// vi et(N); iota(all(et), 0);
	// sort(all(et), [&](int a, int b) { return ord[a] < ord[b]; });
	vi et = laLists;

	vi db(La), blue(Lb, -1);
	rep(i,La) db[i] = et[i % sz(et)];
	out_f(db);
	...
	rep(tIdx, T){
		int t=tList[tIdx];
		out_f("#target", t);
		out_f("#Path", pathList[tIdx]);
		vvi partialEdge(N, vi(N, 0));
		rep(i,sz(pathList[tIdx])-1) {
			int from = pathList[tIdx][i], to = pathList[tIdx][i+1];
			partialEdge[from][to] = partialEdge[to][from] = 1;
		}

		auto distFromGoal = distMap[t];
		distFromGoal = vi(N, INF);
		rep(i,sz(pathList[tIdx])) {
			int from = pathList[tIdx][i];
			distFromGoal[from] = distFromGoal[from] = sz(pathList[tIdx]) - i - 1;
		}
		out_f("#distMap", distFromGoal);
		...
		while(true) {
			...
			// 24 * (1200 - 24) * 12
			rrep(len, Lb, Lb+1) rep(from, La - len + 1) rep(to, Lb - len + 1) {
				// blue[to~to+len] = db[from~from+len]にしたときの
				auto newBlue = blue;
				rep(i, len) newBlue[to+i] = db[from+i];

				bool adjacentSomeone = false;
				rep(i, Lb) if(newBlue[i] != -1) {
					adjacentSomeone = true;
					break;
				}
				if(!adjacentSomeone) continue;

				// currentPosから、できるだけdistが小さい頂点に、newBlueに記述の頂点だけを通って移動
				queue<int> q; q.push(-1);
				vector<pii> distOnlyBlue(Lb, {-1, -1});
				while(!q.empty()){
					int partialPos = q.front(); q.pop();
					rep(i, Lb) if(newBlue[i] != -1 and distOnlyBlue[i].first == -1){
						if(partialPos == -1) {
							if(partialEdge[currentPos][newBlue[i]]) {
								distOnlyBlue[i] = {1, -1};
								q.push(i);
							}
						} else if(partialEdge[newBlue[partialPos]][newBlue[i]]) {
							distOnlyBlue[i] = {distOnlyBlue[partialPos].first + 1, partialPos};
							q.push(i);
						}
					}
				}
				...
				rep(i, Lb) if(newBlue[i] != -1 and distOnlyBlue[i].first != -1) {
					...
					if(chmin(bestNextDist, nextDist)) {
						...
						vi path(1, i);
						while(path.back() != -1) {
							int next = distOnlyBlue[path.back()].second;
							path.push_back(next);
						}
						path.pop_back();
