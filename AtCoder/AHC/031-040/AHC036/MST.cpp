...
public:
    vi solve(Graph<int> &g) {
        ...
        for (int u = 0; u < n; ++u) {
            for (const auto&[e, _] : g.edge[u]) {
                graph[u].push_back(e);
            }
        }

        // 最小全域木を構築
        Graph<int> mst = prim(graph, n);
        ...
        while (odd_degree_nodes.size() > 1) {
            ...
            for (int i : odd_degree_nodes) {
                for (int j : odd_degree_nodes) {
                    if (i != j && shortest_paths[i][j] > max_distance) {
            ...
            while (current != v) {
                for (int next : graph[current]) {
                    if (shortest_paths[next][v] + 1 == shortest_paths[current][v]) {
                        mst.edge[current].push_back({next, 1});
                        mst.edge[next].push_back({current, 1});
                        current = next;
                        break;
                    }
                }
        ...
        for (size_t i = 0; i < path.size() - 1; ++i) {
            ...
            if (shortest_paths[u][v] != 1) {  // 隣接していない場合
                vector<int> segment = reconstruct_path(shortest_paths, u, v, graph);
...
private:
    // 最小全域木 (MST) を構築する
    Graph<int> prim(const vector<vector<int>> &graph, int n) {
        vector<bool> in_mst(n, false);
        Graph<int> mst(n);

        in_mst[0] = true;
		vi rndI(n); iota(all(rndI), 0);
		shuffle(all(rndI), Random);
		rep(n) {
            int u = -1, v = -1;
			for(auto j:rndI) {
                if (in_mst[j]) {
    ...
    // 元の最短経路を復元する
    vector<int> reconstruct_path(const vector<vector<int>> &shortest_paths, int u, int v, const vector<vector<int>> &graph) {
        vector<int> path;
        path.push_back(u);
        while (u != v) {
...
// auto getShortestPath(const Graph<int> &g, int s, int t) {
// 	auto dist = g.bfs(s);
// 	vector<int> path(1, t);
// 	while (path.back() != s) {
// 		for (const auto &[to, _] : g.edge[path.back()]) {
// 			if (dist[to] == dist[path.back()] - 1) {
// 				path.push_back(to);
// 				break;
// 			}
// 		}
...
int main() {
	...
	rep(1){
		...
		if(isContainCycle) {
			debug_f("cyclic");
			outputActions = calcAnswer_OptimizeCyclic(db);
		} else {
			debug_f("universal");
