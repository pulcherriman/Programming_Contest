...
struct GraphSolver600 {
	int evaluate_path(vi path) {
		int okCount = 0;
		for(auto&p:path) if(tSet.contains(p)) okCount++;
		if(okCount == sz(tSet)) return 100000 - sz(path);
		else return sz(path);
	}

	auto find_path_between_two_nodes_bfs_no_direct_edge_no_logging(Graph<int> & g, int u, int v, HashSet<int>&visitedSet) {
		queue<vi> queue;
		queue.push({u});

		while(!queue.empty()) {
			vi path = queue.front(); queue.pop();
			int node = path.back();
			if(sz(path) > 4) break;

			if(node == v && sz(path) > 2) return path;

			for(auto [neighbor,_]:g.edge[node]) {
				if(!visitedSet.contains(neighbor) && find(all(path), neighbor) == path.end()) {
					queue.push(path);
	...
	auto update_path_with_subpath_correctly(vi&original_path, vi&subpath, int u_index) {
		int v_index = u_index+1;
		vi new_path;
		new_path.insert(new_path.end(), original_path.begin(), original_path.begin() + u_index);
		new_path.insert(new_path.end(), all(subpath));
		new_path.insert(new_path.end(), original_path.begin() + v_index + 1, original_path.end());
		return new_path;
	}

	auto perturb_path_avoid_local_optima(Graph<int> & g, vi&path, HashSet<int>& visitedSet) {
		HashMap<int, int> pathIndex;
		rep(i,sz(path)) pathIndex[path[i]] = i;

		vector<pair<int, vi>> candidates;
		rep(vertex,N) if(!visitedSet.contains(vertex))  {
			vi neighbors;
			for(auto [neighbor,_]:g.edge[vertex]) if(visitedSet.contains(neighbor)) neighbors.push_back(neighbor);
			if(sz(neighbors) >= 2) {
				candidates.emplace_back(vertex, neighbors);
			}
		}

		if(candidates.empty()) return path;

		auto [u, u_neighbors_in_path] = candidates[Random(0, sz(candidates))];

		int ai = Random(0, sz(u_neighbors_in_path));
		int bi = Random(0, sz(u_neighbors_in_path));
		while(ai == bi) bi = Random(0, sz(u_neighbors_in_path));

		auto [a_index, b_index] = minmax(pathIndex[u_neighbors_in_path[ai]], pathIndex[u_neighbors_in_path[bi]]);
		vi new_path;
		new_path.insert(new_path.end(), path.begin(), path.begin() + a_index + 1);
		new_path.push_back(u);
		new_path.insert(new_path.end(), path.begin() + b_index, path.end());

		return new_path;
	}

	auto simulated_annealing_with_leaf_insertion(Graph<int> & g, int start, int end) {
		vi current_path = g.bfs(start, end).second;
		int current_score = evaluate_path(current_path);
		vi best_path = current_path;
		int best_score = current_score;

		HashSet<int> visitedSet; for(auto&p:current_path) visitedSet.insert(p);

		int failed_count = 0;
		int reset_count = 0;
		while (true) {
			int idx = Random(0, sz(current_path) - 1);
			int u = current_path[idx], v = current_path[idx + 1];

			visitedSet.erase(v);
			vi subpath = find_path_between_two_nodes_bfs_no_direct_edge_no_logging(g, u, v, visitedSet);
			visitedSet.insert(v);

			if (subpath != vi{u, v}) {
				vi new_path = update_path_with_subpath_correctly(current_path, subpath, idx);
				int new_score = evaluate_path(new_path);

				if (chmax(current_score, new_score)) {
					failed_count = -1;
					current_path = new_path;
					rep(i,1,sz(subpath)-1) visitedSet.insert(subpath[i]);
			...
			failed_count++;
			if (failed_count > sz(current_path) * 2) {
				if (best_score > 10000) break;
				current_path = perturb_path_avoid_local_optima(g, current_path, visitedSet);
				current_score = evaluate_path(current_path);
				failed_count = 0;
				reset_count++;
			}
	...
	auto solve(Graph<int> & g) {
		if(tSet.empty()) rep(i,T) tSet.insert(tList[i]);

		int start_vertex = Random(0, N / 2);
		int end_vertex = Random(N / 2, N);
		auto [path, score] = simulated_annealing_with_leaf_insertion(g, start_vertex, end_vertex);

		HashSet<int> path_set; for(auto&p:path) path_set.insert(p);
		vi final_path;
		for(auto v:path) {
			final_path.push_back(v);
			for(auto [adj,_]:g.edge[v]) {
				if(!path_set.contains(adj) && tSet.contains(adj)) {
					final_path.push_back(adj);
					path_set.insert(adj);
				}
			}
		}

		return make_tuple(score, evaluate_path(final_path), final_path);
	}
};
...
auto calcAnswer_OptimizeCyclic(vi db) {
	...
	rep(i,L) {
		...
		// 集合が連結でない場合、その集合は候補に含まないものとする(cyclicな列の1つめと2つめの区切りで発生)
		if(connectedNodeCount != sz(nodeSet[i])) {
			for(auto&v: nodeSet[i]) {
...
auto compressCyclicDb(vi & db) {
	int defaultLa = La;
	La = sz(db);
	vvi outputActions = calcAnswer_OptimizeCyclic(db);
	vi compressedDb;
	vvi compressedOutputActions;
	...
	for(auto&act: outputActions) {
		if(sz(act) == 3) {
			...
			compressedOutputActions.push_back(newAct);
			score++;
		} else {
			compressedOutputActions.push_back(act);
		}
	}
...
auto testCyclic() {
	vi db;
	static int cyclic600Score = -1, cyclicMSTScore = -1;
	static int cyclicDbCount = 0;
	int is600 = false;
	{
		Graph<int> g(N, INF);
		rep(i, N) rep(j, i) if (edge[i][j]) {
		...
		if(cyclicDbCount++ == 0 or (cyclicDbCount >=2 and cyclic600Score < cyclicMSTScore)) {
			int bestScore = 0, bestAfterScore = 0;
			vi bestPath;
			rep(10){
				auto [score, afterScore, path] = GraphSolver600().solve(g);
				if(chmax(bestAfterScore, afterScore)) {
					bestScore = score;
					bestPath = path;
				}
			}
			...
			bool ok=true;
			HashSet<int> dbSet; for(auto&d:bestPath) dbSet.insert(d);
			for(auto&t:tList) if(!dbSet.contains(t)) ok=false;
			if(ok and bestScore >= 580 and bestAfterScore > 10000) {
				out("#", bestScore);
				out("#", bestPath);
				db =  bestPath;
				is600 = true;
			}else{
				db = GraphSolver().solve(g);
			}
		}else{
			db = GraphSolver().solve(g);
		}
	...
	auto [compressedDb, compressedOutputActions, score] = compressCyclicDb(db);
	if(is600) chmax(cyclic600Score, score);
	else chmax(cyclicMSTScore, score);
...
int main() {
	...
	if(!isCyclicAchieved){
		...
		if(isContainCycle) {
			isCyclicAchieved = true;
			bestDb = db;
			bestOutputActions = calcAnswer_OptimizeCyclic(db);
			int score = 0;
