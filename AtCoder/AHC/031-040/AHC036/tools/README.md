...
rustup update
```
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc036/e5f02df53f30d36e_windows.zip).
...
- `--LA=600` Fix the `L_A` value of the generated inputs to the specified one.
- `--LB=4` Fix the `L_B` value of the generated inputs to the specified one.
...
You can also use a [web visualizer](https://img.atcoder.jp/ahc036/e5f02df53f30d36e.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc036/e5f02df53f30d36e_windows.zip)も用意してあります。
...
- `--LA=600` 生成される入力の `L_A` の値を指定されたものに固定
- `--LB=4` 生成される入力の `L_B` の値を指定されたものに固定
...
より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc036/e5f02df53f30d36e.html?lang=ja)も利用可能です。
