...
def plot_graph(graph, coordinates, path):
    ...
    for i in range(len(path) - 1):
        ...
        x_values = [coordinates[u][0], coordinates[v][0]]
        y_values = [coordinates[u][1], coordinates[v][1]]
        plt.plot(x_values, y_values, color='red', linewidth=2.5, zorder=3)

    # 軸の設定 (0,0) を左上にするために y 軸を反転
    plt.gca().invert_yaxis()

    # グラフの表示
    plt.title(f"Graph with Path Length: {len(path)}")
    plt.show()

plot_graph(graph, coordinates, final_path_time_limited)
