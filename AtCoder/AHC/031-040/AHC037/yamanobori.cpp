...
auto solve(vector<Soda>&soda, vector<Soda> initialRelay) {
	int n=soda.size();
	...
	auto calcCost = [&](int i, int threshold = 1) {
		vector<tuple<int,int,int,int>> candidateList;
		...
		for(auto it1=children[i].begin(); it1!=children[i].end(); it1++){
			for(auto it2=it1; it2!=children[i].end(); it2++){
				...
				candidateList.eb(reduceCost, i, *it1, *it2);
			}
		}
		return candidateList;
	};

	set<tuple<int,int,int,int>> candidates;
	vector<vector<tuple<int,int,int,int>>> candidatesByIndex(n+m);
	...
	while(relay.size() < 4000) {
		auto [maxCost, selectedIndex, c1, c2] = *candidates.rbegin();
		if(maxCost == 0) break;
...
int main() {
	...
	auto bestResult = solve(sodaList, {});
	vector<Soda> initialRelay;
	while(Mgr::timer){
