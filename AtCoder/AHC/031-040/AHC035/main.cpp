...
int getPowerScore (Power& power) {
	// int sum = accumulate(all(power), 0);
	// rep(i,M) if(power[i] == initMaxPower[i]) sum += 1000;
	int sum = 0;
	rep(i,M) sum += 1000 / (initMaxPower[i] - power[i] + 1);
	return sum;
}
...
vi analyze_MaxScore(vector<Power> power) {
	vi sumPower(S, 0);
	rep(i,S) {
		sumPower[i] = getPowerScore(power[i]);
	}
...
vvi analyze(vector<Power> power) {
	...
	rep(i,N*N) {
		rep(j,M) {
			if(power[flatField[i]][j] == initMaxPower[j]) maxConuts[j]++;
		}
	}
	...
	const auto calcScore = [&scoreSums](vector<Power>&power, vi&field) {
		int score = 0;
		rep(i,N)rep(j,N){
			int odds = 4;
			if(i==0 || i==N-1) odds--;
	...
	vi finalField;
	{
		...
		rep(1000) {
			...
			int score = calcScore(power, testField);
		...
		while(counter < 100000){
			...
			rep(i,N*N)rep(j,i) {
				...
				int score = calcScore(power, swappedField);
