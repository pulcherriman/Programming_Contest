...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc038/GhBuR36w_windows.zip).
...
- `--N=15` Fix the $N$ value of the generated inputs to the specified one.
- `--M=23` Fix the $M$ value of the generated inputs to the specified one.
- `--V=5` Fix the $V$ value of the generated inputs to the specified one.
...
You can also use a [web visualizer](https://img.atcoder.jp/ahc038/GhBuR36w.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc038/GhBuR36w_windows.zip)も用意してあります。
...
- `--N=15` 生成される入力の $N$ の値を指定されたものに固定
- `--M=23` 生成される入力の $M$ の値を指定されたものに固定
- `--V=5` 生成される入力の $V$ の値を指定されたものに固定
...
より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc038/GhBuR36w.html?lang=ja)も利用可能です。
