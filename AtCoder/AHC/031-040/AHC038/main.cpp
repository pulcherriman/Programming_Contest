...
template<class K> struct HashSet : __gnu_pbds::gp_hash_table<K, __gnu_pbds::null_type, hash<K>> {
    inline bool contains(const K& k) const { return this->find(k) != this->end(); }
};
#else
...
struct Point {
    ...
    // コンストラクタ
    inline Point(int x_ = 0, int y_ = 0) : x(x_), y(y_) {}
...
struct State {
    ...
    // move コンストラクタと move 代入演算子をデフォルトに
    State(State&&) = default;
...
struct Robot {
    ...
    struct Vertex {
        ...
        Vertex() = default;
        Vertex(int p, int l, int r) : parent(p), length(l), rotation(r) {}
    };
	...
	vector<int> remain_leaves_count;	
    Robot() {}

    // コンストラクタ：頂点数と頂点データを受け取り、ツリーを構築
    Robot(int num_vertices, const Point& init_root_pos, const vector<tuple<int, int>>& vertex_data) {
        V = num_vertices;
		...
		remain_leaves_count.assign(V, 0);
		remain_leaves_count[V-1] = 1;
		rrep(i,V-1) {
			remain_leaves_count[i] += remain_leaves_count[i+1] + is_leaf[i];
		}
    }
    ...
    State find_best_state(const Point& root_diff, const HashSet<Point>& takoyakis, const HashSet<Point>& spots) {
        ...
        best_state.rotations.reserve(V);
        best_state.grab_diffs.reserve(V);
        int max_match_count = -1;

        // 非葉ノードのリストを作成
        vector<int> non_leaf_nodes;
        non_leaf_nodes.reserve(V);
        for (int i = 1; i < V; ++i) {
            if (!is_leaf[i]) non_leaf_nodes.push_back(i);
        }

        // 非葉ノードの全回転パターンを列挙
        int non_leaf_count = static_cast<int>(non_leaf_nodes.size());
        ...
        for (int pattern = 0; pattern < total_patterns; ++pattern) {
            ...
            current_state.rotations.reserve(V);
            current_state.grab_diffs.reserve(V);
            ...
            leaf_possible_positions.reserve(V);
            leaf_possible_rotations.reserve(V);
            ...
            if (match_count > max_match_count) {
                ...
                best_state = std::move(current_state);
            }
        }
	...
	State find_best_state_to_small_V(const Point root_diff, const HashSet<Point>& takoyakis, const HashSet<Point>& spots, int max_count) const {
		vector<int> cumulative_angles(V, 0);
		vector<Point> abs_pos(V, root_pos + root_diff);
		vector<int> rotate_operations(V, 0);
		HashSet<Point> current_takoyakis, current_spots;
		State best_state;
		best_state.root_diff = root_diff;
		best_state.rotations.assign(V, 0);
		best_state.grab_diffs.assign(V, false);

		dfs(1, cumulative_angles, abs_pos, rotate_operations, takoyakis, spots, current_takoyakis, current_spots, best_state, max_count);
    ...
    void calculate_leaf_positions(const State& state, vector<vector<Point>>& leaf_positions, vector<vector<int>>& leaf_rotations) {
        ...
        for (int i = 1; i < V; ++i) {
            ...
            // 葉の場合、全ての回転を試す
            leaf_positions[i].reserve(3);
            leaf_rotations[i].reserve(3);
...

        {
            ...
            for (size_t i = 0; i < leaves.size(); ++i) {
                if (!is_grab[leaves[i]]) {
                    int leaf = leaves[i];
                    bool found = false;
                    for (size_t j = 0; j < leaf_positions[leaf].size(); ++j) {
            ...
            match_count += static_cast<int>(result.size());
    ...
    vector<pair<Point, int>> solve_biparate_matching_single(const vector<Point>& leaf_positions) {
        int n = static_cast<int>(leaf_positions.size());
    ...
    // rotate_point関数の最適化
    inline Point rotate_point(int length, int rot) const {
        rot = (rot % 4 + 4) % 4;
        switch (rot) {
            case 0:
    ...
    State fail_safe(int N, const HashSet<Point>& takoyakis, const HashSet<Point>& spots) {
        ...
        for (int i = 0; i < V; ++i) if (is_grab[i]) {
        ...
        if (after_root_pos.x < 0 || after_root_pos.x >= N || after_root_pos.y < 0 ||
            ...
            fail_safe_state.rotations.reserve(V);
            for (int i = 1; i < V; ++i) { fail_safe_state.rotations[i] = ANGLES[Random(0, 3)]; }
            fail_safe_state.grab_diffs.assign(V, false);
            fail_safe_state.grab_diffs.reserve(V);
	...
	void dfs(
		int target_vertex,	// 探索対象の頂点
		vector<int> &cumulative_angles,	// 各頂点の累積回転量
		vector<Point> &abs_pos,	// 各頂点の絶対座標
		vector<int>& rotate_operations,
		const HashSet<Point> &takoyakis,
		const HashSet<Point> &spots,
		HashSet<Point> & current_takoyakis, // 現在操作できたたこ焼きの座標
		HashSet<Point> & current_spots, // 現在操作できたこ焼きの座標
		State &best_state,
		int &max_applied_count
	) const {
		int parent_vertex = vertices[target_vertex].parent;

		// 今の頂点からV-1番まですべてを使ってもmax_applied_countを超えることがない場合、探索を打ち切る
		int applied_count = current_takoyakis.size() + current_spots.size();
		if(applied_count + remain_leaves_count[target_vertex] <= max_applied_count) {
			return;
		}

		// 回転操作を配列に格納してループを展開
		constexpr int angles[] = {3,1,0};
		for(int op_index = 0; op_index < 3; ++op_index) {
			int angle = angles[op_index];
			int new_angle = (cumulative_angles[parent_vertex] + angle) % 4;
			Point rotated_rel = rotate_point(vertices[target_vertex].length, vertices[target_vertex].rotation + new_angle);
			Point current_abs = abs_pos[parent_vertex] + rotated_rel;

			abs_pos[target_vertex] = current_abs;
			cumulative_angles[target_vertex] = new_angle;
			rotate_operations[target_vertex] = angle;

			bool is_applied_point = false;
			if(is_leaf[target_vertex]) {
				const auto& target = is_grab[target_vertex] ? spots : takoyakis;
				auto& current_target = is_grab[target_vertex] ? current_spots : current_takoyakis;
				if(target.contains(current_abs) && !current_target.contains(current_abs)) {
					is_applied_point = true;
					current_target.insert(current_abs);
				}
			}

			if(target_vertex == V-1) {
				// 全ての頂点を探索した場合
				int total_applied_count = current_takoyakis.size() + current_spots.size();
				if(total_applied_count > max_applied_count) {
					max_applied_count = total_applied_count;
					best_state.rotations = rotate_operations;
					best_state.grab_diffs.assign(V, false);

					HashSet<Point> applied_pos;

					for(int i = 2; i < V; ++i) {
						if(is_leaf[i]) {
							const auto& target = is_grab[i] ? current_spots : current_takoyakis;
							if(target.contains(abs_pos[i]) && !applied_pos.contains(abs_pos[i])) {
								best_state.grab_diffs[i] = true;
								applied_pos.insert(abs_pos[i]);
							}
						}
					}
				}
				...
				if(is_applied_point) {
					current_takoyakis.erase(current_abs);
					current_spots.erase(current_abs);
				}

				continue;
			}

			if(!is_applied_point && (current_takoyakis.size() + current_spots.size() + remain_leaves_count[target_vertex+1] <= max_applied_count)) {
				continue;
			}

			// 次の頂点を見るdfs
			dfs(target_vertex + 1, cumulative_angles, abs_pos, rotate_operations, takoyakis, spots, current_takoyakis, current_spots, best_state, max_applied_count);
...
auto solve(int N, int M, int V, const HashSet<Point>& takoyakis, const HashSet<Point>& spots, int best_turn) {
    Result result;
    ...
    HashSet<Point> takoyakis_copy = takoyakis;
    HashSet<Point> spots_copy = spots;

    while (!spots_copy.empty()) {
        // 最適状態の取得
        ...
        for (const auto& diff : diffs) {
            ...
            State state = V >= 10 ? result.robot.find_best_state(diff, takoyakis_copy, spots_copy) : result.robot.find_best_state_to_small_V(diff, takoyakis_copy, spots_copy, best_score);
            int score = 0;
        ...
        if (best_score == 0) {
            best_state = result.robot.fail_safe(N, takoyakis_copy, spots_copy);
        }

        result.robot.apply_state(best_state, takoyakis_copy, spots_copy);
        result.turn++;
        result.states.push_back(std::move(best_state));
...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
        def(string, s);
        for (int j = 0; j < N; ++j) if (s[j] == '1') { takoyakis.insert(Point(j, i)); }
    }
    ...
    for (int i = 1; i < bestResult.robot.V; ++i) {
    ...
    for (auto& state : bestResult.states) {
        string action;
        action.reserve(2 * bestResult.robot.V);
        action +=
