...
struct Player {
	...
	vvb used_hr_ed, used_diag_ed;
	unordered_set<pii,phash> marks_st, marks_diag_st;
	...
	void Init(){
		...
		rep(i,n) rep(j,n) {
			...
			if(field.f[i][j] == 1){
				...
				marks_st.emplace(i,j);
				marks_diag_st.emplace(i,j);
			}
		}
	}
	...
	int modify = Random(0,2);

	bool Analyze(bool shrink = false){

		vp marks_ed, marks_diag_ed;
		...
		int minLength = 20;
		vector<tuple<int, int, bool, Rect>> pq;
		for(auto[i,j]:marks_st)
		for(auto&[i2,j2]:marks_ed)if(i<i2 and j!=j2){
			if(shrink and (i + min(j,j2)) % 2 == modify) continue;
			...
			Rect points = {pii(i2,j), pii(i,j), pii(i,j2), pii(i2,j2)};
			if(exist2) swap(points[0], points[2]);

			chmin(minLength, len);
			const int score = calcScore(points[0].first, points[0].second);
			pq.emplace_back(len, -score, true, points);
		}
		...
		for(auto[i,j]:marks_diag_st)for(auto&[i2,j2]:marks_diag_ed)if(i!=i2 or j!=j2){
			...
			Rect points  = {pii(i4,j4), pii(i,j), pii(i3,j3), pii(i2,j2)};
			if(exist2) swap(points[0], points[2]);
			
			chmin(minLength, len);
			const int score = calcScore(points[0].first, points[0].second);
			pq.emplace_back(len, score, false, points);
		}

		if(pq.empty()) return false;


		int length = min(3, (int)pq.size());
		sort(all(pq));

		auto [len, score, isHorizontal, points] = pq[Random(0, length)];
		
		if(isHorizontal) AddRectHorizontal(points);
		else AddRectDiagonal(points);
	...
	void AddRectHorizontal(Rect&rect){
		...
		action.push_back(rect);
		{
			...
			marks_st.insert(rect[0]);
			marks_diag_st.insert(rect[0]);
		}
		...
		marks_st.erase({minY,minX});
		marks_st.erase({minY,maxX});
	}
	...
	void AddRectDiagonal(Rect&rect){
		...
		{ // diagonal
			...
			if(rect[0].first + rect[0].second == base){
				...
				marks_diag_st.erase(rect[0]);
				marks_diag_st.erase(rect[1]);
...
int main() {
	...
	while(Mgr::timer){
		...
		while(Mgr::timer) {
			if(!player.Analyze(true)){
				if(!player.Analyze(false)) break;
			}
		}
		debug_f("#",challenge," | score :", (int)round(player.totalScore * scoreBase), "/", player.action.size(), "(time :", Mgr::timer.get(), ")");

		if(chmax(bestScore, player.totalScore)){
			bestPlayer = player;
		}
	}
	debug_f("Final Score :", (int)round(bestScore * scoreBase));
	bestPlayer.Output();
}
