...
struct Manager {
	...
	static inline Timer baseTimer = Timer(TLE-30);
	Manager() {
...
Pt p;
// 方向, 0123->ULDR
int dys[]={-1,0,1,0},dxs[]={0,-1,0,1};
array<array<array<bool, 4>, n>, n> edge;

using Pred = array<array<Pt, n>, n>;
Pt walk(Pred&c, int d){
	Pred r;
	fill(r[0].begin(), r[n-1].end(), 0);
	rep(i,n)rep(j,n){
		if(edge[i][j][d]){
			r[i][j] += c[i][j] * p;
			r[i+dys[d]][j+dxs[d]] += c[i][j] * (1-p);
		}else{
			r[i][j] += c[i][j];
		}
	}
...
int calc(string s){
	Pred current;
	fill(current[0].begin(), current[n-1].end(), 0);
	current[sy][sx]=1;
	...
	rep(i,s.size()){
		...
		turn++;
		Pt goal=0;
		if(c=='U')goal = walk(current, 0);
		if(c=='L')goal = walk(current, 1);
		if(c=='D')goal = walk(current, 2);
		if(c=='R')goal = walk(current, 3);
		S += goal * (401-turn);
	}
	return (int)round(S * 250000);
}
...
string nearest() {
	...
	while(!q.empty()){
		...
		dist[y][x]=c;
		if(y==ty && x==tx) break;
		rep(d,4)if(edge[y][x][d]){
			int ny=y+dys[d], nx=x+dxs[d];
			if(dist[ny][nx]==-1) q.emplace(ny,nx,c+1);
		}
	}
	while(!q.empty())q.pop();
	...
	int cy=ty, cx=tx;
	while(cy!=sy || cx!=sx){
		rep(d,4)if(edge[cy][cx][d]){
			int ny=cy+dys[d], nx=cx+dxs[d];
			if(dist[ny][nx]==dist[cy][cx]-1) {
				cy=ny, cx=nx;
				ans.push_back(d==0?'D':d==1?'R':d==2?'U':'L');
				break;
			}
		}
	}
...
string modify_add(string&s){
	int d=Random(0,s.size());
	return s.substr(0,d)+"LRUD"[Random(0,4)]+s.substr(d);
}

string modify_remove(string&s){
	int d=Random(0,s.size());
	string r=s.substr(0,d)+s.substr(d+1);
	return modify_add(r);
}

string modify_change(string&s){
	string r=s;
	int d=Random(0,s.size());
	r[d]="LRUD"[Random(0,4)];
	return r;
...
int main() {
	...
	string current = nearest();
	debug(current);

	int bestScore = calc(current);
	int challenge=0, fail_count=0;
	string bestAnswer=current;

	function<string(string&)> modifiers[] = {
		modify_add,
		modify_remove,
		modify_change,
	};
	...
	// return 0;
	bool pre_run = true;
	while(Manager::baseTimer){
		++challenge;

		MODE mode=NONE;
		int repeat=1;
		if(current.size() < 200) {
			if(pre_run or Random(0,4) != 0) {
				mode=MODE_ADD;
			} else {
				mode=MODE_CHANGE;
			}
		...
		if(chmax(bestScore, score)){
			...
			current = s;
			fail_count=0;
			debug("cnt:", challenge,"\tmode:", (int)mode, " len:", s.size(), "\t(", Manager::baseTimer.get(), "ms)\t", score);
		} else {
			fail_count++;
		}
	}
	print(bestAnswer);
	debug_f(bestScore, challenge);
}