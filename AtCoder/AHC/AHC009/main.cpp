...
struct Manager {
	...
	static inline Timer baseTimer = Timer(TLE-10);
	Manager() {
...
string modify_add(string&s, bool to_down=false){
	int d=Random(0,s.size());
	return s.substr(0,d)+"LRUDRD"[Random(0,to_down ? 6 : 4)]+s.substr(d);
}
...
int main() {
	...
	rep(10){
		...
		while(current.size() < 200){
			current = modify_add(current, true);
		}
		if(chmax(bestScore, calc(current))) bestAnswer=current;
	}
	current = bestAnswer;

	while(Manager::baseTimer){
		string s=modify_remove(current);
