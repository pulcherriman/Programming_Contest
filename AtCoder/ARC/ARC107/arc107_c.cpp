...
void Main(){
	...
	rep(i,n)range(j,i+1,n){
		bool ok=true;
		rep(l,n)ok&=a[i][l]+a[j][l]<=k;
		if(ok) y.unite(i,j);
	}
	rep(i,n)range(j,i+1,n){
		bool ok=true;
		rep(l,n)ok&=a[l][i]+a[l][j]<=k;
		if(ok) x.unite(i,j);
	}

	auto yc=vec(0,n+1), xc=vec(0,n+1);
	rep(i,n)yc[y.size(i)]++;
	rep(i,n)xc[x.size(i)]++;
	range(i,1,n+1)yc[i]/=i;
	range(i,1,n+1)xc[i]/=i;

	mymint ans=1;
	setMod();
	range(i,1,n+1)loop(yc[i])ans*=fac[i];
	range(i,1,n+1)loop(xc[i])ans*=fac[i];
