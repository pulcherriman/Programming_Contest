...
int main() {
	...
	def(string,s);
	string invs=s;
	rep(i,n)if(s[i]=='d')invs[i]='p';
	else invs[i]='d';

	string ans=s;
	if(count(all(s), 'd') == sz(s)) {
		out(s);
		return 0;
	}
	...
	rep(r,l,n){
		//[l,r]
		string rev = invs.substr(l,r-l+1);
		reverse(all(rev));
		string s2=s.substr(0,l) + rev + s.substr(r+1);
		debug(s.substr(0,l) , rev , s.substr(r+1));
		chmin(ans,s2);
	}
	out(ans);
