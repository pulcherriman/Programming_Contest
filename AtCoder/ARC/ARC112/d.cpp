...
int main(){
    ...
    if(r+c==0){
        ...
        UnionFind uf(h*w*2);
        f[0][0]=f[0][w-1]=f[h-1][0]=f[h-1][w-1]='#';
        
        rep(i,h){
            ll p=-1;
            rep(j,w)if(f[i][j]=='#'){
                uf.unite(i*w+j,i*w+j+h*w);
                if(p==-1)p=j;
                else {
                    uf.unite(i*w+j,i*w+p);
                }
            }
        }
        ...
        rep(j,w){
            ...
            rep(i,h)if(f[i][j]=='#'){
                ...
                if(p==-1)p=i;
                    ...
                    uf.unite(i*w+j,p*w+j);
                }
            }
        }

        map<ll,ll> mp;
        rep(i,h)rep(j,w)mp[uf.size(i*w+j)]++;
        ll island=0;
        for(auto [x,y]:mp)if(x!=1)island+=y/(x/2);
        // puta(island);puta(mp);

        puta(min(h-r-2, w-c-2) + max((isouter ? 0ll : 1ll), island-1));
    }
    return 0;
}