...
int main(){
    ...
    vl par(n-1); cin>>par;
    Graph g(n);
    rep(i,n-1)g.add(par[i]-1,i+1,1,true);

    vl score(n,0),sz(n,0),cng(n,0);
    stack<ll> st,st2;
    vb ret(n,0);
    st.emplace(0);
    while(!st.empty()){
        ...
        if(ret[p]==1){
            ...
            for(auto&[to,_]:g.edge[p]){
                //toの情報がそろってる
                totalsz+=sz[to];
                if(cng[to]==1)score[p]+=score[to];
                else cngd.emplace_back(score[to],size[to]-score[to]);
                // dump(to,sz[to], cng[to]);
            }
            sort(rall(cngd));
            rep(i,cngd.size()){
                if(i%2==0)score[p]+=cngd[i].first;
                else score[p]+=cngd[i].second;
            }
            sz[p]=1+totalsz;
            cng[p]=sz[p]%2;
            continue;
