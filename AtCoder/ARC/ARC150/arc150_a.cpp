...
bool solve(){
	def(ll,n,k);
	def(string,s);
	ll left_1=n, right_1=-1;
	rep(i,n){
		if(s[i]=='1'){
			left_1=min(left_1,i);
			right_1=max(right_1,i);
		}
	}
	if(left_1==n) {
		ll hatena_len=0;
		...
		rep(i,n){
			if(s[i]=='?') hatena_len++;
				...
				if(hatena_len==k) ok++;
				else if(hatena_len>k) return false;
				hatena_len=0;
			}
		}
		if(hatena_len==k) ok++;
	...
	ll len  = right_1-left_1+1;
	if(len > k) return false;
	rep(i,left_1,right_1+1){
		if(s[i]=='0'){
			return false;
		}
	}
	if(len == k) return true;
	ll cnt_hatena_l=0, cnt_hatena_r=0;
	rrep(i,left_1){
		if(s[i]=='?'){
			cnt_hatena_l++;
		}else break;
	...
	if(cnt_hatena_l+cnt_hatena_r+len < k) return false;
	if(cnt_hatena_l * cnt_hatena_r == 0) return true;
	if(cnt_hatena_l+cnt_hatena_r+len == k) return true;
	return false;
}
