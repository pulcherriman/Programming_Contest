...
int main() { /**/
	...
	mymint ans=0;
	rep(ab,m+1)rep(bc,m+1)rep(ca,m+1)rep(i,m+1){
		if(ab+bc+ca+i*2>m) continue;
		auto val = nCr(an, ab+i) * nCr(bn, bc+i) * nCr(cn, ca+i) * nCr(an-ab-i, ca) * nCr(bn-bc-i, ab) * nCr(cn-ca-i, bc);
		ans += val * (i==0 ? 1 : 2);
	}
	out(ans);
}