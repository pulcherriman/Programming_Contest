...
int main() { /**/
	...
	rep(i,n+1)rep(j,n) op[i][j+1] += op[i][j];
	rrep(i,n)rep(j,n+1) op[i][j] += op[i+1][j];

	auto getOp = [&](int l, int r, int k) {
		return op[l][r] - (l<k ? op[l][k-1] : 0) - (k<r ? op[k+1][r] : 0) > 0;
	};

	HashMap<pii, int> memo;
	function<int(int, int)> rec = [&](int l, int r) {
		if(l > r) return 0;
		if(memo.contains({l,r})) return memo[{l, r}];
		int ans = 0;
		rep(i,l,r+1){
			if(!getOp(l, r, i)) continue;
			chmax(ans, rec(l, i-1) + rec(i+1, r) + 1);
		}
		return memo[{l, r}]=ans;
	};
