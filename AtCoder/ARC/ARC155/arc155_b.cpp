...
int main() {
	...
	rep(q){
		...
		if(t==1){
			...
			ll l_l = *(--st.upper_bound(l));
			ll r_l = *(st.upper_bound(l));
			ll l_r = *(--st.upper_bound(r));
			ll r_r = *(st.upper_bound(r));
			debug(l_l,r_l,l_r,r_r, ":",l,r);
			ll ans = min({
				abs(l_l-l),
				abs(l_r-l),
				abs(r_l-l),
			...
			if(l<=r_l and r_l<=r) ans = 0;
			if(l<=l_r and l_r<=r) ans = 0;
