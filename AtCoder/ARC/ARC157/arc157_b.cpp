...
int main() {
	...
	if(x>k){
		...
		bool beginning = true;
		rep(i,n){
			if(s[i]=='Y')beginning = false;
			if(s[i]=='X'){
				int j = i;
				while(j<n&&s[j]=='X')j++;
				if(beginning)pq.emplace(INF,j-1,-1);
				else if(j==n)pq.emplace(INF,i,1);
				else pq.emplace(j-i,i,1);
				i = j-1;
			}
		}
		int ans = 0;
		rep(i,n-1)if(s[i]=='Y'&&s[i+1]=='Y')ans++;
		if(count(all(s),'Y')==0)ans = -1;
		while(k){
			auto [t,pos,diff] = pq.top(); pq.pop();
			ans++;
			t--;
			s[pos] = 'Y';
			if(t) pq.emplace(t, pos+diff, diff);
			else ans++;
			k--;
		}
		ans = 0;
