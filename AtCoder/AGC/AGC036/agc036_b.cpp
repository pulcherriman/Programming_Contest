...
void Main(){
	geta(ll, n, k);
	getv(a,0ll,n);
	auto jump=vec(-1ll,1,n);
	{
		vl memoidx(200001,-1);
		rep(i,n*2){
			ll v=a[i%n];
			if(memoidx[v]!=-1)jump[0][memoidx[v]%n]=i-memoidx[v]+1;
			memoidx[v]=i;
		}
	}

	rep(i,10000){
		jump.emplace_back(n,-1);
		rep(j,n){
			if(jump[i][j] + jump[i][(j+jump[i][j])%n]<n*k)
				jump[i+1][j]=jump[i][j] + jump[i][(j+jump[i][j])%n];
			else jump[i+1][j]=LINF;
		}
		if(min(jump[i+1])>=n*k)break;
	}

	ll p=0;
	while(true){
		bool moved=false;
		rrep(i,jump.size())if(p+jump[i][p%n]<n*k){
			p+=jump[i][p%n];
			moved=true;
			break;
		}
