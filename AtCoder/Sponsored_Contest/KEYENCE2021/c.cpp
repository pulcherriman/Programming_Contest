...
int main(){
    ...
    rep(i,h)rep(j,w){
        if(i>0){
            if(in[i-1][j]&2)dp[i][j]+=dp[i-1][j];
            else if(in[i-1][j]==0)dp[i][j]+=dp[i-1][j]*2/3;
        }
        if(j>0){
            if(in[i][j-1]&1)dp[i][j]+=dp[i][j-1];
            else if(in[i][j-1]==0)dp[i][j]+=dp[i][j-1]*2/3;
        }
    }
    puta(dp[h-1][w-1]);
    return 0;
