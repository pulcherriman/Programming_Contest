...
int main(){
	...
	double x,y,r,sx,sy; int k;
	cin>>x>>y>>r>>sx>>sy>>k;
	auto sr=hypot(sx,sy);
	auto st=sr!=0?acos(sx/sr):0;
	auto dr=hypot(x,y);
	auto dt=dr!=0?acos(x/dr):0;
	auto x2=x*cos(dt-st)-y*sin(dt-st);
	auto y2=x*sin(dt-st)+y*cos(dt-st);
	x=x2,y=y2;
	sx=sr,sy=0;
	VP ps={P(sx,sy),P(sx,sy)};
	bool isFin=false;
	rep(i,k+1){
		VP nxp;
		double ans=0;
		...
		for(auto rig:ps){
			D maxR=0;
			P nxpp;
			for(auto p:tangentPoints(P(x,y),r,rig)){
				VP p1=crosspointLC(rig,p,P(0,0),1);
				P p2=p1[0].X>p1[1].X?p1[1]:p1[0];
				auto pr=hypot(p2.X,p2.Y);
				if(chmax(maxR,pr!=0?acos(p2.X/pr):0)){
					nxpp=p2;
				}
			}
			l[f]=L(rig,nxpp);
		...
		cout<<l<<endl;
		ps=nxp;
		//isFin |= (i>0 and isecLL(l[0].fs, l[0].sc, l[1].fs, l[1].sc));
		fcout(10)<<(isFin?1:min(1.0,ans/2/PI))<<endl;
	}
