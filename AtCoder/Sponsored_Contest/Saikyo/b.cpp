...
int main(){
	...
	rep(i,n){ainv[i].resize(n,0);}
	rep(i,n){b[i].resize(n,0);}
	...
	// PrintMatrix(ainv);
	rep(i,n)rep(j,n)if(ainv[i][j]>0)ainv[i][j]=1;else ainv[i][j]=0;
	// PrintMatrix(ainv);
	b = Mul(ainv,c);
	// PrintMatrix(b);
	rep(i,n)rep(j,n)if(b[i][j]>0)b[i][j]=1;else b[i][j]=0;
	rep(i,n){
