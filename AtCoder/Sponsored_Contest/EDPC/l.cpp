...
int main(){
	...
	vvl dp(n+1,vl(n,0));
	rep(i,n+1){
		fill(all(dp[i]),i%2?0:0);
	}
	bool turn=n%2;//isFirstTurn
	range(i,1,n+1){
		rep(j,n){//[j,j+i)
			if(i+j>n)continue;
			if(turn){
				if(j>0)chmax(dp[i+1][j-1],dp[i][j]+a[j-1]);
				if(j+1<n)chmax(dp[i+1][j],dp[i][j]+a[j+i-1]);
			}else{
				if(j>0)chmin(dp[i+1][j-1],dp[i][j]-a[j-1]);
				if(j+1<n)chmin(dp[i+1][j],dp[i][j]-a[j+i-1]);
			}
		}
		cout<<dp[i+1]<<endl;
		turn^=true;
