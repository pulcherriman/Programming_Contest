...
int main(){
	...
	vector<vvd> dp(a[0]+10,vvd(a[1]+10,vd(a[2]+10,0)));
	rep(k,a[2]+9)rep(j,a[1]+9)rep(i,a[0]+9){
	//rep(i,a[0])rep(j,a[1])rep(k,a[2]){
		dp[i][j][k]+=1;
		double t=i+j+k;
		     dp[i+1][j][k]  +=(t+1)/(i+1)*dp[i][j][k];
		if(i)dp[i-1][j+1][k]+=t/(j+1)*dp[i][j][k];
		if(j)dp[i][j-1][k+1]+=t/(k+1)*dp[i][j][k];
	}
