...
int main(){
	...
	cin>>n>>a>>b>>c;
	mint p=0,q=(100-c)^(n+0_m);
	rep(x,n){
		mint da=a^(n*1_m),db=b^(x*1_m),dc=nCr(n+x-1,x),dd=(100-c)^(x+0_m);
		puta(da,db,dc,dd,q);
		p+=da*db*dc*dd;
	}
	mint aa=p*(q^(MOD-2_m))^(MOD-2_m);

	p=0;
	rep(x,n){
		mint da=a^(x*1_m),db=b^(n*1_m),dc=nCr(n+x-1,x),dd=(100-c)^(x+0_m);
		puta(da,db,dc,dd,q);
		p+=da*db*dc*dd;
	}
	mint ab=p*(q^(MOD-2_m))^(MOD-2_m);
