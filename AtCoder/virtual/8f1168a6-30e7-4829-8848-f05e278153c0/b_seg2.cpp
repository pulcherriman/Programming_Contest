...
struct S{
    modint998244353 sum, product, gcd, min, max;
    static S create(modint998244353 v){ return S{v,v,v,v,v}; }
    static S op(S a, S b){
        return S{
            a.sum+b.sum,
            a.product*b.product,
            std::gcd(a.gcd.val(), b.gcd.val()),
            std::min(a.min.val(), b.min.val()),
            std::max(a.max.val(),b.max.val())
        };
    }
    static S e(){ return S{0,1,1,LINF,0}; }
};
int main(){
    cin.tie(0);
    ...
    segtree<S, S::op, S::e> st(n*2+1);
    st.set(n,S::create(1));

    range(i,n+1,n*2){
        modint998244353 val=0;
        rep(j,k){
            val+=st.prod(i-r[j], i-l[j]+1).sum;
        }
        st.set(i, S::create(val));
    }
    puta(st.get(n*2-1).sum.val());
