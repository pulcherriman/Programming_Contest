...
int main(){
    ...
    vvl dp(h, vl(w, LINF));
    dp[0][0]= f[0][0]=='#' ? 1 : 0;
    auto btm=[&](ll y, ll x){return (f[y][x]=='.' && f[y+1][x]=='#') ? 1 : 0;};
    auto rgt=[&](ll y, ll x){return (f[y][x]=='.' && f[y][x+1]=='#') ? 1 : 0;};
    rep(i,h)rep(j,w){
        if(i!=h-1) chmin(dp[i+1][j], dp[i][j] + btm(i,j));
        if(j!=w-1) chmin(dp[i][j+1], dp[i][j] + rgt(i,j));
    }
    puta(dp[h-1][w-1]);
